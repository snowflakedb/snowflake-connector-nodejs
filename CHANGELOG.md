...
- Fixed bugs in `noProxy` and `NO_PROXY` handling:
  ...
  - `.` was incorrectly matching as any character instead of a literal dot (snowflakedb/snowflake-connector-nodejs#TODO)
  - Partial strings were incorrectly matching instead of requiring full destination match (snowflakedb/snowflake-connector-nodejs#TODO)
