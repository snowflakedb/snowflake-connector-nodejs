...
export RSA_KEY_PATH_AWS_AZURE="$THIS_DIR/wif/rsa_wif_aws_azure"
export RSA_KEY_PATH_GCP="$THIS_DIR/wif/rsa_wif_gcp"
export PARAMETERS_FILE_PATH="$THIS_DIR/wif/parameters_wif.json"
...
run_tests_and_set_result() {
  ...
  ssh -i "$rsa_key_path" -o IdentitiesOnly=yes -p 443 "$host" \
    BRANCH="$BRANCH" \
        ...
        snowflakedb/client-nodejs-chainguard-node18-test:1 \
          bash -c "
            ...
            if [[ \"\$BRANCH\" =~ ^PR-[0-9]+\$ ]]; then
              wget -O - https://github.com/snowflakedb/snowflake-connector-nodejs/archive/refs/pull/\$(echo \$BRANCH | cut -d- -f2)/head.tar.gz | tar -xz
            else
              wget -O - https://github.com/snowflakedb/snowflake-connector-nodejs/archive/refs/heads/$BRANCH.tar.gz | tar -xz
            fi
