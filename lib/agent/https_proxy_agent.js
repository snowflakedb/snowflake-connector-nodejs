...
function createHttpsProxyAgent(opts) {
  ...
  const { host: hostname, port, user: username, password, protocol: rawProtocol, useForOCSP: useForOCSP, ...agentOptions } = opts;
  const protocol = rawProtocol.endsWith(':') ? rawProtocol : `${rawProtocol}:`;
  return new SnowflakeHttpsProxyAgent({ hostname, port, username, password, protocol, useForOCSP }, agentOptions);
}
...
class SnowflakeHttpsProxyAgent extends HttpsProxyAgent {
  ...
  async connect(req, opts) {
    ...
    if (socket instanceof tls.TLSSocket) {
      SocketUtil.secureSocket(socket, opts.host, opts.useForOCSP ? this : null);
    }
    return socket;
  }
}
