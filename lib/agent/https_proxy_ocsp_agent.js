...
function connect(req, opts, fn)
{
  ...
  if (process.env.HTTP_PROXY || process.env.HTTPS_PROXY) {
    const envHttpProxy = process.env.HTTP_PROXY ? 'HTTP_PROXY: ' + process.env.HTTP_PROXY : 'HTTP_PROXY: <unset>'
    const envHttpsProxy = process.env.HTTPS_PROXY ? 'HTTPS_PROXY: ' + process.env.HTTPS_PROXY : 'HTTPS_PROXY: <unset>'
    const envNoProxy = process.env.NO_PROXY ? 'NO_PROXY: ' + process.env.NO_PROXY : 'NO_PROXY: <unset>'
  ...
  const prxPort = proxy.port ? ':' + proxy.port : ''
  const prxUser = proxy.auth ? ' (user: ' + proxy.auth.split(':')[0] + ')' : ''
