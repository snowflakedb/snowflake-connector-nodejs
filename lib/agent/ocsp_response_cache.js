...
function OcspResponseCache()
{
  ...
  {
    Logger.getInstance().debug("Failed to read a OCSP cache file. %s", e);
  }
  ...
  this.downloadCache = function (cb)
  {
    ...
    function checkOCSPResponse(err, cacheContent)
    {
      ...
      {
        Logger.getInstance()
          .debug("Failed to download a OCSP cache file. %s. Ignored", err);
        return cb(err, false);
      }
      try
      {
  ...
  this.deleteCache = function ()
  {
    ...
    {
      Logger.getInstance()
        .debug("failed to delete cache file. %s, %s", cacheFileName, e);
    }
  };
