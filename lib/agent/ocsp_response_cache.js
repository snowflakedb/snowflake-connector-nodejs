...
function OcspResponseCache() {
  ...
  function validateCacheEntry(certIdBase64, ocspResponseBase64) {
    ...
    try {
      ...
      // The on-disk / cache-server store is keyed only by CertID and does not
      // carry the issuer certificate, so the responder signature cannot be
      // verified while loading entries. This is a structural pre-filter only:
      // the responder signature is verified authoritatively at use-time, where
      // the real issuer certificate is available (see socket_util.validateCert
      // and check.js, which both pass the issuer and the expected CertID into
      // verifyOCSPResponse). An entry that survives this pre-filter is never
      // trusted on its own; it is re-verified against the issuer before use.
