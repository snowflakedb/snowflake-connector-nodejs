...
function isValidOCSPError(err) {
  return (
    err && (err.code === ErrorCodes.ERR_OCSP_REVOKED || err.code === ErrorCodes.ERR_OCSP_UNKNOWN)
  );
}

/**
 * Whether an error represents a definitive, authoritative result about the
 * response itself that must be honored regardless of OCSP mode. Such an error
 * means the response the driver received cannot be trusted for the certificate
 * being checked: its signature did not verify, or it does not correspond to
 * that certificate. Unlike an unavailable or unreachable responder (a soft
 * error), these are not tolerated under fail-open.
 *
 * @param err
 * @returns {boolean}
 */
function isDefinitiveOCSPError(err) {
  return (
    err &&
    (err.code === ErrorCodes.ERR_OCSP_INVALID_SIGNATURE ||
      err.code === ErrorCodes.ERR_OCSP_RESPONSE_CERT_MISMATCH)
  );
}
...
function canEarlyExitForOCSP(errors) {
  if (GlobalConfig.getOcspMode() === GlobalConfig.ocspModes.FAIL_CLOSED) {
    ...
    // A definitive result about a received response (bad signature, or a
    // response that does not correspond to the certificate) is authoritative
    // even in fail-open mode, and takes precedence over any soft error,
    // regardless of the order in which the errors were collected.
    const definitiveError = errors.find(isDefinitiveOCSPError);
    if (definitiveError) {
    ...
    for (let errorIndex = 0, length = errors.length; errorIndex < length; errorIndex++) {
      ...
      if (err && !isValidOCSPError(err)) {
        // any of the errors is a genuinely soft error (e.g. the responder was
        // unavailable or unreachable); assume the certificate is not revoked.
        Logger.getInstance().debug(
          `OCSP responder didn't respond correctly. Assuming certificate is not revoked. Details: ${err}`,
        );
        return null;
  ...
  function getOcspResonseAndVerify(err, useCacheServer) {
    ...
    if (ocspResponse) {
      Logger.getInstance().trace(
        'Returning OCSP status for certificate %s from cache',
        cert.serialNumber,
      );
      // Confirm the cached response corresponds to this certificate.
      const expectedCertId = CertUtil.computeCertIdComponents(cert);
      const status = CertUtil.verifyOCSPResponse(decoded.issuer, ocspResponse, expectedCertId);
      if (!status.err) {
