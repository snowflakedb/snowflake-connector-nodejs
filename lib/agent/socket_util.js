...
function isValidOCSPError(err) {
  return (
    err &&
    (err.code === ErrorCodes.ERR_OCSP_REVOKED ||
...
function canEarlyExitForOCSP(errors) {
  if (GlobalConfig.getOcspMode() === GlobalConfig.ocspModes.FAIL_CLOSED) {
    for (
      let errorIndex = 0, length = errors.length;
      errorIndex < length;
      errorIndex++
    ) {
      // first error
...
function validateCert(cert, cb) {
  ...
  getOcspResonseAndVerify(
    null,
    variables.SF_OCSP_RESPONSE_CACHE_SERVER_ENABLED
  );
}
