 ...
 * @param {Object} mock
 *
 * @returns {socket: Object, socketError: Object | undefined}
 */
exports.secureSocket = function (socket, host, agent, mock)
{
  ...
  if (socket.authorized) { // when socket is authorized we can perform additional OCSP validation now
    socket.cork();
    validate();
    return {
      socket,
    ...
    // when the socket is secure, let's perform additional OCSP validation when it becomes authorized
