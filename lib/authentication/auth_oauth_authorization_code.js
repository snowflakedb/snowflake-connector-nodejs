  ...
  this.authenticate = async function () {
    ...
    if (accessTokenFromCache && connectionConfig.getClientStoreTemporaryCredential()) {
      ...
      token = await coordinateAuth(
        ...
        connectionConfig.username,
        AuthenticationTypes.OAUTH_AUTHORIZATION_CODE + '_refresh',
        async () => this.getAccessTokenUsingRefreshToken(refreshTokenFromCache),
      );
    } else {
      token = await coordinateAuth(
        connectionConfig.host,
        connectionConfig.username,
        AuthenticationTypes.OAUTH_AUTHORIZATION_CODE,
        async () => this.executeFullAuthorizationCodeFlow(),
      );
    }
  };
