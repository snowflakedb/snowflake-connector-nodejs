...
function AuthOauthAuthorizationCode(connectionConfig, httpClient) {
  ...
  this.getAccessTokenUsingRefreshToken = async function (refreshToken) {
    ...
    if (result.access_token) {
      ...
      if (result.refresh_token) {
        ...
        Logger.getInstance().warn(
          'There is no refresh_token value to write to cache. Clearing refresh token in cache',
        );
        await removeFromCache(refreshTokenKey);
      }
    } else {
  ...
  function getAuthorizationUrl(options) {
    ...
    Logger.getInstance().debug(`Url used for receiving authorization code: ${authCodeUrl}`);
  ...
  async function buildRedirectUri(options) {
    ...
    Logger.getInstance().debug(`Authorization code redirect URL: ${redirectUri}`);
    return redirectUri;
  }
  ...
  async function browserAuthorizationCodeProvider(authorizationUrl) {
    Logger.getInstance().debug(
      `Opening your browser to obtain the authorization code: ${authorizationUrl}`,
    );
