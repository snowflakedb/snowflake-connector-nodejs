...
function AuthOauthAuthorizationCode(connectionConfig, httpClient) {
  ...
  const accessTokenKey = authUtil.buildOauthAccessTokenCacheKey(connectionConfig.host,
    connectionConfig.username, AuthenticationTypes.OAUTH_AUTHORIZATION_CODE);
  const refreshTokenKey = authUtil.buildOauthRefreshTokenCacheKey(connectionConfig.host,
    connectionConfig.username, AuthenticationTypes.OAUTH_AUTHORIZATION_CODE);
  ...
  this.executeFullAuthorizationCodeFlow = async function (){
    ...
    const codeChallengeMethod = connectionConfig.getChallengeMethod() || 'S256'; // TODO: should be verified with "discovery" response
