...
const {
  withBrowserActionTimeout,
  writeToCache,
  removeFromCache,
  readCache,
} = require('./authentication_util');
...
// Tokens are persisted on disk via writeToCache()/readCache(). Encrypt them at
// rest so filesystem access to the cache file alone doesn't yield a directly
// replayable OAuth access/refresh token.
const CACHE_ENCRYPTION_KEY = crypto.scryptSync(
  `${os.hostname()}:${os.homedir()}`,
  'snowflake-oauth-token-cache',
  32,
);

const GCM_AUTH_TAG_LENGTH = 16;

function encryptCachedToken(value) {
...
function decryptCachedToken(value) {
  if (!value) {
    return value;
  }
  try {
    ...
    Logger().warn('Failed to decrypt cached OAuth token; treating it as absent.');
    return null;
  }
}
...
function AuthOauthAuthorizationCode(connectionConfig, httpClient) {
  ...
  this.authenticate = async function () {
    ...
    token = await coordinateAuth(
      connectionConfig.host,
      connectionConfig.username,
      AuthenticationTypes.OAUTH_AUTHORIZATION_CODE,
      async () => {
        const accessTokenFromCache = decryptCachedToken(await readCache(accessTokenKey));
        const refreshTokenFromCache = decryptCachedToken(await readCache(refreshTokenKey));
  ...
  this.reauthenticate = async function (body) {
    const refreshToken = decryptCachedToken(await readCache(refreshTokenKey));
    ...
    if (refreshToken) {
      try {
        ...
        Logger().warn(
          format(
            'Error while getting access token using refresh token. Message: %s. The refresh token is removed form cache - authentication must be proceed from the beginning',
            error.message,
          ),
        );
  ...
  this.executeFullAuthorizationCodeFlow = async function () {
    ...
    const authorizationUrlWithParams = await prepareAuthorizationUrl(
      authorizationUrl,
      client,
      redirectUri,
      codeChallenge,
    ...
    const authorizationCodeResponse = await requestAuthorizationCode(
    ...
    const params = oauth.validateAuthResponse(
      ...
      client,
      authorizationUrlWithParams,
      authorizationCodeResponse.state,
    );
  ...
  this.getAccessTokenUsingRefreshToken = async function (refreshToken) {
    ...
    const response = await oauth.refreshTokenGrantRequest(as, client, clientAuth, refreshToken, {
      ...
      [oauth.customFetch]: async (url, options) =>
        await convertToResponseType(httpClient, url, options),
    });
    ...
    if (result.access_token) {
      ...
      if (accessTokenKey) {
        await writeToCache(accessTokenKey, encryptCachedToken(result.access_token));
      }
      ...
      if (result.refresh_token) {
        ...
        if (refreshTokenKey) {
          await writeToCache(refreshTokenKey, encryptCachedToken(result.refresh_token));
        }
      }
    } else {
      throw Error(
        `Response doesn't contain OAuth access token. Requested URI: Host: ${tokenUrl.host} Path: ${tokenUrl.pathname}`,
      );
    }
  ...
  async function prepareAuthorizationUrl(
    authorizationUrl,
    client,
    redirectUri,
    codeChallenge,
  ...
  async function requestAuthorizationCode(authorizationUrl, browserActionTimeout) {
    ...
    try {
      ...
      Logger().debug(
        `Opening your browser to obtain the authorization code: ${authorizationUrl.href}`,
      );
      ...
      const codeResponse = await withBrowserActionTimeout(browserActionTimeout, receiveData).catch(
        (rejected) => {
          throw new Error(rejected);
        },
      );

      const autorizationCodeResponseParameters = querystring.parse(
        codeResponse.substring(codeResponse.indexOf('?') + 1),
      );
      const code = autorizationCodeResponseParameters['code'];
      const state = autorizationCodeResponseParameters['state'].replace(
        new RegExp('\\sHTTP/.*'),
        '',
      );
