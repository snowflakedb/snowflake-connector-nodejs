...
function AuthOauthAuthorizationCode(connectionConfig, httpClient) {
  ...
  const credentialManager = GlobalConfig.getCredentialManager();

  let oauth;
  let token;

  const accessTokenKey = Util.buildCredentialCacheKey(connectionConfig.host,
    connectionConfig.username, AuthenticationTypes.OAUTH_AUTHORIZATION_CODE_FLOW + 'access_token');
  const refreshTokenKey = Util.buildCredentialCacheKey(connectionConfig.host,
    connectionConfig.username, AuthenticationTypes.OAUTH_AUTHORIZATION_CODE_FLOW + 'refresh_token');
  ...
  this.authenticate = async function () {
    ...
    const accessTokenFromCache = await credentialManager.read(accessTokenKey);
    //verify that there is refresh token in the cache
    const refreshTokenFromCache = await credentialManager.read(refreshTokenKey);
  ...
  this.reauthenticate = async function (body) {
    ...
    await credentialManager.remove(accessTokenKey);

    const refreshToken = await credentialManager.read(refreshTokenKey);
    ...
    if (refreshToken) {
      try {
        ...
        await credentialManager.remove(refreshTokenKey);
  ...
  this.getAccessTokenUsingRefreshToken = async function (refreshToken){
    ...
    if (result.access_token) {
      await credentialManager.write(accessTokenKey, result.access_token);
    } else {
      throw new Error('Error during refreshing access token');
    }

    if (result.refresh_token) {
      await credentialManager.write(refreshTokenKey, result.refresh_token);
    } else {
