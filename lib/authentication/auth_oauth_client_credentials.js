...
const { exists, format, dynamicImportESMInTypescriptWithCommonJS } = require('../util');
...
function AuthOauthClientCredentials(connectionConfig, httpClient) {
  ...
  this.loadOauth4webapi =  async function () {
    if (!oauth) {
      oauth = await dynamicImportESMInTypescriptWithCommonJS('oauth4webapi');
    }
  };
  ...
  this.authenticate = async function () {
    ...
    if (exists(accessTokenFromCache) && connectionConfig.getClientStoreTemporaryCredential()) {
      ...
      if (connectionConfig.oauthEnableSingleUseRefreshTokens) {
