...
function AuthOkta(connectionConfig, httpClient) {
  ...
  */
  async function step4(oneTimeToken, ssoUrl) {
    ...
    const timeout = (connectionConfig.getRetryTimeout() - connectionConfig.getElapsedTime()) * 1000;
    const startTime = Date.now();
    const totalTimeout = startTime + timeout;
    let numofRetries = connectionConfig.getNumofRetries(); 
    let response;
    while (timeout === 0 || Date.now() < totalTimeout) {
      try { 
        ...
        if ( totalTimeout < Date.now()) {
          Logger.getInstance().warn(`step4: Fail to get SAML response, timeout reached: ${timeout} miliseconds`);
          throw err;
        } else {
