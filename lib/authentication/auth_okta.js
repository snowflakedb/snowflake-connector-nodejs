...
function AuthOkta(connectionConfig, httpClient) {
  ...
  * @returns {Object}
  */
  async function step4(oneTimeToken, ssoUrl, retryOption = { totalElapsedTime: null, numofRetries: 0 }) {
    // Query IDP URL to get SAML response
    const maxRetryTimeout = connectionConfig.getRetryTimeout();
    let remainingTimeout;
    let response;

    if (!retryOption.remainingTimeout){
      remainingTimeout = connectionConfig.getTimeout();
    } else if (connectionConfig.getRetryTimeout() === 0){
      remainingTimeout = 0;
    } else {
      remainingTimeout = maxRetryTimeout - retryOption.totalElapsedTime;
    }
