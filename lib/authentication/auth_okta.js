...
function AuthOkta(connectionConfig, httpClient) {
  ...
  */
  this.authenticate = async function (authenticator, serviceName, account, username) {
    ...
    const ssoUrl = responseData['data']['ssoUrl'];
    const tokenUrl = responseData['data']['tokenUrl'];
  ...
  this.reauthenticate = async function (body) {
    await this.authenticate(connectionConfig.getAuthenticator(),
  ...
  */
  async function step4(oneTimeToken, ssoUrl) {
    ...
    const timeout = connectionConfig.getRetryTimeout() * 1000;
    const currentTime = Date.now();
    let response;
    while (timeout === 0 || Date.now() < currentTime + timeout) {
      try { 
