...
function AuthOkta(connectionConfig, httpClient) {
  ...
  */
  async function step4(oneTimeToken, ssoUrl) {
    ...
    const timeout = connectionConfig.getRetryTimeout();
    const currentTime = Date.now();
    let response;
    while (timeout === 0 || Date.now() < currentTime + (timeout * 1000)) {
      try { 
        ...
        if (Date.now() < currentTime + timeout) {
          Logger.getInstance().warn(`step4: Fail to get SAML response, timeout reached: ${timeout}`);
        } else {
