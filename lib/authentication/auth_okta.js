...
function AuthOkta(connectionConfig, httpClient) {
  ...
  this.authenticate = async function (authenticator, serviceName, account, username) {
    ...
    if (
      ...
      throw new Error(
        'Unable to use provided Okta address as an authenticator. Is the authenticator URL correct?',
      );
    }

    if (success !== true) {
      throw new Error(
        `Unable to use provided Okta address as an authenticator. Error code: ${errorCode}, error message: ${errorMessage}`,
      );
    }
    ...
    const responseHtml = await getSAMLResponse(
      await createAccessToken(tokenUrl, username, password),
      ssoUrl,
    );
  ...
  this.reauthenticate = async function (body, retryOption) {
    ...
    while (util.shouldRetryOktaAuth(authRetryOption)) {
      try {
        responseHtml = await getSAMLResponse(
          await createAccessToken(tokenUrl, user, password),
          ssoUrl,
        );
        break;
      } catch (err) {
    ...
    if (remainingTimeout !== 0 && startTime + remainingTimeout < Date.now()) {
      Logger.getInstance().warn(
        `getSAMLResponse: Fail to get SAML response, timeout reached: ${remainingTimeout} miliseconds`,
      );
    ...
    if (maxRetryCount < authRetryOption.numRetries) {
      Logger.getInstance().warn(
        `getSAMLResponse: Fail to get SAML response, max retry reached: ${maxRetryCount} time`,
      );
  ...
  this.validateURLs = function (authenticator, ssoUrl, tokenUrl) {
    const compareUrlsByProtocolAndHost = (firstUrl, secondUrl) =>
      firstUrl.protocol === secondUrl.protocol && firstUrl.host === secondUrl.host;
  ...
  function validateSAML(responseHtml) {
    ...
    if (!connectionConfig.getDisableSamlURLCheck()) {
      if (postBackUrl.substring(0, 20) !== fullUrl.substring(0, 20)) {
        throw new Error(
          util.format(
            'The specified authenticator and destination URL ' +
              'in the SAML assertion do not match: expected: %s postback: %s',
            fullUrl,
            postBackUrl,
          ),
        );
