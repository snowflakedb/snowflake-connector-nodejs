...
function AuthOkta(connectionConfig, httpClient) {
  ...
  */
  ...
  this.reauthenticate = async function (body, retryOption) {
    ...
    while ((maxRetryTimeout === 0 || Date.now() < startTime + remainingTimeout) && retryOption.numRetries <= maxRetryCount) {
      try { 
    ...
    if (maxRetryCount < retryOption.numRetries){
      Logger.getInstance().warn(`step4: Fail to get SAML response, max retry reached: ${maxRetryCount} time`);
