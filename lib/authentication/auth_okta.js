...
function AuthOkta(connectionConfig, httpClient) {
  ...
  */
  ...
  this.reauthenticate = async function (body, retryOption) {
    ...
    const maxRetry = connectionConfig.getRetrySfMaxLoginRetries();
    const remainingTimeout = maxRetryTimeout !== 0 ? (maxRetryTimeout - retryOption.totalElapsedTime) * 1000 : 0;
    const startTime = Date.now();
    let responseHtml;

    while (remainingTimeout === 0 || Date.now() < startTime + remainingTimeout) {
      try { 
        responseHtml = await step4( await step3(tokenUrl, user, password), ssoUrl);
        break;
      } catch (err) {   
        Logger.getInstance().debug('step4: refresh token for re-authentication');
        retryOption.numRetries++;
      }
    }
    ...
    if (maxRetry < retryOption.numRetries){
      Logger.getInstance().warn(`step4: Fail to get SAML response, max retry reached: ${maxRetry} time`);
