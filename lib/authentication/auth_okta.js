...
function AuthOkta(connectionConfig, httpClient) {
  ...
  */
  async function step4(oneTimeToken, ssoUrl, retryOption = { remainingTimeout: null, numofRetries: 0 }) {
    ...
    const remainingTimeout = (retryOption.remainingTimeout || connectionConfig.getTimeout()) * 1000;
    let response;
    ...
    retryOption.remainingTimeout = (remainingTimeout - (Date.now() - startTime)) / 1000;
