...
function AuthOkta(connectionConfig, httpClient) {
  ...
  */
  ...
  this.reauthenticate = async function (body) {
    const responseHtml = await step4( await step3(tokenUrl, user, password), ssoUrl);
    step5(responseHtml);
  ...
  */
  async function step4(oneTimeToken, ssoUrl) {
    ...
    const remainingTimeout = (maxRetryTimeout - connectionConfig.getElapsedTime()) * 1000;
    let numofRetries = connectionConfig.getNumofRetries(); 
    let response;

    const startTime = Date.now();
    while (maxRetryTimeout === 0 || Date.now() < startTime + remainingTimeout) {
      try { 
    ...
    if (maxRetryTimeout !== 0 && startTime + remainingTimeout < Date.now()) {
      Logger.getInstance().warn(`step4: Fail to get SAML response, timeout reached: ${maxRetryTimeout} miliseconds`);
      throw new Error('Reached out to the Login Timeout');
    }

    connectionConfig.setElapsedTime(connectionConfig.getElapsedTime() + (new Date() - startTime) / 1000);
    connectionConfig.setNumofRetries(numofRetries);
