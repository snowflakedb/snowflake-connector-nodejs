...
function AuthWeb(connectionConfig, httpClient, webbrowser) {
  ...
  this.authenticate = async function (authenticator, serviceName, account, username) {
    ...
    // When custom parameters are not provided, 
    // localServerPort and localServerHost will both be 0,
    // which will use a random port and fallback to localhost
    server.listen(Util.parseAddress(samlRedirectUri));
