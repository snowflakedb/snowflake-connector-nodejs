...
function throwErrorIfExperimentalAuthnNotEnabled(connectionConfig) {
  if (
    !connectionConfig.getEnableExperimentalAuthentication() &&
    [
    ...
    ].includes(connectionConfig.getAuthenticator())
  ) {
    throw new Error(
      `Following authentication method not yet supported: ${connectionConfig.getAuthenticator()}`,
    );
  }
}
...
exports.getAuthenticator = function getAuthenticator(connectionConfig, httpClient) {
  ...
  let auth;
  if (
    authType === AuthenticationTypes.DEFAULT_AUTHENTICATOR ||
    ...
    Logger.getInstance().warn(
      `No authenticator found for '${authType}'. Using default authenticator as a fallback`,
    );
    auth = new AuthDefault(connectionConfig);
  }
  return auth;
