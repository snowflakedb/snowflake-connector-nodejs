...
function throwErrorIfExperimentalAuthnNotEnabled(connectionConfig) {
  if (!connectionConfig.getEnableExperimentalAuthentication() &&
    [AuthenticationTypes.PROGRAMMATIC_ACCESS_TOKEN, AuthenticationTypes.OAUTH_AUTHORIZATION_CODE_FLOW, AuthenticationTypes.OAUTH_CLIENT_CREDENTIALS].includes(connectionConfig.getAuthenticator())
  ) {
    throw new Error(`Following authentication method not yet supported: ${connectionConfig.getAuthenticator()}`);
  }
}
...
exports.getAuthenticator = function getAuthenticator(connectionConfig, httpClient) {
  throwErrorIfExperimentalAuthnNotEnabled(connectionConfig);
  ...
  } else if (authType === AuthenticationTypes.OAUTH_AUTHORIZATION_CODE_FLOW ) {
