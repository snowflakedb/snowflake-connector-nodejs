 ...
 * @param {{ renderer?: (result: { error?: string }) => string }} [options]
 *
 * @returns {Server}
 */
  ...
  const server = net.createServer(function (socket) {
    socket.on('data', function (chunk) {
      try {
        ...
        const error = data[0].includes('?error=') ? prepareError(data[0]) : undefined;
        const body = renderer ? renderer({ error }) : escapeHTML(error ? error : successResponse);
