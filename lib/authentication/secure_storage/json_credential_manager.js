...
function JsonCredentialManager(credentialCacheDir, timeoutMs = 60000) {
  ...
  this.tryTokenDir = async function (dir, subDirs) {
    const credPath = path.join(dir, ...subDirs);
    try {
      ...
      if (!cacheStat.isDirectory()) {
        ...
        await fs.mkdir(cacheDir, options).then(() => {
          dirCreated = true;
        });
        return dirCreated;
      } else {
        ...
        let setMode = false;
        await fs.chmod(cacheDir, 0o700).then(() => {
          setMode = true;
        });
        return setMode;
      }
    } catch {
      Logger.getInstance().warn(`The path location ${credPath} is invalid. Please check this location is accessible or existing`);
      return false;
    }
  };
