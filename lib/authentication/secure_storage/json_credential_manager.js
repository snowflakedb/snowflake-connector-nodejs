...
function JsonCredentialManager(credentialCacheDir) {
  ...
  async function validatePermission(filePath) {
    ...
    const isUserPermission =  mode & fs.constants.S_IRUSR | fs.constants.S_IWUSR;
    const isGroupPermission = mode & fs.constants.S_IRGRP | fs.constants.S_IWGRP;
    const isOtherPermission = mode & fs.constants.S_IROTH | fs.constants.S_IWOTH;

    if (isUserPermission || isGroupPermission || isOtherPermission) {
      Logger.getInstance().info('Validated that the user has read and write permission');
    } else {
      throw new Error(`You do not have read permission to the credential file: ${filePath}.`);
    }
  }
  ...
  this.write = async function (key, token) {
    ...
    try {
      await fs.writeFile(await this.getTokenDir(), JSON.stringify(jsonCredential), { mode: fs.constants.S_IRUSR | fs.constants.S_IWUSR });
    } catch (err) {
      throw new Error(`Failed to write token data from the file. Please check the permission or the file format of the token. ${err.message}`);
    }
  };
