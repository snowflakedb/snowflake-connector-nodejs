...
function JsonCredentialManager(credentialCacheDir, timeoutMs = 60000) {
  ...
  this.getTokenDirCandidates = function () {
    ...
    candidates.push({
      folder: process.env.SF_TEMPORARY_CREDENTIAL_CACHE_DIR,
      subfolders: [],
    });
    ...
    switch (process.platform) {
      case 'win32':
        candidates.push({
          folder: os.homedir(),
          subfolders: defaultJsonTokenCachePaths['win32'],
        });
        break;
      case 'linux':
        candidates.push({
          folder: process.env.XDG_CACHE_HOME,
          subfolders: ['snowflake'],
        });
        candidates.push({
  ...
  this.tryTokenDir = async function (dir, subDirs) {
    ...
    try {
      ...
      Logger.getInstance().warn(
        `The location ${cacheDir} is invalid. Please check this location is accessible or existing`,
      );
      return false;
    }
  };
  ...
  this.lockFile = async function (filename) {
    ...
    while (attempts <= 10) {
      Logger.getInstance().debug(
        'Attempting to get a lock on file %s, attempt: %d',
        filename,
        attempts,
      );
      attempts++;
  ...
  this.write = async function (key, token) {
    ...
    await this.withFileLocked(async (filename) => {
      ...
      try {
        ...
        Logger.getInstance().warn(
          `Failed to write token data in ${filename}. Please check the permission or the file format of the token. ${err.message}`,
        );
      }
    });
  };
    ...
    await this.withFileLocked(async (filename) => {
      ...
      if (jsonCredential && jsonCredential[tokenMapKey] && jsonCredential[tokenMapKey][keyHash]) {
        try {
          ...
          Logger.getInstance().warn(
            `Failed to remove token data from the file in ${filename}. Please check the permission or the file format of the token. ${err.message}`,
          );
        }
      }
    });
