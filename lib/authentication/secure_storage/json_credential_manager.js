...
function JsonCredentialManager() {
  ...
  this.getTokenDir = function () {
    ...
    if (!Util.exists(tokenDir)) {
      ...
      Logger.getInstance().debug(`The credential cache directory is configured by the user. The token will be saved at ${tokenDir}`);
    }
    ...
    if (fs.existsSync(tokenCacheFile)) {
      try {
        fs.accessSync(tokenCacheFile,  fs.constants.R_OK | fs.constants.W_OK);
      } catch (err) {
        Logger.getInstance().info(`You do not have permission to the credential file: ${tokenCacheFile}.`);
        return null;
      }
    }
