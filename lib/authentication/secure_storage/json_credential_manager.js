...
function JsonCredentialManager(credentialCacheDir, timeoutMs = 60000) {
  ...
  this.getTokenDirCandidates = function () {
    ...
    case 'win32':
      candidates.push({ folder: os.homedir(), subfolders: ['AppData', 'Local', 'Snow'] });
      break;
    case 'linux':
      if (home !== undefined) {
  ...
  this.tryTokenDir = async function (dir, subDirs) {
    ...
    try {
      ...
      if (!exists) {
        ...
        await fs.promises.mkdir(cacheDir, options);
        return true;
      } else {
        const cacheStat = fs.statSync(cacheDir);
        if (process.platform === 'win32') {
          return true;
        }
  ...
  this.readJsonCredentialFile = async function (file) {
    try {
      const cred = await fs.promises.readFile(file, 'utf8');
  ...
  this.withFileLocked = async function (fun) {
    ...
    while (attempts <= 10) {
      ...
      attempts++;
      await fs.promises.mkdir(lckFile, options).then(() => {
        locked = true;
      }, () => {});
      if (locked) {
        break;
    ...
    await fs.promises.rmdir(lckFile);
    return res;
  };
  ...
  this.write = async function (key, token) {
    ...
    await this.withFileLocked(async (file) => {
      ...
      try {
        await fs.promises.writeFile(file, JSON.stringify(jsonCredential), { mode: 0o600 });
      } catch (err) {
