...
const fs = require('fs');
const { isString, exists, isFileWritableOnlyByUser } = require('../util');
...
function ConfigurationUtil(fsPromisesModule, processModule) {
  ...
  this.getClientConfig = async function (configFilePath) {
    ...
    const isFileOk = await isFileWritableOnlyByUser(path, fsPromises).catch(err => {
      throw new ConfigurationError(`Configuration file: ${path} cannot be accessed`, err);
    });
  ...
  async function findConfig(filePathFromConnectionString) {
    ...
    const filePathFromDefDirs = await searchForConfigInDefaultDirectories();
    if (exists(filePathFromDefDirs)) {
      Logger.getInstance().info('Using client configuration path from a default directory: %s', filePathFromDefDirs);
      return filePathFromDefDirs;
    }
