...
function levelFromString(value) {
  ...
  if (!allLevels.includes(level)) {
    ...
    Logger.getInstance().debug(`Tried to create unsupported log level from string: ${value}`);
...
function ConfigurationUtil(fsPromisesModule, processModule) {
  ...
  this.getClientConfig = async function (configFilePath) {
    ...
    if (!exists(path) || path === '') {
      Logger.getInstance().info('No config path found. Client config will not be used.');
      return null;
    }

    const isFileOk = await isFileNotWritableByGroupOrOthers(path, fsPromises).catch(err => {
      Logger.getInstance().warn('Failed to inspect config path permissions. Client config will not be used.');
      throw new ConfigurationError('Finding client configuration failed', err);
    });

    if (!isFileOk) {
      Logger.getInstance().warn(`Config path permissions are invalid. File: ${path} can be modified by group or others. Client config will not be used.`);
      throw new ConfigurationError(`Configuration file: ${path} can be modified by group or others`, 'IncorrectPerms');
    }
    Logger.getInstance().trace(`Config path permissions are valid. Path: ${path}.`);
  ...
  function readFileConfig(filePath) {
    Logger.getInstance().debug(`Reading config file: ${filePath}`);

    if (!filePath) {
      Logger.getInstance().trace(`Path is not specified. Nothing to read. Path: ${filePath}`);
  ...
  function parseConfigFile(path, configurationJson) {
    ...
    try {
      ...
      Logger.getInstance().trace('File contains correct JSON structure. Validating the input.');

      checkUnknownEntries(parsedConfiguration);
      validate(parsedConfiguration);

      Logger.getInstance().debug('File contains valid configuration input.');
      ...
      Logger.getInstance().error('File contains correct JSON. Validating the input.');
