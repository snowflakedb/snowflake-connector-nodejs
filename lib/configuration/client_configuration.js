...
function ConfigurationUtil(fsPromisesModule, processModule) {
  ...
  this.getClientConfig = async function (configFilePath, mock = false, delay = 0) {
    ...
    if (!exists(path) || path === '') {
      Logger.getInstance().info('Client configuration load failed — path not found.');
      return null;
    }

    const isFileExist = mock ? mock : IsFileExisted(path);
    if (!isFileExist) {
      Logger.getInstance().info(`Unable to load client configuration — the file does not exist at the specified path: '${path}`);
      return null;
    }
    ...
    try {
      ...
      if (!isWindows()){
        ...
        //Compare the file descriptor information from the 'open' call with the file descriptor information after reading to validate whether the file has been modified.
