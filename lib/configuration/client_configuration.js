...
const { isFileNotWritableByGroupOrOthers, IsFileExisted, lockFile, unlockFile } = require('../file_util');
...
function ConfigurationUtil(fsPromisesModule, processModule) {
  ...
  this.getClientConfig = async function (configFilePath, mock = false) {
    ...
    try {
      ...
      configFileContents = await readFileConfig(path);
  ...
  async function findConfig(filePathFromConnectionString) {
    ...
    // if (exists(filePathFromConnectionString) && await IsFileExisted(filePathFromConnectionString)) {
    ...
    // if (exists(filePathFromEnvVariable) && await IsFileExisted(filePathFromConnectionString)) {
