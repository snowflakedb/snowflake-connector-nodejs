...
const { IsFileExisted, lockFile, unlockFile } = require('../file_util');
...
function ConfigurationUtil(fsPromisesModule, processModule) {
   ...
   */
  this.getClientConfig = async function (configFilePath, mock = false) {
    ...
    try {
      ...
      if (!(isWindows() || mock)) {
        if (!(stats.uid === 0 )) {
          Logger.getInstance().warn('This file is not owned by root(Admin). Cannot read this file for the security issue');
          return null;
        }

        if (!isFilePermissionValid(stats.mode)) {
      ...
      if (await fsPromises.stat(path).mtimeMs !== stats.mTime) {
        Logger.getInstance().error(`The file was modified after the driver opened the config file and can no longer be used.`);
        throw new ConfigurationError('The config file has been modified', "InvalidaConfigFile");
      }
