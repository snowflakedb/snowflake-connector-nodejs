...
function readTokenFromFile(fixedConfiguration, disablePermissionCheckForTomlConfig = false) {
  ...
  if (!disablePermissionCheckForTomlConfig) {
...
function loadConnectionConfiguration(disablePermissionCheckForTomlConfig = false) {
  ...
  validateOnlyUserReadWritePermissionAndOwner(resolvedPath);
    ...
    Logger.default().warn('Permission check is currently disabled. Make sure this is intentional, as it may expose files with improper permissions.');
  }
