...
function Connection(context)
{
  ...
  */
  ...
  this.connectAsync = async function (callback)
  {
    ...
    //   connectionConfig.idToken = await SecureStorage.readCredential(connectionConfig.host, connectionConfig.username, 'ID_TOKEN');
  
    //   if(connectionConfig.getClientRequestMFAToken()) {
    //     connectionConfig.mfaToken = await SecureStorage.readCredential(connectionConfig.host, connectionConfig.username, 'MFA_TOKEN');
    //   }
    // }
    connectionConfig.idToken = null;
    ...
    try
    {
      ...
      await auth.authenticate(connectionConfig.getAuthenticator(),
        ...
        //   if (connectionConfig.getClientStoreTemporaryCredential()) {
        //   if(connectionConfig.getAuthenticator() === Authenticator.authenticationTypes.EXTERNAL_BROWSER_AUTHENTICATOR) {
        //    await SecureStorage.writeCredential(connectionConfig.host, connectionConfig.username, 'ID_TOKEN', body['data']['TOKEN'])
        //   }
          
        //   if (connectionConfig.getClientRequestMFAToken()) {
        //     await SecureStorage.writeCredential(connectionConfig.host, connectionConfig.username, 'MFA_TOKEN', body['data']['TOKEN']);
        //   }
        // } 
          // Request connection
          services.sf.connect({
