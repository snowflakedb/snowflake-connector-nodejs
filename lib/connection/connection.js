...
function Connection(context) {
  ...
  */
  ...
  function extractQueryStatus(queryResponse) {
    ...
    status = queries.length > 0 ? queries[0]['status'] : QueryStatus.code.NO_QUERY_DATA;
  ...
  this.getResultsFromQueryId = async function (options) {
    ...
    let status, err, noDataCounter = 0, retryPatternPos = 0;
    ...
    while (queryStillExecuting) {
      ...
      status = await this.getQueryStatusThrowIfError(queryId, options.complete);
      queryStillExecuting = this.isStillRunning(status);

      if (!queryStillExecuting || QueryStatus.code.NO_QUERY_DATA) {
        break;
      }
