  ...
  */
  ...
  this.getResultsFromQueryId = async function (options) {
    ...
    while (queryStillExecuting) {
      Logger.getInstance().trace('Connection[id: %s] - checking if query with Query[id: %s] is still executing. Retries with no data count: %d', this.getId(), queryId, noDataCounter);
      ...
      if (!queryStillExecuting || status === QueryStatus.code.NO_QUERY_DATA) {
        Logger.getInstance().trace('Connection[id: %s] - end of waiting for query with Query[id: %s] to finish executing. ' +
    ...
    Logger.getInstance().debug('Connection[id: %s] - query with Query[id: %s] succeeded. Fetching the result.', this.getId(), queryId);
