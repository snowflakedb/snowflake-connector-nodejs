      ...
      await auth.authenticate(connectionConfig.getAuthenticator(),
        ...
        .then(() =>
        {
          ...
          if (connectionConfig.getAuthenticator() === Authenticator.awwwuthenticationTypes.EXTERNAL_BROWSER_AUTHENTICATOR &&
            ...
            SecureStorage.writeCredential(connectionConfig.host, connectionConfig.username, 'ID_TOKEN', body['data']['TOKEN']).then(()=>{
              Logger.getInstance().info("id token is saved into the secure storage");
            });

            if (connectionConfig.getClientRequestMFAToken()) {
              SecureStorage.writeCredential(connectionConfig.host, connectionConfig.username, 'MFA_TOKEN', body['data']['TOKEN']).then(()=>{
                Logger.getInstance().info("mfa token is saved into the secure storage");
              });
            }
          } 
