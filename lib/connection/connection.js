  ...
  */
  ...
  this.getResultsFromQueryId = async function (options) {
    ...
    while (queryStillExecuting) {
      Logger.getInstance().trace('Connection[id: %s] - checking if query with queryId: %s is still executing.', this.getId(), queryId);
      ...
      if (QueryStatus.code[status] === QueryStatus.code.NO_DATA) {
        noDataCounter++;
        Logger.getInstance().trace('Connection[id: %s] - no data returned for queryId: %s. Retry count: %d', this.getId(), queryId, noDataCounter);
