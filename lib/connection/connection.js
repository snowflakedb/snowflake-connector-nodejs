  ...
  */
  ...
  this.connectAsync = async function (callback)
  {
    ...
    if (connectionConfig.getClientStoreTemporaryCredential()) {
      connectionConfig.idToken = await SecureStorage.readCredential(connectionConfig.account, connectionConfig.username, 'ID_TOKEN');
  
      if(connectionConfig.getClientRequestMFAToken()) {
        connectionConfig.mfaToken = await SecureStorage.readCredential(connectionConfig.account, connectionConfig.username, 'MFA_TOKEN');
      }
    }
    // connectionConfig.idToken = "ver:1-hint:92019682442222-";

    console.log(connectionConfig.idToken);
    // Get authenticator to use
    const auth = Authenticator.getAuthenticator(connectionConfig, context.getHttpClient());
