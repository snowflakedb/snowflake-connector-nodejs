...
function Connection(context) {
  ...
  Logger.getInstance().info(
    'Creating connection with host: {}, account: {}, accessUrl: {}, user: {}, ' +
    'password is {}, role: {}, database: {}, schema: {}, warehouse: {}, region: {}' +
      'authenticator: {}, ocsp mode: {}, os: {}, os version: {}, ' +
      'passcode in password is {}, passcode is {}, private key is {}, ' +
      'application: {}, client name: {}, client version: {}, retry timeout: {}, ' +
      'private key path: {}, private key pass is {}, ' +
      'client store temporary credential: {}, browser response timeout: {}',
    connectionConfig.host, connectionConfig.account, connectionConfig.accessUrl, connectionConfig.username,
    LoggingUtil.describePresence(connectionConfig.password), connectionConfig.getRole(), connectionConfig.getDatabase(),
    connectionConfig.getSchema(), connectionConfig.getWarehouse(), connectionConfig.region, connectionConfig.getAuthenticator(),
    connectionConfig.getClientEnvironment().OCSP_MODE, connectionConfig.getClientEnvironment().OS,
    connectionConfig.getClientEnvironment().OS_VERSION, LoggingUtil.describePresence(connectionConfig.getPasscodeInPassword()),
    LoggingUtil.describePresence(connectionConfig.getPasscode()), LoggingUtil.describePresence(connectionConfig.getPrivateKey()),
    connectionConfig.getClientApplication(), connectionConfig.getClientName(), connectionConfig.getClientVersion(),
    connectionConfig.getRetryTimeout(), connectionConfig.getPrivateKeyPath(), LoggingUtil.describePresence(connectionConfig.getPrivateKeyPass()),
    connectionConfig.getClientStoreTemporaryCredential(), connectionConfig.getBrowserActionTimeout()
  );
  ...
  */
  this.isTokenValid = function () {
    ...
    Logger.getInstance().trace('Connection[id: %s] - isTokenValid called. Returning: %s', this.getId(), areTokensValid);
  ...
  */
      ...
      initEasyLogging(connectionConfig.clientConfigFile)
        .then(() => {
          ...
          try {
            ...
            // TODO: actually when run without havig the cloud vm working - it is anyway printed even tho this is not true - connection was not established
            const connectingDuration = timer.getDuration();
            Logger.getInstance().info(
  ...
  async function getQueryResponse(queryId) {
    Logger.getInstance().trace('Connection[id: %s] - requested query response for queryId: %s', id, queryId);
    ...
    Logger.getInstance().debug('Connection[id: %s] - queryId: %s is valid', id, queryId);
    ...
    Logger.getInstance().debug('Connection[id: %s] - fetching query response for queryId: %s', id, queryId);
    const timer = new ExecutionTimer().start();
    ...
    Logger.getInstance().debug('Connection[id: %s] - query response for queryId: %s fetched successfully after: %s milliseconds', id, queryId, fetchingDuration);
  ...
  this.getQueryStatus = async function (queryId) {
    Logger.getInstance().trace('Connection[id: %s] - getQueryStatus called for queryId: %s', this.getId(), queryId);
  ...
  this.getQueryStatusThrowIfError = async function (queryId) {
    Logger.getInstance().trace('Connection[id: %s] - getQueryStatusThrowIfError called for queryId: %s', this.getId(), queryId);
    ...
    if (this.isAnError(status) ) {
      ...
      Logger.getInstance().debug('Connection[id: %s] - query error for queryId: %s. Error: %s. SQLState: %s', this.getId(), queryId, message, sqlState);
  ...
  this.getResultsFromQueryId = async function (options) {
    ...
    Logger.getInstance().trace('Connection[id: %s] - getResultsFromQueryId called for queryId: %s.', this.getId(), queryId);
    ...
    while (queryStillExecuting) {
      Logger.getInstance().trace('Connection[id: %s] - checking if query with queryId: %s is still executing. Retries with no data count: %d', this.getId(), queryId, noDataCounter);
      ...
      if (!queryStillExecuting || status === QueryStatus.code.NO_QUERY_DATA) {
        Logger.getInstance().trace('Connection[id: %s] - end of waiting for query with queryId: %s to finish executing. ' +
      ...
      if (QueryStatus.code[status] === QueryStatus.code.NO_DATA) {
        noDataCounter++;
        Logger.getInstance().trace('Connection[id: %s] - no data returned for queryId: %s. Retries with no data count: %d', this.getId(), queryId, noDataCounter);
        // Check if retry for no data is exceeded
        if (noDataCounter > asyncNoDataMaxRetry) {
          Logger.getInstance().error('Connection[id: %s] - no data returned for queryId: %s. Retry limit: %s reached.', this.getId(), queryId, asyncNoDataMaxRetry);
          throw Errors.createClientError(
    ...
    if (QueryStatus.code[status] === QueryStatus.code.NO_QUERY_DATA) {
      Logger.getInstance().error('Connection[id: %s] - queryId: %s did not succeed. Final status: %s', this.getId(), queryId, status);
      throw Errors.createClientError(
    ...
    Logger.getInstance().debug('Connection[id: %s] - query with queryId: %s succeeded. Fetching the result.', this.getId(), queryId);
