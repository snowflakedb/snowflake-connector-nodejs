...
function Connection(context) {
  ...
  */
  ...
  async function getQueryResponse(queryId) {
    // When lack of server response, a non-intuitive error is thrown
    Logger.getInstance().trace(`Getting query response for queryId ${queryId}`);
  ...
  this.getQueryStatusThrowIfError = async function (queryId) {
    ...
    if (this.isAnError(status)) {
      ...
      sqlState = response['state'] || '';
      let responseData = response['data'];

      if (responseData) {
        if (responseData['queries'].length > 0) {
          const firstQuery = responseData['queries'][0];
          message += firstQuery['errorMessage'];
          code = firstQuery['errorCode'];
          sqlState = firstQuery['state'];
          responseData = {};
          [
            'id', 'requestId', 'status', 'state', 'sqlText', 'sessionId', 'clientSendTime', 'startTime',
            'endTime', 'isDescribe', 'errorCode', 'errorMessage', 'warehouseName', 'queryTag'
          ].forEach( (key) => {
            responseData[key] = firstQuery[key];
          });
        } else {
          responseData = {};
        }
