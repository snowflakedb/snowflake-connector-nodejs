...
const { normalizeHost, isPrivateLink } = require('../url_util');
...
const { buildCacheKey, CacheTokenType } = require('../authentication/cache_key_builder');
...
function Connection(context) {
  ...
  this.setupOcspPrivateLink = function (host) {
    if (GlobalConfig.isOCSPChecksDisabled()) {
      Logger.getInstance().warn(
        'setupOcspPrivateLink() has no effect because OCSP is off. ' +
          'Enable OCSP with snowflake.configure({ disableOCSPChecks: false }) or ' +
          'snowflake.configure({ ocspFailOpen: true|false }) before calling it.',
      );
      return;
    }
    const normalized = normalizeHost(host);
    process.env.SF_OCSP_RESPONSE_CACHE_SERVER_URL = `http://ocsp.${normalized}/ocsp_response_cache.json`;
  };
  ...
  this.connectAsync = async function (callback) {
    ...
    if (isPrivateLink(connectionConfig.host)) {
      if (!GlobalConfig.isOCSPChecksDisabled()) {
        ...
        Logger.getInstance().debug(
          'Connection[id: %s] - skipping PrivateLink OCSP cache URL because OCSP is off',
          this.getId(),
        );
      }
    }
  ...
  this.destroy = function (callback) {
    ...
    services.sf.destroy({
      callback: function (err) {
        ...
        // Tear down the per-connection HTTP(S) agents so their pooled
        // keep-alive sockets and file descriptors are released.
        connectionConfig.destroyAgentCache();
        Logger.getInstance().trace('Connection[id: %s] - HTTP(S) agents destroyed', self.getId());
