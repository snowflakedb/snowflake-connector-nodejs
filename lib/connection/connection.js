...
} = require('../authentication/secure_storage/json_credential_manager');
...
function Connection(context) {
  ...
  Logger.getInstance().info(
    ...
    connectionConfig.getRetryTimeout(),
  ...
  this.isUp = function () {
    ...
    Logger.getInstance().trace(
      'Connection[id: %s] - isUp called. Returning: %s',
      this.getId(),
      isUp,
    );
    return isUp;
  ...
  this.isTokenValid = function () {
    ...
    Logger.getInstance().trace(
      'Connection[id: %s] - isSessionTokenValid: %s',
      this.getId(),
      isSessionValid,
    );
    ...
    Logger.getInstance().trace(
      'Connection[id: %s] - isMasterTokenValid: %s',
      this.getId(),
      isMasterValid,
    );
    ...
    Logger.getInstance().trace(
      'Connection[id: %s] - isTokenValid returned: %s',
      this.getId(),
      areTokensValid,
    );
  ...
  this.isValidAsync = async () => {
    ...
    try {
      ...
      Logger.getInstance().debug(
        'Connection[id: %s] - heartbeat failed: %s',
        this.getId(),
    ...
    return function (err) {
      ...
      if (err) {
        Logger.getInstance().error(
          'Connection[id: %s] - connection failed after %s milliseconds. Error: %s',
          self.getId(),
          connectingDuration,
          err.message || err,
        );
        ...
        Logger.getInstance().info(
          'Connection[id: %s] - connection established successfully after %s milliseconds',
          self.getId(),
          connectingDuration,
        );
      }
      ...
      if (!err && Parameters.getValue(Parameters.names.CLIENT_SESSION_KEEP_ALIVE)) {
        ...
        const KEEP_ALIVE_HEARTBEAT_FREQUENCY_IN_MS =
          Parameters.getValue(Parameters.names.CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY) *
  ...
  this.determineConnectionDomain = () =>
    connectionConfig.accessUrl && connectionConfig.accessUrl.includes('snowflakecomputing.cn')
      ? 'CHINA'
      : 'GLOBAL';
  ...
  this.connect = function (callback) {
    ...
    Logger.getInstance().info(
      'Connection[id: %s] - connecting. Associated Snowflake domain: %s',
      this.getId(),
      connectionDomain,
    );
    // invalid callback
    ...
    Logger.getInstance().debug(
      'Connection[id: %s] - using authentication type: %s',
      this.getId(),
      authenticationType,
    );

    // check if authentication type is compatible with connect()
    // external browser is allowed only when allowExternalBrowserSyncConnect is enabled
    // okta is not compatible with connect() due to its usage of async functions and special retry logic
    if (
      (authenticationType === AuthenticationTypes.EXTERNAL_BROWSER_AUTHENTICATOR &&
        !connectionConfig.getAllowExternalBrowserSyncConnect()) ||
      isOktaAuth(authenticationType)
    ) {
      const connectingDuration = timer.getDuration();
      Logger.getInstance().error(
        'Connection[id: %s] - connecting failed after %s milliseconds.' +
    ...
    auth
      ...
      .then(
        () => {
          Logger.getInstance().info(
            'Connection[id: %s] - authentication successful using: %s',
            this.getId(),
            connectionConfig.getAuthenticator(),
          );
          // JSON for connection
          ...
          initEasyLogging(connectionConfig.clientConfigFile).then(
            () => {
              Logger.getInstance().debug(
                'Connection[id: %s] - easyLogging initialized',
                this.getId(),
              );
              try {
                Logger.getInstance().debug(
                  'Connection[id: %s] - connecting through service',
                  this.getId(),
                );
                services.sf.connect({
                ...
                Logger.getInstance().debug(
                  'Connection[id: %s] - connection request initiated, waiting for response...',
                  this.getId(),
                );
                return this;
              } catch (e) {
                ...
                const connectingDuration = timer.getDuration();
                Logger.getInstance().error(
                  'Connection[id: %s] - failed to initiate connection after %s milliseconds. ' +
                    'Error: Unexpected error from calling connectCallback function in snowflake service - %s',
                  this.getId(),
                  connectingDuration,
                  e,
                );
            ...
            () => {
              const connectingDuration = timer.getDuration();
              Logger.getInstance().error(
                'Connection[id: %s] - failed to initialize easyLogging. ' +
                  'Connecting failed after %s milliseconds',
                this.getId(),
                connectingDuration,
              );
              callback(
                Errors.createClientError(ErrorCodes.ERR_CONN_CONNECT_INVALID_CLIENT_CONFIG, true),
                self,
              );
            },
          );
        ...
        (err) => {
          const connectingDuration = timer.getDuration();
          Logger.getInstance().error(
            'Connection[id: %s] - authentication failed. Error: %s. ' +
              'Connecting failed after %s milliseconds',
            this.getId(),
            err,
            connectingDuration,
  ...
  this.connectAsync = async function (callback) {
    ...
    Logger.getInstance().info(
      'Connection[id: %s] - async connecting. Associated Snowflake domain: %s',
      this.getId(),
      connectingDomain,
    );
    ...
    try {
      ...
      const connectingDuration = timer.getDuration();
      Logger.getInstance().error(
        'Connection[id: %s] - failed to initialize authenticator. ' +
          'Connecting failed after %s milliseconds',
        this.getId(),
        connectingDuration,
      );
    ...
    try {
      ...
      // JSON for connection
      body = Authenticator.formAuthJSON(
        connectionConfig.getAuthenticator(),
        connectionConfig.account,
        connectionConfig.username,
        connectionConfig.getClientType(),
      ...
      const connectingDuration = timer.getDuration();
      Logger.getInstance().info(
        'Connection[id: %s] - failed to connect async after %s milliseconds.' +
          'Failed during authentication. Error: %s',
        this.getId(),
        connectingDuration,
        authErr,
      );
      ...
      Logger.getInstance().error(
        'Connection[id: %s] - failed during authentication. Error: %s',
        this.getId(),
        authErr,
      );
      callback(authErr);
  ...
  this.fetchResult = function (options) {
    Logger.getInstance().trace(
      'Connection[id: %s] - fetchResult called with options',
      this.getId(),
    );
  ...
  this.destroy = function (callback) {
    ...
    services.sf.destroy({
      callback: function (err) {
        if (Util.exists(self.keepalive)) {
          ...
          Logger.getInstance().trace(
            'Connection[id: %s] - keepAlive interval cleared',
            self.getId(),
          );
        }
    ...
    Logger.getInstance().trace(
      'Connection[id: %s] - connection destroyed successfully',
      this.getId(),
    );
    // return the connection to facilitate chaining
    return this;
  ...
  async function getQueryResponse(queryId) {
    Logger.getInstance().trace(
      'Connection[id: %s] - requested query response for Query[id: %s]',
      id,
      queryId,
    );
    ...
    Errors.checkArgumentValid(
      queryIdRegex.test(queryId),
      ErrorCodes.ERR_GET_RESPONSE_QUERY_INVALID_UUID,
      queryId,
    );
    ...
    Logger.getInstance().debug(
      'Connection[id: %s] - fetching query response for Query[id: %s]',
      id,
      queryId,
    );
    const timer = new ExecutionTimer().start();
    ...
    Logger.getInstance().debug(
      'Connection[id: %s] - query response for Query[id: %s] fetched successfully after: %s milliseconds',
      id,
      queryId,
      fetchingDuration,
    );
  ...
  function extractQueryStatus(queryResponse) {
    ...
    if (!queryResponse || !queryResponse['data'] || !queryResponse['data']['queries']) {
      Logger.getInstance().trace(
        'Connection[id: %s] - No query data found, returning NO_QUERY_DATA status',
        id,
      );
      return status;
    }
  ...
  this.getQueryStatus = async function (queryId) {
    Logger.getInstance().trace(
      'Connection[id: %s] - getQueryStatus called for Query[id: %s]',
      this.getId(),
      queryId,
    );
  ...
  this.getQueryStatusThrowIfError = async function (queryId) {
    Logger.getInstance().trace(
      'Connection[id: %s] - getQueryStatusThrowIfError called for Query[id: %s]',
      this.getId(),
      queryId,
    );
    ...
    if (this.isAnError(status)) {
      ...
      Logger.getInstance().debug(
        'Connection[id: %s] - query error for Query[id: %s]. Error: %s. SQLState: %s',
        this.getId(),
        queryId,
        message,
        sqlState,
  ...
  this.getResultsFromQueryId = async function (options) {
    ...
    Logger.getInstance().trace(
      'Connection[id: %s] - getResultsFromQueryId called for Query[id: %s].',
      this.getId(),
      queryId,
    );
    ...
    while (queryStillExecuting) {
      Logger.getInstance().trace(
        'Connection[id: %s] - checking if Query[id: %s] is still executing. Retries with no data count: %d',
        this.getId(),
        queryId,
        noDataCounter,
      );
      ...
      if (!queryStillExecuting || status === QueryStatus.code.NO_QUERY_DATA) {
        Logger.getInstance().trace(
          'Connection[id: %s] - end of waiting for Query[id: %s] to finish executing. ' +
            queryStillExecuting
      ...
      if (QueryStatus.code[status] === QueryStatus.code.NO_DATA) {
        noDataCounter++;
        Logger.getInstance().trace(
          'Connection[id: %s] - no data returned for Query[id: %s]. Retries with no data count: %d',
          this.getId(),
          queryId,
          noDataCounter,
        );
        ...
        if (noDataCounter > asyncNoDataMaxRetry) {
          Logger.getInstance().error(
            'Connection[id: %s] - no data returned for Query[id: %s]. Retry limit: %s reached.',
            this.getId(),
            queryId,
            asyncNoDataMaxRetry,
          );
          throw Errors.createClientError(
            ErrorCodes.ERR_GET_RESULTS_QUERY_ID_NO_DATA,
            true,
            queryId,
          );
        }
    ...
    if (QueryStatus.code[status] === QueryStatus.code.NO_QUERY_DATA) {
      Logger.getInstance().error(
        'Connection[id: %s] - Query[id: %s] did not succeed. Final status: %s',
        this.getId(),
        queryId,
        status,
      );
    ...
    Logger.getInstance().debug(
      'Connection[id: %s] - Query[id: %s] succeeded. Fetching the result.',
      this.getId(),
      queryId,
    );
  ...
  this.isStillRunning = function (status) {
    Logger.getInstance().trace(
      'Connection[id: %s] - checking if status %s is still running',
      this.getId(),
      status,
    );
