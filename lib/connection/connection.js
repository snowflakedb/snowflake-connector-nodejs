...
function Connection(context) {
  ...
  this.connectAsync = async function (callback) {
    ...
    // Short-circuit: when session tokens are pre-supplied, the service is already
    // in Connected state (TokenInfo was populated in the constructor). No login
    // request is needed; just invoke the success callback directly.
    if (
      connectionConfig.getAuthenticator() === AuthenticationTypes.SESSION_TOKEN &&
      connectionConfig.sessionToken
    ) {
      Logger.getInstance().info(
        'Connection[id: %s] - session tokens pre-supplied, skipping login request',
        this.getId(),
      );
      connectCallback(self, callback, timer)(null);
      return this;
    }
