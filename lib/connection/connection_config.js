...
/**
 * Default chunk size (in MiB) used for S3 stage uploads when no
 * `uploadPartSizeMb` is configured. 8 MiB is comfortably above the 5 MiB S3
 * multipart minimum and keeps per-upload memory bounded for typical workloads.
 */
const DEFAULT_UPLOAD_PART_SIZE_MB = 8;
/**
 * Lower bound (in MiB) for `uploadPartSizeMb`. AWS S3 rejects multipart parts
 * smaller than 5 MiB except for the final part of an upload, so we apply this
 * floor uniformly to keep the multipart code path valid.
 */
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  // S3 stage upload part size, in mebibytes. The file-transfer agent reads
  // files in chunks of this size: small files (<= partSize) become a single
  // PutObject with a Buffer body; larger files become multipart uploads with
  // one Buffer-bodied UploadPart per chunk. Memory per upload is bounded by
  // partSize regardless of total file size, and per-part retries replace
  // whole-file retries on transient errors. AWS S3 imposes a 5 MiB minimum on
  // every part except the final one, hence the floor below. Stored as MiB
  // (rather than bytes) so connection-config callers express a human-readable
  // budget; we multiply to bytes at the use site in s3_util.js.
  ...
  /**
   * Returns the S3 stage upload part size in mebibytes. Used by the
   * file-transfer agent to chunk files into Buffer-bodied PutObject /
   * UploadPart requests; the call site multiplies by 1024*1024 to obtain
   * the byte size.
   *
   * @returns {Number}
   */
