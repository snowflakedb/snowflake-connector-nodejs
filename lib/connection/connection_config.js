...
function consolidateHostAndAccount(options) {
  ...
  if (Util.exists(options.region)) {
    Logger.default().info('The \'region\' configuration item is deprecated. Please use the \'account\' parameter with either the regionless format (e.g., myorgname-myaccountname) or the locator format (e.g., myaccountlocator.myregion). For more details, refer to: https://docs.snowflake.com/en/user-guide/admin-account-identifier');
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (validateCredentials) {
    ...
    if (!options.authenticator || !noUsernameRequiredFor.includes(options.authenticator.toUpperCase())) {
