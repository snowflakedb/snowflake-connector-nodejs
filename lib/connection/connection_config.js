...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (Util.isNode()) {
    ...
    if (!proxy && GlobalConfig.isEnvProxyActive()) {
      let proxyEnv = Util.getEnvVar('http_proxy') || Util.getEnvVar('https_proxy');

      if (proxyEnv) {
        proxyEnv = new URL(proxyEnv);
        proxy = {
          host: Util.validateEmptyString(proxyEnv.hostname),
          port: Number(Util.validateEmptyString(proxyEnv.port)),
          user: Util.validateEmptyString(proxyEnv.username),
          password: Util.validateEmptyString(proxyEnv.password),
          protocol: Util.validateEmptyString(proxyEnv.protocol),
          noProxy: Util.getNoProxyEnv(),
        };
      }
    }
  }
