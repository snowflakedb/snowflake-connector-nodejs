...
/**
 * Default chunk size (in MiB) used for stage uploads when no
 * `uploadPartSizeMb` is configured. 8 MiB sits comfortably above the only
 * hard floor in play (S3's 5 MiB multipart-part minimum) and keeps
 * per-upload memory bounded for typical workloads. Azure block-blob
 * `stageBlock` and the GCS Buffer-bodied PUT path impose no hard lower
 * bound, so the same default applies cleanly to all three CSPs.
 */
const DEFAULT_UPLOAD_PART_SIZE_MB = 8;
/**
 * Lower bound (in MiB) for `uploadPartSizeMb`. AWS S3 rejects multipart
 * parts smaller than 5 MiB except for the final part of an upload; Azure
 * `stageBlock` and the GCS upload path accept arbitrarily small chunks,
 * so this floor is driven entirely by S3. Applied uniformly across all
 * three CSPs to keep `connectionConfig` validation single-shot.
 */
const MIN_UPLOAD_PART_SIZE_MB = 5;
/**
 * Upper bound (in MiB) for `uploadPartSizeMb`. Azure `stageBlock` rejects
 * blocks larger than 4000 MiB; AWS S3 accepts up to 5120 MiB per part and
 * GCS resumable accepts arbitrarily large chunks, so 4000 is the tightest
 * floor across all three CSPs. Applied uniformly so a single value works
 * everywhere without per-CSP validation branching.
 */
const MAX_UPLOAD_PART_SIZE_MB = 4000;
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  // Stage upload part size, in MiB. Used by all three cloud-provider
  // backends in `lib/file_transfer_agent/`:
  //  - S3 (`s3_util.js`): files <= partSize become a single Buffer-bodied
  //    PutObject; larger files become an S3 multipart upload with one
  //    Buffer-bodied UploadPart per chunk.
  //  - Azure (`azure_util.js`): files <= partSize become a single
  //    Buffer-bodied `blockBlobClient.upload`; larger files split into
  //    Buffer-bodied `stageBlock` calls finalized by `commitBlockList`.
  //  - GCS (`gcs_util.js`): files <= partSize and the legacy presigned-URL
  //    path use a single Buffer-bodied PUT; larger files on the access-
  //    token path use a GCS resumable upload session with one Buffer-bodied
  //    chunk PUT per `Content-Range`.
  // Memory per upload is bounded by partSize on all three paths regardless
  // of total file size, and per-part retries replace whole-file retries on
  // transient errors. The 5 MiB floor is driven by S3's hard minimum on
  // every multipart part except the final one; the 4000 MiB ceiling is
  // driven by Azure `stageBlock`'s hard maximum (S3 accepts up to 5120 MiB
  // and GCS resumable is unbounded, so Azure is the tightest floor).
  // Stored as MiB (rather than bytes) so connection-config callers express
  // a human-readable budget; the call sites multiply to bytes.
  let uploadPartSizeMb = DEFAULT_UPLOAD_PART_SIZE_MB;
  if (Util.exists(options.uploadPartSizeMb)) {
    Errors.checkArgumentValid(
      Util.number.isPositiveInteger(options.uploadPartSizeMb) &&
        options.uploadPartSizeMb >= MIN_UPLOAD_PART_SIZE_MB &&
        options.uploadPartSizeMb <= MAX_UPLOAD_PART_SIZE_MB,
      ErrorCodes.ERR_CONN_CREATE_INVALID_UPLOAD_PART_SIZE_MB,
    );

    uploadPartSizeMb = options.uploadPartSizeMb;
  }
  ...
  if (options.queryTag) {
    Errors.checkArgumentValid(
      Util.isString(options.queryTag) && options.queryTag.length <= 2000,
      ErrorCodes.ERR_CONN_CREATE_INVALID_QUERY_TAG,
    );
  ...
  /**
   * Returns the stage upload part size in MiB. Prefer
   * {@link getUploadPartSizeBytes} from upload code paths; this getter is
   * kept for tests and for any caller that wants the human-readable unit.
   *
   * @returns {Number}
   */
  this.getUploadPartSizeMb = function () {
    return uploadPartSizeMb;
  };

  /**
   * Returns the stage upload part size in bytes. Used by all three
   * cloud-provider backends in the file-transfer agent (S3, Azure, GCS) to
   * size Buffer-bodied uploads.
   *
   * @returns {Number}
   */
  this.getUploadPartSizeBytes = function () {
    return uploadPartSizeMb * 1024 * 1024;
  };
