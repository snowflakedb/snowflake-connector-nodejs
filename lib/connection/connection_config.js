...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo)
{
  ...
  if (Util.exists(options.disableQueryContextCache)) {
    Errors.checkArgumentValid(Util.isBoolean(options.disableQueryContextCache),
      ErrorCodes.ERR_CONN_CREATE_INVALID_DIABLED_QUERY_CONTEXT_CACHE);
      disableQueryContextCache = options.disableQueryContextCache;
  }

  // var queryContextCacheSize = 5;
  // if(Util.exists(options.queryContextCacheSize)){
  //   Errors.checkArgumentValid(Util.isNumber(options.queryContextCacheSize),
  //     ErrorCodes.ERR_CONN_CREATE_INVALID_QUERY_CONTEXT_CACHE_SIZE);
  //     queryContextCacheSize = options.queryContextCacheSize;
  // }
...
const PARAM_QUERY_CONTEXT_CAHCE_SIZE = 'queryContextCacheSize';
...
function createParameters()
{
  ...
  return [
    ...
    {
      name: PARAM_QUERY_CONTEXT_CAHCE_SIZE,
      defaultValue: 5,
      validate: isNonNegativeNumber
    },
  ];
...
ConnectionConfig.prototype.getQueryContextCacheSize = function ()
{
  return this._getParameterValue(PARAM_QUERY_CONTEXT_CAHCE_SIZE);
};
