...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  let httpHeadersCustomizers = [];
  if (Util.exists(options.httpHeadersCustomizers)) {
    Errors.checkArgumentValid(Util.checkValidHTTPHeaderCustomizers(options.httpHeadersCustomizers),
      ErrorCodes.ERR_CONN_CREATE_INVALID_HTTP_HEADER_CUSTOMIZERS);

    httpHeadersCustomizers = options.httpHeadersCustomizers;
  }
  ...
  this.getHTTPHeadersCustomizers = function () {
