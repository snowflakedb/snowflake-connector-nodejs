...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  let disablePermissionCheckForTomlConfig = options.disablePermissionCheckForTomlConfig;
   if (Util.exists(options.disablePermissionCheckForTomlConfig)) {
    Errors.checkArgumentValid(
      Util.isBoolean(options.clientStoreTemporaryCredential),
      ErrorCodes.ERR_CONN_CREATE_INVALID_CLIENT_STORE_TEMPORARY_CREDENTIAL
    );

    disablePermissionCheckForTomlConfig = options.disablePermissionCheckForTomlConfig;
  }
  ...
  this.getDisablePermissionCheckForTomlConfig = function () {
    return disablePermissionCheckForTomlConfig;
  }
