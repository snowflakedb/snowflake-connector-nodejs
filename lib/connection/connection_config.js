...
const DEFAULT_PARAMS = [
  ...
  'disableConsoleLogin',
  'allowExternalBrowserSyncConnect',
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  let allowExternalBrowserSyncConnect = false;
  if (Util.exists(options.allowExternalBrowserSyncConnect)) {
    Errors.checkArgumentValid(
      Util.isBoolean(options.allowExternalBrowserSyncConnect),
      ErrorCodes.ERR_CONN_CREATE_INVALID_ALLOW_EXTERNAL_BROWSER_SYNC_CONNECT,
    );

    allowExternalBrowserSyncConnect = options.allowExternalBrowserSyncConnect;
  }
  ...
  /**
   * Returns whether the external browser authenticator is allowed with connect() (sync).
   *
   * @returns {Boolean}
   */
  this.getAllowExternalBrowserSyncConnect = function () {
    return allowExternalBrowserSyncConnect;
  };
