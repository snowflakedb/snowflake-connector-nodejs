...
const {
  readTokenFromFile,
  shouldReadTokenFromFile,
} = require('../configuration/connection_configuration');
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  // backends in `lib/file_transfer_agent/` (S3, Azure, GCS) to bound
  // per-upload memory regardless of total file size and to give per-part
  // retry granularity on transient errors. The 5 MiB floor is driven by
  // S3's hard minimum on every multipart part except the final one; the
  // 4000 MiB ceiling is driven by Azure `stageBlock`'s hard maximum
  // (S3 accepts up to 5120 MiB and GCS resumable is unbounded, so Azure
