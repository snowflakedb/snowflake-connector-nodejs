...
const { readTokenFromFile } = require('../configuration/connection_configuration');
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  // If an OAUTH token_file_path (or the normalized tokenFilePath) was passed in
  // the connection options, read the token from it. The SDK otherwise only does
  // this when it loads connections.toml itself (createConnection with no options).
  if (
    Util.exists(options.authenticator) &&
    options.authenticator.toUpperCase() === AuthenticationTypes.OAUTH_AUTHENTICATOR &&
    !Util.string.isNotNullOrEmpty(options.token) &&
    (options.token_file_path || options.tokenFilePath)
  ) {
    readTokenFromFile(options);
  }
