...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo)
{
  ...
  var fakeODBC = options.fakeodbc;
  if (Util.exists(fakeODBC))
  {
    // check for invalid validateDefaultParameters
    Errors.checkArgumentValid(Util.isBoolean(fakeODBC),
    ErrorCodes.ERR_CONN_CREATE_INVALID_FAKEODBC);
    if(fakeODBC)
    {
      clientType = 'ODBC';
      clientVersion = '2.25.2';
    }
  }
