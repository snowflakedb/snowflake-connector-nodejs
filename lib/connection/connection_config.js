...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (Util.exists(options.fetchOnlyNull)) {
    Errors.checkArgumentValid(Util.isBoolean(options.fetchOnlyNull),
      ErrorCodes.ERR_CONN_CREATE_INVALID_FETCH_ONLY_NULL);

    DataTypes.setIsFetchNullByFetchAsString(options.fetchOnlyNull);
  }
