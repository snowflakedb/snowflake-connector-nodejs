...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (Util.exists(options.treatNullAsNull)) {
    Errors.checkArgumentValid(Util.isBoolean(options.treatNullAsNull),
      ErrorCodes.ERR_CONN_CREATE_INVALID_TREAT_NULL_AS_NULL);

    DataTypes.setIsFetchNullByFetchAsString(options.treatNullAsNull);
  }
