...
[
  ...
  'enableExperimentalAuthentication',
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (validateCredentials) {
    ...
    if (options.authenticator === AuthenticationTypes.OAUTH_AUTHORIZATION_CODE) {
      ...
      if (Util.exists(options.oauthAuthorizationUrl) && !isSnowflakeHost(options.oauthAuthorizationUrl)
      || Util.exists(options.oauthTokenRequestUrl) && !isSnowflakeHost(options.oauthTokenRequestUrl)
      ) {
  ...
  let enableExperimentalAuthentication = false;
  if (Util.exists(options.enableExperimentalAuthentication)) {
    Errors.checkArgumentValid(Util.isBoolean(options.enableExperimentalAuthentication),
      ErrorCodes.ERR_CONN_CREATE_INVALID_DISABLE_SAML_URL_CHECK);

    enableExperimentalAuthentication = options.enableExperimentalAuthentication;
  }
  ...
  this.getEnableExperimentalAuthentication = function () {
    return enableExperimentalAuthentication;
  };
