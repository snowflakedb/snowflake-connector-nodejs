...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (Util.exists(options.originalNullValueByDefault)) {
    Errors.checkArgumentValid(Util.isBoolean(options.originalNullValueByDefault),
      ErrorCodes.ERR_CONN_CREATE_INVALID_ORIGINAL_NULL_VALUE_BY_DEFAULT);

     DataTypes.setNullValue(!options.originalNullValueByDefault);
  }
