...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (Util.exists(options.workloadIdentityHost)) {
    // The value's shape is validated later by the AWS attestation code, since it is dictated
    // by AWS STS.
    Errors.checkArgumentValid(
      Util.isString(options.workloadIdentityHost),
      ErrorCodes.ERR_CONN_CREATE_INVALID_WORKLOAD_IDENTITY_PARAMETERS,
      'workloadIdentityHost must be a string',
    );
  }
  ...
  this.workloadIdentityHost = options.workloadIdentityHost;
  ...
  // Per-connection cache of HTTP(S) agents. Keeping this on the connection
  // config (rather than a module-global Map) guarantees that agents, their
  // pooled TLS sockets, and proxy routing are never shared across independent
  // connections.
 ...
 */
ConnectionConfig.prototype.destroyAgentCache = function () {
