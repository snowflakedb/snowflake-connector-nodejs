...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (Util.exists(options.nullStringWithFetchAsString)) {
    Errors.checkArgumentValid(Util.isBoolean(options.nullStringWithFetchAsString),
      ErrorCodes.ERR_CONN_CREATE_INVALID_NULL_STRING_WITH_FETCH_AS_STRING);

    DataTypes.setIsFetchNullByFetchAsString(options.nullStringWithFetchAsString);
  }
