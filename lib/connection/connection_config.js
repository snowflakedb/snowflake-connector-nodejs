...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (validateCredentials) {
    const noUsernameRequiredFor = [
      ...
      AuthenticationTypes.SESSION_TOKEN,
    ];
    if (
      (!options.authenticator && !Util.exists(options.sessionToken)) ||
    ...
    if (
      (!Util.exists(options.authenticator) && !Util.exists(options.sessionToken)) ||
  ...
  // (unless session tokens are pre-supplied, in which case use SESSION_TOKEN)
  if (!Util.exists(authenticator)) {
    if (Util.exists(options.sessionToken)) {
      authenticator = AuthenticationTypes.SESSION_TOKEN;
    } else {
