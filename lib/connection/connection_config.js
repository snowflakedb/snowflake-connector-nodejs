...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  if (Util.exists(options.disableGCPTokenUpload)) {
    Errors.checkArgumentValid(Util.isBoolean(options.disableGCPTokenUpload),
      ErrorCodes.ERR_CONN_CREATE_INVALID_DISABLE_GCP_TOKEN_UPLOAD);

    process.env.SNOWFLAKE_DISABLE_GCP_TOKEN_UPLOAD = options.disableGCPTokenUpload;
  } else {
    process.env.SNOWFLAKE_DISABLE_GCP_TOKEN_UPLOAD = false;
  }
