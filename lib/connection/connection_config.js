...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  // check for invalid serverSessionKeepAlive
  let serverSessionKeepAlive = false;
  if (Util.exists(options.serverSessionKeepAlive)) {
    Errors.checkArgumentValid(
      Util.isBoolean(options.serverSessionKeepAlive),
      ErrorCodes.ERR_CONN_CREATE_INVALID_SERVER_SESSION_KEEP_ALIVE,
    );
    serverSessionKeepAlive = options.serverSessionKeepAlive;
  }
  ...
  /**
   * Returns the server session keep alive setting. When true, the session is
   * not destroyed on the server side when the connection is closed, allowing
   * async queries to continue running.
   *
   * @returns {Boolean}
   */
  this.getServerSessionKeepAlive = function () {
    return serverSessionKeepAlive;
  };
