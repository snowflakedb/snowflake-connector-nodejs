...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  //  - GCS (`gcs_util.js`): always a single Buffer-bodied PUT today;
  //    `partSize` is consulted only as a soft advisory upper bound for
  //    in-flight memory and triggers a debug log when exceeded.
  // Memory per upload is bounded by partSize on the S3 and Azure paths
  // regardless of total file size, and per-part retries replace whole-file
  // retries on transient errors. The 5 MiB floor below is driven by S3's
  // hard minimum on every multipart part except the final one; Azure and
  // GCS accept arbitrarily small chunks. Stored as MiB (rather than bytes)
  // so connection-config callers express a human-readable budget; the
  // call sites multiply to bytes.
  let uploadPartSizeMb = DEFAULT_UPLOAD_PART_SIZE_MB;
  if (Util.exists(options.uploadPartSizeMb)) {
    Errors.checkArgumentValid(
      Util.isNumber(options.uploadPartSizeMb) &&
        Number.isInteger(options.uploadPartSizeMb) &&
