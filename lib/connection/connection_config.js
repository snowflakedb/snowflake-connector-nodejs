...
function consolidateHostAndAccount(options) {
  ...
  if (!Util.isString(options.accessUrl) || !Util.exists(options.accessUrl)) {
    ...
    if (Util.exists(options.host)) {
      ...
      options.accessUrl = Util.format('%s://%s.%s.snowflakecomputing.com%s', protocol, options.account, options.region, port);
    } else {
      options.accessUrl = Util.format('%s://%s.snowflakecomputing.com%s', protocol, options.account, port);
    }
