...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo)
{
  ...
  if (Util.isNode() && (Util.exists(proxyHost) || Util.exists(proxyPort)))
  {
    ...
    if (Util.exists(proxyUser) || Util.exists(proxyPassword))
    {
      ...
      if (!proxyProtocol) {
        console.warn("Warning: configured to connect to an authenticated proxy server through HTTP. To use HTTPS, set 'proxyProtocol' to 'HTTPS'")
      }
  ...
  var socketKeepAlive = options.socketKeepAlive ?? true;
  if (Util.exists(options.socketKeepAlive))
  {
    Errors.checkArgumentValid(Util.isBoolean(options.socketKeepAlive),
      ErrorCodes.ERR_CONN_CREATE_INVALID_SOCKET_KEEP_ALIVE);
  }
  ...
  this.getSocketKeepAlive = function ()
  {
  ...
  this.getCachedAgent = function (reqUrl)
  {
    const key = getOrigin(reqUrl);
    return agentCache.get(key);
  };

  this.setCachedAgent = function (reqUrl, agent)
  {
    const key = getOrigin(reqUrl);
    Logger.getInstance().debug(`Caching agent for origin '${key}'`);
