...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
  ...
  let httpHeaderCustomizers = [];
  if (Util.exists(options.httpHeaderCustomizers)) {
    Errors.checkArgumentValid(Util.isValidHTTPHeaderCustomizers(options.httpHeaderCustomizers),
      ErrorCodes.ERR_CONN_CREATE_INVALID_HTTP_HEADER_CUSTOMIZERS);

    httpHeaderCustomizers = options.httpHeaderCustomizers;
  }
  ...
  this.getHttpHeaderCustomizers = function () {
    return httpHeaderCustomizers;
  };
