...
function convertRawObject(rawColumnValue, column, context) {
  const metadata =  context.metadataFields.reduce(function (map, obj) {
    map[obj.name] = obj;
    return map;
  }, {});
  ...
  Object.keys(json).forEach(function (key) {
    result[key] = mapStructuredTypeFieldValue(json[key], metadata[key], context, column);
  });
...
function mapStructuredTypeFieldValue(columnValue, columnMetadata, context, column) {
  ...
  case 'boolean':
    ...
    break;
  case 'timestamp_ltz':
    console.log(`TIMESTAMP_LTZ_OUTPUT_FORMAT : ${context.statementParameters['TIMESTAMP_LTZ_OUTPUT_FORMAT']}`);
    console.log(`TIMESTAMP_OUTPUT_FORMAT : ${context.statementParameters['TIMESTAMP_OUTPUT_FORMAT']}`);
    console.log(`TIMESTAMP_LTZ_OUTPUT_FORMAT : ${context.statementParameters['TIMESTAMP_LTZ_OUTPUT_FORMAT']}`);
    console.log.info(`TIMESTAMP_OUTPUT_FORMAT : ${context.statementParameters['TIMESTAMP_OUTPUT_FORMAT']}`);
    ...
    break;
  case 'timestamp_ntz':
    console.log(`TIMESTAMP_NTZ_OUTPUT_FORMAT : ${context.statementParameters['TIMESTAMP_NTZ_OUTPUT_FORMAT']}`);
    console.log(`TIMESTAMP_OUTPUT_FORMAT : ${context.statementParameters['TIMESTAMP_OUTPUT_FORMAT']}`);
    ...
    value = convertTimestampNtzString(columnValue, context.format, moment.tz.zone('UTC'), column.getScale()).toSfDate();
    break;
  case 'timestamp_tz':
    console.log(`TIMESTAMP_TZ_OUTPUT_FORMAT : ${context.statementParameters['TIMESTAMP_OUTPUT_FORMAT']}`);
    console.log(`TIMESTAMP_TZ_OUTPUT_FORMAT : ${context.statementParameters['TIMESTAMP_OUTPUT_FORMAT']}`);
