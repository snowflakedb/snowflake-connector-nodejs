...
function Column({
  ...
  /**
   * Returns the value of this column in a row without caching the converted
   * value in the row. Intended for single-pass consumers (row streaming) that
   * read every cell exactly once, so the row keeps only the raw wire values.
   *
   * @param {Object} row
   *
   * @returns {*}
   */
  this._getRowValueOnce = function (row) {
    return extractFromRowOnce.call(this, row, context, false);
  };

  /**
   * Returns the value of this column in a row as a String without caching the
   * converted value in the row.
   *
   * @param {Object} row
   *
   * @returns {String}
   */
  this._getRowValueAsStringOnce = function (row) {
    return extractFromRowOnce.call(this, row, context, true);
  };
}
      ...
      return GlobalConfig.jsonColumnVariantParser(rawColumnValue, {
        onNonJsonCompliantVariant: () => {
          Logger.getInstance().warn(
            `Column "${column.getName()}" contains a non-JSON-compliant variant value. ` +
...
/**
 * Extracts the value of a column from a given row without writing the
 * converted value back into the row. Produces the same value as
 * extractFromRow(); a cell that was already converted through the caching path
 * is read through that path so both stay consistent.
 *
 * @param {Object} row
 * @param {Object} context
 * @param {Boolean} asString
 ...
 * @returns {*}
 */
function extractFromRowOnce(row, context, asString) {
  const columnIndex = this.getIndex();

  if (row._arrayProcessedColumns[columnIndex]) {
    return extractFromRow.call(this, row, context, asString);
  }

  let ret = row.values[columnIndex];

  // same shortcut as extractFromRow(): a variant requested as a string is the
  // raw wire value, no parse needed
  if (asString && this.isVariant()) {
    return ret;
  }

  if (ret !== null) {
    ret = context.convert(ret, this, context);
  }

  const extractFn = !asString ? context.toValue : context.toString;
  return extractFn(ret);
}
