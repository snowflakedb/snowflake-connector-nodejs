...
function Column({ columnMetadata, index, statementParameters, resultVersion, onPrecisionLoss }) {
  ...
  this.onPrecisionLoss = onPrecisionLoss;
 ...
 * @returns {Object}
 */
function convertRawNumber(rawColumnValue, column) {
 ...
 * @returns {Object}
 */
function convertJsonObject(json, column, context) {
  if (context.fieldsMetadata) {
    ...
    Object.keys(json).forEach(function (key) {
      ...
      result[key] = mapStructuredTypeValue(json[key], column, context, fieldMetadata);
    });
    return result;
  } else {
    return json;
 ...
 * @returns {Object}
 */
function convertJsonArray(json, column, context) {
  if (context.fieldsMetadata) {
    const result = [];
    json.forEach(function (value) {
      result.push(mapStructuredTypeValue(value, column, context, context.fieldsMetadata[0]));
    });
    return result;
  } else {
    return json;
 ...
 * @returns {Object}
 */
function convertJsonMap(json, column, context) {
  if (Array.isArray(context.fieldsMetadata) && context.fieldsMetadata.length === 2) {
    ...
    Object.keys(json).forEach(function (key) {
      const convertedKey = mapStructuredTypeValue(key, column, context, keyMetadata);
      const convertedValue = mapStructuredTypeValue(json[key], column, context, valueMetadata);
...
const convertRawStructuredType = (convertJsonFn) => (rawColumnValue, column, context) => {
  if (Util.string.isNotNullOrEmpty(rawColumnValue)) {
    try {
      ...
      return convertJsonFn(json, column, context);
...
function mapStructuredTypeValue(columnValue, column, context, metadataField) {
  const formatLtz =
  ...
  switch (metadataField.type) {
    ...
    case 'real':
    case 'fixed':
      value = toValueFromNumber(convertRawNumber(columnValue, column));
      break;
    case 'boolean':
    ...
    case 'object': {
      ...
      value = convertJsonObject(columnValue, column, internalContext);
      break;
    }
    case 'array': {
      ...
      value = convertJsonArray(columnValue, column, internalArrayContext);
      break;
    }
    case 'map': {
      ...
      value = convertJsonMap(columnValue, column, internalMapContext);
      break;
    }
    default:
