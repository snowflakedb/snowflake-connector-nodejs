...
function mapStructuredTypeValue(columnValue, context, metadataField) {
  ...
  switch (metadataField.type) {
    ...
    case 'time':
      ...
      value = convertTimeString(columnValue, context.format, 'UTC', metadataField.scale).toSfTime();
      break;
    case 'binary':
...
const convertTimestampTzString = function (stringValue, formatSql, timezone, scale) {
  const dateFormat = dateTimeFormatConverter.convertSnowflakeFormatToDateFormat(formatSql, scale);
  const epochSeconds = dayjs(stringValue, dateFormat).unix();
...
const convertTimestampNtzString = function (stringValue, formatSql, timezone, scale) {
  const dateFormat = dateTimeFormatConverter.convertSnowflakeFormatToDateFormat(formatSql, scale);
  const epochSeconds = dayjs.utc(stringValue, dateFormat).unix();
  return new SfTimestamp(epochSeconds, 0, scale, timezone, formatSql);
};

const convertDateString = function (stringValue, formatSql) {
  const dateFormat = dateTimeFormatConverter.convertSnowflakeFormatToDateFormat(formatSql, 0);
  const epochSeconds = dayjs.utc(stringValue, dateFormat).unix();
...
const convertTimeString = function (stringValue, formatSql, timezone, scale) {
  const dateFormat = dateTimeFormatConverter.convertSnowflakeFormatToDateFormat(formatSql, scale);
  const parsedTime = dayjs(stringValue, dateFormat);
  const epochSeconds = parsedTime.hour() * 3600 + parsedTime.minute() * 60 + parsedTime.second();
