...
function Column(options, index, statementParameters, resultVersion) {
  ...
  this.isMap = createFnIsColumnOfType(type, () => SqlTypes.isArray(type, fieldsMetadata), SqlTypes);
...
function convertJsonMap(json, context) {
  ...
  if (Array.isArray(context.metadataFields) && context.metadataFields.length === 2) {
    const keyMetadata = context.metadataFields[0];
    const valueMetadata = context.metadataFields[1];
    Object.keys(json).forEach(function (key) {
...
function mapStructuredTypeValue(columnValue, context, metadataField) {  let value;
