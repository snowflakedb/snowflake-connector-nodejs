...
function Result(options) {
  ...
  const onPrecisionLoss = () => {
    if (precisionLossTelemetrySent) return;
    precisionLossTelemetrySent = true;
    const telemetryRequest = buildInbandTelemetryRequest(
  ...
  const onNonJsonCompliantVariant = () => {
    if (nonJsonVariantTelemetrySent) return;
    ...
    this._services.sf
      .requestAsync(telemetryRequest)
      .catch((err) =>
        Logger().debug('Failed to send non-JSON-compliant variant telemetry: %s', err),
      );
  };
  ...
  const processChunk = function (chunk) {
    ...
    const processRows = function () {
      ...
      while (rowIndex < rowsLength) {
        ...
        if (
          rowIndex - startIndex >= context.rowBatchSize &&
    ...
    timer.stop();
    Logger().debug(
      `Finished processing Chunks: Processed ${rowsLength} rows in ${timer.getDuration()} ms`,
    );
  };
...
Result.prototype.getNumUpdatedRows = function () {
  ...
  if (!this._numUpdatedRows) {
    ...
    if (StatementType.isDml(statementTypeId)) {
      if (
        StatementType.isInsert(statementTypeId) ||
