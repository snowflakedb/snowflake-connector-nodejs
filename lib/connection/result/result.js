...
Result.prototype.fetchRows = function (options) {
  ...
  var end = options.endIndex;
    ...
    !Util.isNumber(end) || end >= returnedRows
      ? returnedRows - 1
      : Math.floor(end);
...
function findFirstOverlappingChunk(chunks, windowStartIndex, windowEndIndex) {
  var helper = function (
    ...
    if (
      chunkIndexMiddle === chunkIndexLeft ||
      ...
      if (chunkIndexLeft === chunkIndexRight) {
        ...
        if (
          chunks[chunkIndexLeft].overlapsWithWindow(
            windowStartIndex,
            windowEndIndex
          )
        ) {
        ...
        // that's the chunk we were looking for
          ...
          chunks[chunkIndexRight].overlapsWithWindow(
            windowStartIndex,
            windowEndIndex
          )
        ) {
  ...
  if (!this._numUpdatedRows) {
    ...
    if (StatementType.isDml(statementTypeId)) {
      if (
        ...
        for (
          var rowIndex = 0, rowsLength = rows.length;
          rowIndex < rowsLength;
          rowIndex++
        ) {
          var row = rows[rowIndex];
          for (
            var colIndex = 0, colsLength = columns.length;
            colIndex < colsLength;
            colIndex++
          ) {
