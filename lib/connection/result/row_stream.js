...
function RowStream(statement, context, options) {
  ...
  this._read = function () {
    ...
    } else if (context.result || isStatementErrorFatal(context.resultError, context.type)) {
 ...
 * @returns {Boolean}
 */
function isStatementErrorFatal(error, contextType) {
  return (Errors.isOperationFailedError(error) && error.sqlState) || isFileUploadError(error, contextType);
}
