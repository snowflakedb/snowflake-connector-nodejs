...
function RowStream(statement, context, options) {
  ...
  let useOnePassExtraction = false;
  ...
  const processRowBuffer = function processRowBuffer() {
    ...
    if (!mapColumnIdToExtractFnName) {
      mapColumnIdToExtractFnName = buildMapColumnExtractFnNames(
        columns,
        fetchAsString,
        useOnePassExtraction,
      );
    }
  ...
  const onResultStreamData = function onResultStreamData(chunk) {
    ...
    previousChunk = chunk;
    useOnePassExtraction = chunk.isRowsetDiscardable();
    mapColumnIdToExtractFnName = null;
 ...
 *   strings.
 * @param {Boolean} useOnePassExtraction whether converted values can be
 *   returned without caching them in the chunk rows.
 *
 * @returns {Object}
 */
function buildMapColumnExtractFnNames(columns, fetchAsString, useOnePassExtraction) {
  const fnNameGetColumnValue = useOnePassExtraction ? '_getRowValueOnce' : '_getRowValue';
  const fnNameGetColumnValueAsString = useOnePassExtraction
    ? '_getRowValueAsStringOnce'
    : '_getRowValueAsString';
...
function externalizeRow(row, columns, mapColumnIdToExtractFnName, rowMode) {
  ...
  for (let index = 0, length = columns.length; index < length; index++) {
    ...
    externalizedRow[isArrayRowMode ? index : column.getName()] = column[extractFnName](row);
  }
