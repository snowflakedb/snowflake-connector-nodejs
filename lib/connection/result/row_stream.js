...
function buildMapColumnExtractFnNames(columns, fetchAsString) {
  // Every cell of a streamed row is externalized exactly once, so the
  // converted value is returned directly instead of being cached in the row.
  const fnNameGetColumnValue = '_getRowValueOnce';
  const fnNameGetColumnValueAsString = '_getRowValueAsStringOnce';
