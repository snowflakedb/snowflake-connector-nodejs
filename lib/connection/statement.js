...
const file_transfer_agent = require(".././file_transfer_agent/file_transfer_agent");
...
exports.createStatementPreExec = function (
  ...
  let threshold = Parameters.getValue(
    Parameters.names.CLIENT_STAGE_ARRAY_BINDING_THRESHOLD
  );
  ...
  if (numBinds > threshold) {
    return createStageStatementpreExec(
      options,
      context,
      services,
      connectionConfig
...
exports.createStatementPostExec = function (
  ...
  if (Util.exists(statementOptions.streamResult)) {
    Errors.checkArgumentValid(
      Util.isBoolean(statementOptions.streamResult),
...
function createStageStatementpreExec(
  ...
  return new StageBindingStatementPreExec(
    statementOptions,
    statementContext,
    services,
    connectionConfig
...
function createContextPreExec(statementOptions, services, connectionConfig) {
  ...
  if (!Util.exists(statementOptions.requestId)) {
    ...
    Errors.checkArgumentExists(
      Util.exists(statementOptions.sqlText),
      ErrorCodes.ERR_CONN_EXEC_STMT_MISSING_SQL_TEXT
    );

    // check for invalid sql text
    Errors.checkArgumentValid(
      Util.isString(statementOptions.sqlText),
  ...
  if (Util.exists(statementOptions.requestId)) {
    Errors.checkArgumentValid(
      Util.isString(statementOptions.requestId),
  ...
  if (Util.exists(statementOptions.parameters)) {
    Errors.checkArgumentValid(
      Util.isObject(statementOptions.parameters),
  ...
  if (Util.exists(binds)) {
    ...
    for (let index = 0, length = binds.length; index < length; index++) {
      Errors.checkArgumentValid(
        JSON.stringify(binds[index]) !== undefined,
        ErrorCodes.ERR_CONN_EXEC_STMT_INVALID_BIND_VALUES,
        binds[index]
      );
    }
  }
  ...
  if (Util.exists(statementOptions.internal)) {
    Errors.checkArgumentValid(
      Util.isBoolean(statementOptions.internal),
...
function BaseStatement(statementOptions, context, services, connectionConfig) {
  ...
  this.getColumn = function (columnIdentifier) {
    return context.result
      ? context.result.getColumn(columnIdentifier)
      : undefined;
  };
  ...
  context.refresh = function (callback) {
    ...
    const sendRequestFn = context.statementId
      ? sendRequestPostExec
      : sendRequestPreExec;
  ...
  context.onStatementRequestComp = async function (err, body) {
    ...
    if (!err) {
      ...
      if (
        !context.statementId &&
        Errors.isOperationFailedError(err) &&
        err.data
      ) {
    ...
    if (!suppressComplete) {
      ...
      Logger.getInstance().debug(
        "refreshed result of statement with %s",
        context.requestId
...
function RowStatementPreExec(
   ...
   * @param {Object} body
   */
  context.onStatementRequestSucc = createOnStatementRequestSuccRow(
    this,
    context
  );
  ...
  return function (body) {
    ...
    if (!context.result) {
      if (body.data.resultIds != undefined && body.data.resultIds.length > 0) {
        ...
        exports.createStatementPostExec(
          context,
          context.services,
        ...
        context.statementId = context.result.getQueryId();
      }
    } else {
    ...
    if (context.isMulti == null || context.isMulti == false) {
      ...
      if (
        context.type !== statementTypes.ROW_POST_EXEC ||
...
function FileStatementPreExec(
  ...
  context.onStatementRequestSucc = async function (body) {
    ...
    const fta = new file_transfer_agent(context);
    await fta.execute();
...
function StageBindingStatementPreExec(
   ...
   * @param {Object} connectionConfig
   */
  this.StageBindingRequest = async function (
    options,
    context,
    services,
    connectionConfig
...
function createFnFetchRows(statement, context) {
  return function (options) {
    ...
    // if there was an error the last time we tried to get the result
      ...
      if (
        Errors.isOperationFailedError(context.resultError) &&
...
function fetchRowsFromResult(options, statement, context) {
  ...
  operation.on("complete", function (err, continueCallback) {
    ...
    if (
      Errors.isLargeResultSetError(err) &&
      err.response &&
...
function sendRequestPreExec(statementContext, onResultAvailable) {
  ...
  if (Util.exists(statementContext.parameters)) {
    ...
    Logger.getInstance().debug(
      "context.parameters=" + statementContext.parameters
    );
  }
...
function sendSfRequest(statementContext, options, appendQueryParamOnRetry) {
  ...
  options.callback = async function (err) {
    ...
    if (
      ...
      err &&
      Util.isRetryableHttpError(
        err.response,
        false // no retry for HTTP 403
      )
    ) {
...
function hasNextResult(statement, context) {
  return function (options) {
    return (
      context.multiResultIds != null &&
      context.multiCurId + 1 < context.multiResultIds.length
    );
  };
}
