  ...
  statementOptions, services, connectionConfig)
{
  ...
  // TODO: Temporary change to pretend as ODBC to enable multiple statements
  if(connectionConfig.getClientType() == "ODBC")
  {
    if(statementOptions.parameters == undefined)
    {
      statementOptions.parameters = {};
    }
    statementOptions.parameters["ODBC_QUERY_RESULT_FORMAT"] = "JSON";
  }
  ...
  statementOptions, services, connectionConfig)
{
  ...
  if (Util.exists(binds))
  {
    ...
    if (Parameters.names.MULTI_STATEMENT_COUNT in binds)
    {
      statementOptions.parameters[Parameters.names.MULTI_STATEMENT_COUNT] = 
      binds[Parameters.names.MULTI_STATEMENT_COUNT];
      delete binds[Parameters.names.MULTI_STATEMENT_COUNT];
    }
