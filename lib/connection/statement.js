...
function BaseStatement(
  ...
  context.onStatementRequestComp = async function (err, body) {
    Logger.getInstance().debug(`context.onStatementRequestComp ${JSON.stringify(err)}`);
    Logger.getInstance().debug(`context.onStatementRequestComp ${JSON.stringify(body)}`);
    ...
    if (!err) {
      Logger.getInstance().debug(`context.onStatementRequestSucc(body);`);
...
function createOnStatementRequestSuccRow(statement, context) {
  return function (body) {
    Logger.getInstance().debug('createOnStatementRequestSuccRow(statement, context)');
    // if we don't already have a result
    if (!context.result) {
      Logger.getInstance().debug('if (!context.result)');
      ...
      if (body.data.resultIds !== undefined && body.data.resultIds.length > 0) {
        Logger.getInstance().debug('if (!context.result) 22');
        //multi statements
        ...
        Logger.getInstance().debug('if (!context.result) 33');
      ...
      Logger.getInstance().debug('context.result.refresh(body);');
    ...
    if (context.isMulti == null || context.isMulti === false) {
      ...
      if (context.type !== statementTypes.ROW_POST_EXEC || context.type !== statementTypes.FILE_POST_EXEC) {
        Logger.getInstance().debug(' statementTypes.ROW_POST_EXEC');
...
function FileStatementPreExec(
  ...
  context.onStatementRequestSucc = async function (body) {
    Logger.getInstance().debug('  context.fileMetadata = body;');

    context.fileMetadata = body;

    const fta = new FileTransferAgent(context);
    Logger.getInstance().debug(`new FileTransferAgent(context) ${JSON.stringify(context)}`);
    await fta.execute();

    Logger.getInstance().debug(`new FileTransferAgent(context) executed ${JSON.stringify(context)}`);
