  ...
  statementOptions, services, connectionConfig)
{
  ...
  statementContext.arrSqlText = statementOptions.arrSqlText;
  statementContext.multiCurId = statementOptions.multiCurId;
  if (statementOptions.arrSqlText) {
    statementContext.sqlText = statementOptions.arrSqlText[statementOptions.multiCurId];
  }
  // set the statement type
  ...
  statementOptions, context, services, connectionConfig)
{
  ...
  context.onStatementRequestComp = async function (err, body)
  {
    ...
    this.NextResult = createNextResult(this, context);
  };
  ...
  return function (body)
  {
    ...
    if (!context.result)
    {
      if (body.data.resultIds != undefined && body.data.resultIds.length > 0)
      {
        ...
        context.arrSqlText = context.sqlText.split(';');
        if (this._resultIds.length != context.arrSqlText.length) {
          //special case for store proc
          var upperCaseSql = context.sqlText.toUpperCase();
          var splitByEnd = context.sqlText.split("END;");
            context.arrSqlText = [];
            var start = 0;
            var end = 0;
            for (var i = 0; i < splitByEnd.length; i++)
            {
                if (splitByEnd[i].length > 0) {
                    end = end + splitByEnd[i].length + 4;
                    if (end > context.sqlText.length - 1)
                        end = context.sqlText.length - 1;
                    var sqlString = context.sqlText.substring(start, end);
                    context.arrSqlText.push(sqlString);
                    start = end+1;
                }
...
function createNextResult(statement, context)
{
  return function (options)
  {
