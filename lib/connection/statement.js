...
exports.createStatementPostExec = function (statementOptions, services, connectionConfig) {
  ...
  Errors.checkArgumentValid(
    Util.isString(statementOptions.queryId),
...
function createContextPreExec(statementOptions, services, connectionConfig) {
  ...
  if (Util.exists(statementOptions.asyncExec)) {
    Errors.checkArgumentValid(
      Util.isBoolean(statementOptions.asyncExec),
  ...
  if (Util.exists(statementOptions.describeOnly)) {
    Errors.checkArgumentValid(
      Util.isBoolean(statementOptions.describeOnly),
...
function createFnFetchRows(statement, context) {
  return function (options) {
    ...
    if (context.isFetchingResult) {
      process.nextTick(function () {
        options.end(
          Errors.createClientError(ErrorCodes.ERR_STMT_FETCH_ROWS_FETCHING_RESULT).externalize(),
          statement,
        );
      });
...
function sendSfRequest(statementContext, options, appendQueryParamOnRetry) {
  ...
  options.callback = async function (err) {
    ...
    console.log('ERRRRR ----->', err.cause.code);
    // if (err.cause.code === 'ECONNRESET') {
    //   await callbackOrig.call(this, err);
    // }
    // Track connection errors and throw error to original callback
    // if (err && err.code && err.code.startsWith('ECONN')) {
    //   Logger.getInstance().debug(
    //     'Connection error detected for statement with request id %s: %s',
    //     statementContext.requestId,
    //     err.code,
    //   );

    //   // Don't retry on connection errors, invoke original callback with error
    //   Logger.getInstance().error(
    //     'Connection error details for statement with request id %s: %s',
    //     statementContext.requestId,
    ...
    // Determine if error is retryable: either an HTTP retryable status, or a network error (no response)
    if (
