...
function createFnStreamRows(statement, context)
{
  return function (options)
  {
    ...
    if (Util.exists(options))
    {
      ...
      var arrayRowMode = options.arrayRowMode;
      if (Util.exists(arrayRowMode))
      {
        Errors.checkArgumentValid(Util.isBoolean(arrayRowMode),
          ErrorCodes.ERR_STMT_STREAM_ROWS_INVALID_ARRAY_ROW_MODE);
      }
    }
