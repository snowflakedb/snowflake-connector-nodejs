...
function Core(options)
{
  ...
  var createConnection = function createConnection(connectionOptions, config)
  {
    ...
    var connectionConfig =
      new ConnectionConfig(connectionOptions, validateCredentials, qaMode, Object.assign(clientInfo, GlobalConfig.getCustomClientEnvironment()));
  ...
  var instance =
    {
      ...
      */
      ...
      configure: function (options)
      {
        ...
        // Allow merging of additional fields into client environment.
        let clientEnvironment = options.clientEnvironment;
        if (Util.exists(clientEnvironment)) {
          Errors.checkArgumentValid(Util.isObject(clientEnvironment),
              ErrorCodes.ERR_GLOBAL_CONFIGURE_INVALID_CLIENT_ENVIRONMENT);

          GlobalConfig.setCustomClientEnvironment(clientEnvironment);
        }
      }
    };
