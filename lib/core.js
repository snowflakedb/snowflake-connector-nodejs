...
function Core(options) {
  ...
  if (Util.exists(connectionClass)) {
    ...
    Logger.getInstance().debug(
      'Connection class was not overridden. Default connection class will be used.',
    );
  }
  ...
  const createConnection = function createConnection(connectionOptions, config) {
    ...
    if (!connectionOptions) {
      Logger.getInstance().info(
        'Connection options were not specified. Loading connection configuration.',
      );
      try {
        connectionOptions = loadConnectionConfiguration();
      } catch (error) {
        Logger.getInstance().error(
          'Unable to load the connection configuration. Error: %s',
          error.message,
        );
        Errors.checkArgumentExists(
          Util.exists(connectionOptions),
    ...
    Logger.getInstance().info(
      'Connection[id: %s] - connection object created successfully.',
      connection.getId(),
    );
    return connection;
  };
  ...
  const instance = {
    ...
    deserializeConnection: function (options, serializedConnection) {
      ...
      Errors.checkArgumentValid(
        Util.isString(serializedConnection),
    ...
    serializeConnection: function (connection) {
      Logger.getInstance().trace(
        'Connection[id: %s] - serializing connection.',
        connection.getId(),
      );
    ...
    configure: function (options) {
      ...
      if (Util.exists(jsonColumnVariantParser)) {
        Errors.checkArgumentValid(
          Util.isFunction(jsonColumnVariantParser),
      ...
      if (Util.exists(xmlColumnVariantParser)) {
        Errors.checkArgumentValid(
          Util.isFunction(xmlColumnVariantParser),
        ...
        Logger.getInstance().debug(
          'Creating XML Column Variant Parser with parameters from core options',
        );
      }
      ...
      if (Util.exists(customCredentialManager)) {
        Errors.checkArgumentValid(
          Util.isObject(customCredentialManager),
        ...
        Logger.getInstance().debug(
          'Setting customCredentialManager to value from core options %s',
  ...
  function extractLogLevel(options) {
    ...
    if (Util.exists(logTag)) {
      Errors.checkArgumentValid(
        LoggerCore.isValidLogTag(logTag),
  ...
  function ConnectionFactory(connectionOptions) {
    ...
    const { serializeFirstConnect } = require('./authentication/auth_coordinator');

    const authenticator = connectionOptions
      ? (connectionOptions.authenticator || '').toUpperCase()
      : '';
    ...
    this.create = function () {
      ...
      const doConnect = () => {
        ...
        return new Promise((resolve, reject) => {
          connection.connect(function (err, conn) {
            if (err) {
              Logger.getInstance().error(
                'Connection[id: %s] - Unable to connect. Error: %s',
                conn.getId(),
                err.message,
              );
              ...
              Logger.getInstance().debug(
                'Connection[id: %s] - connected successfully. Callback called.',
                conn.getId(),
              );
              resolve(conn);
            }
      ...
      if (isBrowserAuth) {
        const host = connectionOptions.host || connectionOptions.account || '';
        const username = connectionOptions.username || '';
        return serializeFirstConnect(host, username, authenticator, doConnect);
      }
    ...
    this.destroy = function (connection) {
      ...
      return new Promise((resolve) => {
        connection.destroy(function (err, conn) {
          if (err) {
            Logger.getInstance().error(
              'Connection[id: %s] - disconnecting failed with error: %s',
              conn.getId(),
              err.message,
            );
          } else {
            Logger.getInstance().debug(
              'Connection[id: %s] - connection disconnected successfully. Callback called.',
              conn.getId(),
            );
          }
          resolve();
    ...
    this.validate = async function (connection) {
      Logger.getInstance().debug(
        'Connection[id: %s] - validating connection instance',
        connection.getId(),
      );
