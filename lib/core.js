...
function Core(options) {
  ...
  if (Util.exists(connectionClass)) {
    Logger.getInstance().info('Connection class provided in driver core options will be used.');
    Errors.assertInternal(
  ...
  const createConnection = function createConnection(connectionOptions, config) {
    ...
    if (connectionOptions == null) {
      Logger.getInstance().info( 'Connection options were not specified. Loading local connection configuration.');
      try {
    ...
    Logger.getInstance().debug( 'Connection configuration validated.');
