...
function Core(options) {
  ...
  Logger.getInstance().info('Loading driver core module.');
  ...
  if (Util.exists(connectionClass)) {
    ...
    Logger.getInstance().debug('Connection class was not overrode. Default connection class will be used.');
  }
  ...
  const createConnection = function createConnection(connectionOptions, config) {
    ...
    Logger.getInstance().info( 'Connection object created successfully');
    return connection;
  };
  ...
  const instance =
    {
      ...
      */
      ...
      deserializeConnection: function (options, serializedConnection) {
        ...
        Logger.getInstance().debug('Deserializing connection');
      ...
      serializeConnection: function (connection) {
        Logger.getInstance().debug('Serializing connection.');
      ...
      configure: function (options) {
        Logger.getInstance().info('Configuring Snowflake core module.');
  ...
  function ConnectionFactory(connectionOptions) {
    ...
    this.create = function () {
      ...
      return new Promise((resolve, reject) => {
        connection.connect(
          function (err, conn) {
            if (err) {
              Logger.getInstance().error('Unable to connect. Error: %s', err.message);
              reject(new Error(err.message));
            } else {
              Logger.getInstance().debug('Connected successfully. Callback called.');
              resolve(conn);
            }
          }
        );
    ...
    this.destroy = function (connection) {
      ...
      return new Promise((resolve) => {
        connection.destroy(function (err) {
          if (err) {
            Logger.getInstance().error('Disconnecting failed with error: %s', err.message);
          } else {
            Logger.getInstance().debug('Connection disconnected successfully. Callback called.');
          }
          resolve();
        });
      });
    ...
    */
    this.validate = async function (connection) {
      Logger.getInstance().debug('Validating connection instance');
