...
/**
 * Creates a new StageError.
 *
 * @param {Number} errorCode the error code to use when building the error.
 * @param {Object} response the response sent by AWS  S3, Azure Blob.
 *
 * @returns {Error}
 */
exports.createStageError = function(errorCode, response)
{
  return createError(errorTypes.StageError,
  {
    code     : errorCode,
    response : response
  });
...
/**
 * Determines if a given error is an StageError.
 *
 * @param {Error} error
 *
 * @returns {Boolean}
 */
exports.isStageError = function(error)
{
  return isErrorOfType(error, errorTypes.StageError);
};
