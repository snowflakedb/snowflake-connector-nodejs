...
const HTTP_HEADER_CONTENT_ENCODING_UTF8 = 'UTF-8';
...
function AzureUtil(connectionConfig) {
   ...
   * call (small files) or a multi-block `stageBlock` + `commitBlockList`
   * sequence (large files), depending on `connectionConfig.uploadPartSizeMb`.
   *
   * Why Buffer-bodied uploads, not a Readable stream:
   *  - Memory per upload is bounded by `uploadPartSizeMb`, regardless of file
   *    size.
   *  - Per-block retry granularity for the multipart path on transient errors,
   *    instead of re-uploading the whole file on a single-PUT failure.
   *  - Aligns with the multipart pattern already used in the Python and Java
   *    Snowflake connectors and Azure's own recommendation for non-trivial
   *    uploads.
   *  - Some HTTP runtimes have known regressions in stream-piped request
   *    bodies (e.g. bun 1.3.x compiled binaries — `body.pipe(httpRequest)`
   *    transmits zero bytes for `fs.ReadStream` after a few HTTPS calls in a
   *    long-running process, hanging stage uploads at the server-side idle
   *    timeout); Buffer bodies sidestep this entirely.
   *
  ...
  this.uploadFile = async function (dataFile, meta, encryptionMetadata, maxConcurrency) {
    // `uploadPartSizeMb` is exposed in MiB for human readability; convert to
    // bytes for chunk math.
    ...
    try {
      ...
      try {
        await blockBlobClient.commitBlockList(blockIds, {
          ...
          blobHTTPHeaders: {
            blobContentEncoding: HTTP_HEADER_CONTENT_ENCODING_UTF8,
            blobContentType: HTTP_HEADER_VALUE_OCTET_STREAM,
          },
        });
      } catch (err) {
        applyAzureUploadError(err, meta);
      ...
      // error; for other errors (e.g., short read) record the raw error so
      // the caller's retry logic still kicks in.
   ...
   *
   * Internal SDK callers reach this with a Buffer (the encrypt path produces
   * a Buffer; `uploadFile` reads to a Buffer for the small-file path).
   * External SDK consumers can still pass a `Readable` for the legacy
   * `uploadStream` codepath; that path is preserved for backward
   * compatibility but is best avoided where memory bounds and
   * compiled-bun compatibility matter — prefer `uploadFile(path, ...)`
   * for those cases.
   *
...
/**
 * Detect a SAS-token expiry surfaced as a 403 with Azure's standard
 * "signature not valid" or "server failed to authenticate" error text.
 * Shared by `applyAzureUploadError`; module-scoped (no closure over
 * `AzureUtil` instance state) for symmetry with the S3 helper.
 */
 ...
 * `meta.resultStatus` convention. Shared by the single-block
 * (`uploadFileStream`) and multi-block (`uploadFileMultipart`) paths
 * so retry semantics stay consistent.
 */
 ...
 * when the upload is encrypted. Shared by `uploadFileStream` and
 * `uploadFileMultipart` (used at `commitBlockList` time for multi-block
 * uploads, since blob metadata attaches at commit, not at stageBlock).
 */
