    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        readableStream.on('data', (data) => {
          console.log(`Blob client downloaded ${JSON.stringify(data)} bytes for ${fullDstPath}`);
          writer.write(data, () => {
            console.log(`[Callback] Blob client downloaded ${data.length} bytes for ${fullDstPath}`);
          });
        });
        readableStream.on('end', () => {
          console.log(`Blob client ended for ${fullDstPath}`);
          writer.end(resolve);
        });
        readableStream.on('error', reject);
      });
    } catch (err) {
      console.log(`Error occurred when fetching file to ${fullDstPath}: ${JSON.stringify(err)}`)
      if (err['statusCode'] == 403 && detectAzureTokenExpireError(err)) {
        meta['lastError'] = err;
        meta['resultStatus'] = resultStatus.RENEW_TOKEN;
        return;
