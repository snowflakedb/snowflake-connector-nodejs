...
function AzureUtil(azure, filestream) {
  ...
  const execFileOptions = process.platform === 'win32' ? { shell: true } : {};
  if (NODE_MAJOR_VERSION < 16) {
    const cp = require('child_process');
    const result = cp.execSync('npm ls @azure/core-lro').toString();
    if (!result.includes('@azure/core-lro@2.6')) {
      console.log(`execFileOptions  ${execFileOptions}`);
      cp.execFileSync('npm', ['install', '@azure/core-lro@2.6.0'], execFileOptions);
    }
  }
