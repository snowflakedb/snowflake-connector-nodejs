  ...
  */
    ...
    tmpDir = null, chunkSize = blockSize * 4 * 1024)
  {
    ...
    await new Promise(function (resolve, reject)
    {
      ...
      infile.on('data', function (chunk)
      {
        console.log(`Chunk is ${JSON.stringify(chunk)} and has length ${chunk.length}`)
        if (chunk.length !== 0) {
          size += chunk.length;
          ...
          Logger.getInstance().warn('Fetched chunk has length 0');
        }
      });
      infile.on('close', function ()
      {
        Logger.getInstance().warn(`Decrypting final content of size ${size}`);
        if (size > 0) {
          outfile.write(decipher.final());
        } else {
          Logger.getInstance().warn(`There are not chunk content so we do not decipher anything in ${tempOutputFileName}`);
        }
        outfile.close(resolve);
      });
    });
