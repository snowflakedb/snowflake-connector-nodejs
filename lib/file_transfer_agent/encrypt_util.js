...
const AES_BLOCK_SIZE_256 = 256;
const blockSize256 = parseInt(AES_BLOCK_SIZE_256 / 8);  // in bytes
...
function encrypt_util(encrypt, filestream, temp)
{
  ...
  */
    ...
    tmpDir = null, chunkSize = blockSize * 4 * 1024)
  {
    ...
    if (decodedKey.length == blockSize256)
    {
      cipher = crypto.createCipheriv(AES_ECB_256, decodedKey, null);
    }
    else
    {
  ...
  */
    ...
    tmpDir = null, chunkSize = blockSize * 4 * 1024)
  {
    ...
    if (decodedKey.length == blockSize256)
    {
      decipher = crypto.createDecipheriv(AES_ECB_256, decodedKey, null);
    }
    else
    {
      decipher = crypto.createDecipheriv(AES_ECB, decodedKey, null);
    }
