...
function TempFileGenerator() {
  ...
  this.file = function (
    option = { dir: os.tmpdir(), prefix: '', postfix: '', extension: '' },
    callback,
  ) {
    try {
  ...
  this.checkDirInTemp = function (directoryPath) {
    ...
    if (directoryPath.includes(os.tmpdir())) {
      ...
      Logger.getInstance().warn(
        `dir option must be relative to ${os.tmpdir()}, found ${directoryPath}`,
      );
    }
    return false;
  };
...
function EncryptUtil(encrypt, filestream, temp) {
  ...
  const fs = typeof filestream !== 'undefined' ? filestream : require('../filesystem');
  ...
  function createEncryptionMetadata(
    encryptionMaterial,
    keySize,
    encryptedKey,
    dataIv,
  ...
  this.encryptDataCBC = function (encryptionMaterial, data) {
    ...
    return {
      encryptionMetadata: createEncryptionMetadata(
        encryptionMaterial,
        keySize,
        encryptedKey,
        dataIv,
   ...
   */
  this.encryptFile = async function (
    encryptionMaterial,
    inputFilePath,
    tmpDir = null,
  ...
  this.encryptFileCBC = async function (
    encryptionMaterial,
    inputFilePath,
    tmpDir = null,
    chunkSize = blockSize * 4 * 1024,
    ...
    const fileKey = getSecureRandom(keySize);
    const dataCipher = crypto.createCipheriv(AES_CBC.cipherName(keySize), fileKey, dataIv);
    const encryptedFilePath = await performFileStreamCrypto(
      dataCipher,
      tmpDir,
      inputFilePath,
      chunkSize,
  ...
  this.encryptFileGCM = async function (encryptionMaterial, inputFilePath, tmpDir = null) {
    ...
    const encryptedFilePath = await writeContentToFile(
      tmpDir,
   ...
   */
  this.decryptFile = async function (
    metadata,
    encryptionMaterial,
    inputFilePath,
    tmpDir = null,
    ...
    return await this.decryptFileCBC(
      metadata,
      encryptionMaterial,
      inputFilePath,
      tmpDir,
  ...
  this.decryptFileCBC = async function (
    metadata,
    encryptionMaterial,
    inputFilePath,
    tmpDir = null,
    ...
    const keyDecipher = crypto.createDecipheriv(
      AES_ECB.cipherName(decodedKek.length),
      decodedKek,
      null,
    );
    ...
    const dataDecipher = crypto.createDecipheriv(
      AES_CBC.cipherName(fileKey.length),
      fileKey,
      ivBytes,
    );
  ...
  //TODO: SNOW-940981: Add proper usage when feature is ready
  this.decryptFileGCM = async function (
    metadata,
    encryptionMaterial,
    inputFilePath,
    tmpDir = null,
