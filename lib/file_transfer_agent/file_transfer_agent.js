...
const SnowflakeFileEncryptionMaterial =
  require('./remote_storage_util').SnowflakeFileEncryptionMaterial;
...
function FileTransferAgent(context) {
  ...
  async function uploadFilesinSequential(fileMeta) {
    ...
    while (index < fileMetaLen) {
      ...
      // Credential renewal maps to the two GCS credential styles (see
      // gcs_util.js): RENEW_TOKEN refreshes the client/token and re-applies it
      // to the remaining metas; RENEW_PRESIGNED_URL re-mints URLs by re-running
      // updateFileMetasWithPresignedUrl(). For GCS these are mutually exclusive —
      // token mode never renews a presigned URL, and vice versa.
  ...
  async function uploadOneFile(meta) {
    ...
    try {
      if (meta['requireCompress']) {
        const result = await SnowflakeFileUtil.compressFileWithGZIP(
          meta['srcFilePath'],
          meta['tmpDir'],
        );
  ...
  async function updateFileMetasWithPresignedUrl() {
    ...
    // Skip if not GCS
    if (storageClient !== remoteStorageUtil || stageLocationType !== GCS_FS) {
      return;
    }

    if (commandType === CMD_TYPE_UPLOAD) {
      // re-run (re-resolve) the PUT once per file to mint
      // a presigned URL per destination object (the two GCS credential styles
      // are explained in gcs_util.js). Final object names are decided on the
      // client — wildcard expansion + compression can rename `data.csv` ->
      // `data.csv.gz` — so GS cannot presign them up front, and each presigned
      // URL is bound to one exact object key.
      //
      // Skip entirely in access-token mode: a downscoped token
      // (stageInfo.creds.GCS_ACCESS_TOKEN) is folder-scoped and already covers
      // every file, so there is nothing per-file to fetch.
      //
      // Skipping is also required for correctness, not just speed. On versioned
      // stages every resolution advances a server-side "put rank" and returns a
      // NEW path (.../versions/<id>/<rank>/...). The original token stays pinned
      // to <rankA>, so a re-resolution would repoint the upload to <rankB> while
      // we still send the <rankA> token -> GCS 403 (storage.objects.create),
      // even for a single file.
      if (stageInfo['creds'] && stageInfo['creds']['GCS_ACCESS_TOKEN']) {
        return;
      }
      ...
      for (const meta of fileMetadata) {
        ...
        commandWithSingleFile = commandWithSingleFile.replace(
          filePathToReplace,
          fileNameToReplaceWith,
        );

        const options = { sqlText: commandWithSingleFile };
        const newContext = statement.createContext(
          options,
          context.services,
  ...
  function initEncryptionMaterial() {
    if (response['data'] && response['data']['encryptionMaterial']) {
      ...
      if (commandType === CMD_TYPE_UPLOAD) {
        encryptionMaterial.push(
          new SnowflakeFileEncryptionMaterial(
  ...
  function processFileCompressionType() {
    ...
    if (sourceCompression === 'auto_detect') {
      ...
      if (
        userSpecifiedSourceCompression === typeof 'undefined' ||
