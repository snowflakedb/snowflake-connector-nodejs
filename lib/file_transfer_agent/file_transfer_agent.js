...
function file_transfer_agent(context)
{
  ...
  */
  ...
  function getAllFilesInfo (rootdir, includeSubDir) {
    const fileInfos = [];
    const subDirectories = [];
    const matchingFileNames = glob.sync(path.join(rootdir, '*'));
    ...
    if(includeSubDir) {
        for (const subDir of subDirectories) {
          fileInfos.push(...getAllFilesInfo(subDir, includeSubDir));
        }
    }
  ...
  function getSubDirectoryFileObject() {
