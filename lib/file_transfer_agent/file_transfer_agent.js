...
var SnowflakeRemoteStorageUtil = require('./remote_storage_util').remote_storage_util;
...
function file_transfer_agent(context) {
  var context = context;
  var remoteStorageUtil = new SnowflakeRemoteStorageUtil(context.connectionConfig);
  ...
  */
  async function transferAccelerateConfig() {
    if (stageLocationType === S3_FS) {
      var client = remoteStorageUtil.createClient(stageInfo, false);
      ...
      try {
        ...
        remoteStorageUtil.destroyClient(stageInfo, client);
      }
    }
  }
