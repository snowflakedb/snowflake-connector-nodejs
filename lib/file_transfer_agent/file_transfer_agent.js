  ...
  async function updateFileMetasWithPresignedUrl() {
    ...
    if (storageClient === remoteStorageUtil) {
      ...
      if (stageLocationType === GCS_FS) {
        if (commandType === CMD_TYPE_UPLOAD) {
          // This per-file re-resolution exists solely to mint a presigned URL for
          // each destination object. When the server scopes credentials with an
          // access token instead (stageInfo.creds.GCS_ACCESS_TOKEN), there is no
          // presigned URL to fetch and the token already covers the whole staging
          // directory, so a single resolution suffices for every file. A session
          // uses one credential style throughout (access token OR presigned URL,
          // never a mix), so the presence of an access token here is sufficient to
          // skip. Re-resolving in that case is not only wasteful but unsafe: for
          // versioned stages whose staging path carries a per-PUT segment, the
          // re-resolved path would differ from the one the existing token is scoped
          // to and the upload would fail with 403.
