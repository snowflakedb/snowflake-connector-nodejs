...
function file_transfer_agent(context)
{
  ...
  */
  async function uploadOneFile(meta)
  {
    ...
    catch (err)
    {
      ...
      meta['errorDetails'] += ` file=${meta['srcFileName']}, real file=${meta['realSrcFileName']}`;
    }
    finally
    {
      fs.rmdirSync(meta['tmpDir'], { recursive: true });
    }
    ...
    if (storageClient === SnowflakeRemoteStorageUtil)
    {
      ...
      if (stageLocationType === GCS_FS)
      {
        ...
        else if (commandType == CMD_TYPE_DOWNLOAD)
        {
          ...
          {
            if (presignedUrl.length > index)
            {
              meta['presignedUrl'] = presignedUrl[index];
            }
          }
        }
      }
  ...
  */
  function getFileNameFromPutCommand(command)
  {
    ...
    var startIndex = command.indexOf('file://') + 7;
  ...
  */
  function parseCommand()
  {
    ...
    if (commandType === CMD_TYPE_UPLOAD)
    {
      ...
      if (fs.existsSync(root))
      {
        ...
        if (dir.isDirectory())
        {
          ...
          var fileNameFullPath = root + '\\' + fileName;

          // If file name has a wildcard
          if (fileName.includes('*'))
          {
            // Get all file names that matches the wildcard
            var matchingFileNames = glob.sync(root + '\\' + fileName);
    ...
    presignedUrl = stageInfo['presignedUrl'];
  }
