  ...
  async function uploadOneFile(meta)
  {
    ...
    finally
    {
      ...
      for (var matchingFileName of matchingFileNames)
      {
        try
        {
          await fs.promises.unlink(matchingFileName);
        }
        catch (err)
        {
          throw (err);
      ...
      //await Promise.all(matchingFileNames.map(async (matchingFileName) =>
      //{
      //  await fs.unlink(matchingFileName);
      //}));
      ...
      //// Remove all files inside tmp folder and delete tmp folder
      //var rimraf = require("rimraf");
      //rimraf(meta['tmpDir'], { maxRetries: 3, retryDelay: 10 }, (err) =>
      //{
      //  if (err)
      //    throw (err);
      //});
