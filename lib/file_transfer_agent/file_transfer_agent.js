  ...
  */
  async function transferAccelerateConfig()
  {
    if (stageLocationType === S3_FS)
    {
      ...
      var command = new GetBucketAccelerateConfigurationCommand({ Bucket: s3location.bucketName});

      try
      {
        const getBucketResponse = await client.send(command);
        useAccelerateEndpoint = (getBucketResponse['AccelerateConfiguration']['Status'] == 'Enabled');
      }
      catch (err)
      {
