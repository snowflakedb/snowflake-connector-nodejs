...
function FileTransferAgent(context) {
  ...
  */
  this.execute = async function () {
    if (fileStream) {
      Logger.getInstance().debug(`fileStream`);
      ...
      Logger.getInstance().debug(` else fileStream`);
      parseCommand();
      Logger.getInstance().debug(` else fileStream 2 `);
      initFileMetadata();
      Logger.getInstance().debug(` else fileStream 3`);
      if (commandType === CMD_TYPE_UPLOAD) {
        if (filesToPut.length === 0) {
          throw new Error('No file found for: ' + fileName);
        }
        Logger.getInstance().debug(` processFileCompressionType`);
      ...
      Logger.getInstance().debug(` transferAccelerateConfig`);
      await transferAccelerateConfig();
      Logger.getInstance().debug(` updateFileMetasWithPresignedUrl`);
      await updateFileMetasWithPresignedUrl();
      Logger.getInstance().debug(` updateFileMetasWithPresignedUrl  2`);
      ...
      if (commandType === CMD_TYPE_UPLOAD) {
        Logger.getInstance().debug(` upload`);
        await upload(largeFileMetas, smallFileMetas);
      }

      if (commandType === CMD_TYPE_DOWNLOAD) {
        Logger.getInstance().debug(` download`);
  ...
  */
  async function upload(largeFileMetas, smallFileMetas) {
    Logger.getInstance().debug(` createClient finish`);
    const storageClient = getStorageClient(stageLocationType);
    Logger.getInstance().debug(` createClient finish ${JSON.stringify(stageInfo)}`);
    const client = storageClient.createClient(stageInfo, false);
    Logger.getInstance().debug(` createClient finish`);
    ...
    if (smallFileMetas.length > 0) {
      Logger.getInstance().debug(` uploadFilesinSequential(smallFileMetas)`);
      //await uploadFilesinParallel(smallFileMetas);
      await uploadFilesinSequential(smallFileMetas);
    }
    if (largeFileMetas.length > 0) {
      Logger.getInstance().debug(`uploadFilesinSequential(largeFileMetas)`);
  ...
  */
  async function download(largeFileMetas, smallFileMetas) {
    ...
    if (smallFileMetas.length > 0) {
      ...
      Logger.getInstance().debug(` downloadFilesinSequential`);
      await downloadFilesinSequential(smallFileMetas);
    }
    if (largeFileMetas.length > 0) {
      Logger.getInstance().debug(` downloadFilesinSequential large`);
  ...
  */
  async function downloadFilesinSequential(fileMeta) {
    ...
    while (index < fileMetaLen) {
      ...
      Logger.getInstance().debug(` downloadOneFile result`);
      if (result['resultStatus'] === resultStatus.RENEW_TOKEN) {
        const client = renewExpiredClient();
        for (let index2 = index; index2 < fileMetaLen; index2++) {
          fileMeta[index2]['client'] = client;
  ...
  */
  async function downloadOneFile(meta) {
    ...
    try {
      const storageClient = getStorageClient(meta['stageLocationType']);
      Logger.getInstance().debug(` await storageClient.downloadOneFile(meta);`);
  ...
  */
  async function updateFileMetasWithPresignedUrl() {
    const storageClient = getStorageClient(stageLocationType);
    Logger.getInstance().debug(`getStorageClient(stageLocationType)`);
      ...
      if (stageLocationType === GCS_FS) {
        if (commandType === CMD_TYPE_UPLOAD) {
          ...
          for (const meta of fileMetadata) {
            ...
            Logger.getInstance().debug(` statement.sendRequest(newContext) ${JSON.stringify(newContext)}`);
            const ret = await statement.sendRequest(newContext);
            Logger.getInstance().debug(` statement.sendRequest(newContext) ${JSON.stringify(ret)}`);
