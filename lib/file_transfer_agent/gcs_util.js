...
function GCSUtil(connectionConfig, httpClient) {
   ...
   *      available and the file exceeds `uploadPartSizeMb`. Bounds
   *      in-flight memory by `uploadPartSizeMb` and gives per-chunk
   ...
   * so `partSize` is rounded down to that granularity for non-final
   * chunks; the final chunk eats the remainder.
   *
   * @param {String} dataFile
