...
const { MULTIPART_THRESHOLD_BYTES, MULTIPART_PART_SIZE_BYTES } = require('./multipart');
...
function GCSUtil(connectionConfig, httpClient) {
  ...
  this.uploadFile = async function (dataFile, meta, encryptionMetadata, maxConcurrency) {
    ...
    // the multipart threshold on the small-file branch and by file size on
    // the legacy branch.
   ...
   *
   * GCS requires every chunk except the final to be a 256 KiB multiple,
   * so `MULTIPART_PART_SIZE_BYTES` is rounded down to that granularity for
   * non-final chunks; the final chunk eats the remainder.
   *
   * @param {String} dataFile
  ...
  this.uploadFileResumable = async function (dataFile, fileSize, meta, encryptionMetadata) {
    ...
    // Round MULTIPART_PART_SIZE_BYTES down to a 256-KiB multiple so non-final
    // chunks satisfy GCS's hard alignment requirement. The remainder rides in
    // the final chunk, which has no alignment constraint.
    const CHUNK_GRANULARITY = 256 * 1024;
    let alignedChunkSize =
      Math.floor(MULTIPART_PART_SIZE_BYTES / CHUNK_GRANULARITY) * CHUNK_GRANULARITY;
