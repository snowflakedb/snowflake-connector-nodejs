...
function GCSUtil(connectionConfig, httpClient) {
      ...
      try {
        ...
        while (position < fileSize) {
          ...
          if (chunkResp.status === 308) {
            ...
            if (!parsedRange) {
              throw new Error(
                `GCS resumable 308 returned no usable Range header (got "${commitedRangeStr}"); cannot safely resume`,
              );
            }
  ...
  this.generateFileURL = function (stageInfo, filename) {
    ...
    // Default to https:// only when no scheme is present, so an explicit http://
    // (e.g. a local mock) is kept. Matches Universal Driver code.
 ...
 *
 * Note on 403: a persistent 403 here is usually deterministic — an
 * auth-boundary/path mismatch (e.g. a token scoped to a different put-rank
 * folder than the upload target; see `updateFileMetasWithPresignedUrl`) — not a
 * transient failure. It is still mapped to NEED_RETRY as a best effort for the
 * genuinely transient 403s, but retries won't fix a boundary mismatch.
 */
