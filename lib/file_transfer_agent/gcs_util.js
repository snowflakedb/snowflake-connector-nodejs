...
function GCSUtil(connectionConfig, httpClient, fileStream) {
  ...
  if (connectionConfig.getOverrideEnvProxy()) {
    proxyString = ProxyUtil.convertProxyToString(proxy);
    originalHttpsProxy = process.env.HTTPS_PROXY;
    isOverrideEnvProxyEnabled = Util.exists(proxyString) && proxyString !== originalHttpsProxy;
  }
  /**
  ...
  this.getFileHeader = async function (meta, filename) {
    if (meta['resultStatus'] === resultStatus.UPLOADED ||
      ...
      if (meta['presignedUrl']) {
        await axiosInstance.get(meta['presignedUrl'])
          .catch(err => {
          ...
          if (shouldPerformGCPBucket(accessToken)) {
            ...
            if (isOverrideEnvProxyEnabled) {
              ...
              metadata = await meta['client'].gcsClient
                ...
                .getMetadata();
              Util.exists(originalHttpsProxy) ? process.env.HTTPS_PROXY = originalHttpsProxy : delete process.env.HTTPS_PROXY;
            } else {
              metadata = await meta['client'].gcsClient
                .bucket(gcsLocation.bucketName)
                .file(gcsLocation.path + filename)
            ...
            const response = await axiosInstance.head(url, { headers: gcsHeaders });
  ...
  this.uploadFileStream = async function (fileStream, meta, encryptionMetadata) {
    ...
    try {
      if (shouldPerformGCPBucket(accessToken)) {
        ...
        await axiosInstance.put(uploadUrl, fileStream, { maxBodyLength: Infinity, headers: gcsHeaders });
      }
    } catch (err) {
  ...
  this.nativeDownloadFile = async function (meta, fullDstPath) {
    ...
    try {
      if (shouldPerformGCPBucket(accessToken)) {
        ...
        let response;
        await axiosInstance.get(downloadUrl, {
          headers: gcsHeaders,
          responseType: 'stream'
        }).then(async (res) => {
          response = res;
