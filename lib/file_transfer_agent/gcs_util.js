...
function GCSUtil(connectionConfig, httpClient) {
  ...
  /**
   * Read the file into a Buffer and upload it via a single PUT to the
   * presigned URL (or the access-token-authenticated equivalent).
   *
   * Why Buffer-bodied uploads, not a Readable stream:
   *  - Memory per upload is bounded to one file-size allocation; no extra
   *    streaming buffers compound on top.
   *  - The single-PUT wire shape (Buffer body + known `Content-Length`) is
   *    what GCS presigned URLs are actually signed for. True multi-part /
   *    resumable upload against GCS uses a different endpoint
   *    (`/upload/storage/v1/b/<bucket>/o?uploadType=resumable`) that
   *    Snowflake's stage-upload presigned URLs are not signed for today;
   *    chunked Buffer-bodied resumable uploads remain a follow-up that
   *    needs server-side cooperation to mint upload-session URLs.
   ...
   *    transmits zero bytes for `fs.ReadStream` after a few HTTPS calls in
   *    a long-running process, hanging stage uploads at the server-side
   *    idle timeout); Buffer bodies sidestep this entirely.
   *
   * `connectionConfig.uploadPartSizeMb` is consulted only as a soft
   * advisory upper bound for in-flight memory; uploads larger than the
   * configured part size still proceed via single PUT today, with a
   * runtime debug log so operators can correlate memory spikes back to
   * stage-upload sizes.
   *
  ...
  this.uploadFile = async function (dataFile, meta, encryptionMetadata, maxConcurrency) {
    ...
    if (fileSize > partSize) {
      Logger().debug(
        `GCS upload exceeds uploadPartSizeMb (file=${fileSize} bytes, partSize=${partSize} bytes). ` +
          `Single-PUT path engaged; in-flight memory will reach the file size. ` +
          `Chunked resumable upload requires a session URL that the server does not mint today.`,
      );
    }
