  ...
  */
  this.uploadFile = async function (dataFile, meta, encryptionMetadata, maxConcurrency)
  {
    ...
    Logger.getInstance().debug(
      "gcs uploadFile() uploading file: %s", dataFile);

    if (!uploadURL)
    {
      Logger.getInstance().debug(
		  "gcs uploadFile() uploading file using access token: %s", dataFile);
    ...
    Logger.getInstance().debug(
      "gcs uploadFile() contentEncoding: %s", contentEncoding);
    ...
    try
    {
      await axios.put(uploadURL, fileStream, { headers: gcsHeaders });
    }
    catch (err)
    {
      Logger.getInstance().debug(
        "gcs uploadFile() error occus: %s", err['code']);
	  throw err;
