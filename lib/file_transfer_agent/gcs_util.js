  ...
  this.nativeDownloadFile = async function (meta, fullDstPath) {
    ...
    try {
      if (this.shouldUseJsonApi(meta)) {
        ...
        Logger().info(`Use GCP client to download the file.`);
        Logger().info(`Send Get Request to the Bucket: ${bucketName}, GET request: ${path}`);
        await meta['client'].gcsClient
          .bucket(bucketName)
        ...
        Logger().info(`Downloading file from GCS using Axios`);
        let response;
        await axios
          .get(downloadUrl, {
            headers: gcsHeaders,
    ...
    if (typeof httpClient === 'undefined') {
      ...
      if (proxy || Util.getEnvVar('http_proxy')) {
        ...
        const proxyAgent = getProxyAgent(
          proxy,
          new URL(connectionConfig.accessUrl),
