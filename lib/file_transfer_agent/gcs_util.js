...
function GCSUtil(connectionConfig, httpClient) {
  ...
  this.getFileHeader = async function (meta, filename) {
    if (meta['resultStatus'] === resultStatus.UPLOADED ||
      ...
      if (meta['presignedUrl']) {
        ...
        try {
          const customHeaders = this.applyCustomHeaders('HEAD', url);
  ...
  */
  ...
  this.uploadFileStream = async function (fileStream, meta, encryptionMetadata) {
    ...
    try {
      const customHeaders = this.applyCustomHeaders('PUT', uploadUrl);
      if (shouldPerformGCPBucket(accessToken) && !isProxyEnabled && Object.keys(customHeaders).length === 0) {
        const gcsLocation = this.extractBucketNameAndPath(meta['stageInfo']['location']);
  ...
  this.nativeDownloadFile = async function (meta, fullDstPath) {
    ...
    try {
      const customHeaders = this.applyCustomHeaders('GET', downloadUrl);
      if (shouldPerformGCPBucket(accessToken) && !isProxyEnabled && Object.keys(customHeaders).length === 0) {
        const gcsLocation = this.extractBucketNameAndPath(meta['stageInfo']['location']);
        await meta['client'].gcsClient
          .bucket(gcsLocation.bucketName)
  ...
  */
  ...
  this.applyCustomHeaders = function (method, endPoint) {
    const customHeaders = {};
    if (connectionConfig.getHTTPHeadersCustomizers().length > 0) {
      // add custom HTTP headers
      for (const customizer of connectionConfig.getHTTPHeadersCustomizers()) {
