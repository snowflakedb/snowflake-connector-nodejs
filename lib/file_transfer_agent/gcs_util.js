...
function GCSUtil(connectionConfig, httpClient) {
   ...
   *    transmits zero bytes for `fs.ReadStream` after a few HTTPS calls
   *    in a long-running process, hanging stage uploads at the GCS
   *    server-side idle timeout). Buffer bodies sidestep this entirely.
   ...
   *
   * Uses the **XML API** resumable shape, not the JSON API:
   *   - Initiation:  `POST <bucket>/<object>` with `x-goog-resumable: start`
   *   - Chunks:      `PUT <session_url>` with `Content-Range: bytes a-b/total`
   *   - Abort:       `DELETE <session_url>`
   * The XML API initiation reaches the same bucket-path URL the single-PUT
   * path uses (just with method=POST and the resumable header), which means
   * the same network allowlists that already permit single PUTs to the
   * stage bucket also permit resumable initiation. The JSON API path
   * (`/upload/storage/v1/b/<bucket>/o`) gets shape-blocked by some
   * outbound proxies (e.g. the CNG hostagent's CSP allowlist as of the
   * SYSTEM$ALLOWLIST() population today) — staying on XML sidesteps that.
   *
  ...
  this.uploadFileResumable = async function (
    ...
    // Use the same canonical bucket-path URL the single-PUT path uses.
    // This keeps proxy allowlist matchers (which key on the bucket-path
    // shape) happy: same URL, same auth, just different method/headers
    // for the resumable initiation.
