...
function createFileHeaderError(meta, reason) {
  const storageType = meta['stageInfo']['locationType'] || 'STORAGE';
  const err = meta['lastError'];
  const rawCode = (err && (err['Code'] || err['code'] || err['name'])) || reason || 'UnknownError';
  const rawHttpStatus =
  ...
  return new Error(
    `Failed to get file header (${sanitize(storageType)}:${sanitize(rawCode)}:${sanitize(rawHttpStatus)}:${operation})`,
  );
}

function validateFileHeader(meta, fileHeader, requireEncryptionMetadata) {
  if (fileHeader && (!requireEncryptionMetadata || fileHeader.encryptionMetadata)) {
    return;
  }

  const reason = fileHeader ? 'MissingEncryptionMetadata' : 'EmptyResponse';
  if (
  ...
  throw createFileHeaderError(meta, reason);
}
...
function RemoteStorageUtil(connectionConfig) {
  ...
  this.downloadOneFile = async function (meta) {
    ...
    Logger().debug(
      `Started downloading result. File: ${meta.dstFileName}, location: ${meta.stageInfo.location}`,
    );
    ...
    for (let retry = 0; retry < maxRetry; retry++) {
      ...
      try {
        ...
        validateFileHeader(
          meta,
          fileHeader,
          Boolean(meta['encryptionMaterial'] && !meta['presignedUrl']),
        );
        ...
        if (meta['resultStatus'] === resultStatus.DOWNLOADED) {
          ...
          timer.stop();
          Logger().debug(
            `Successfully downloaded result. File: ${meta.dstFileName}, download time: ${timer.getDuration()} ms, chunkSize: ${meta.srcFileSize} bytes.`,
          );
          if (meta['encryptionMaterial']) {
            /**
             * For storage utils that do not have the privilege of getting the
             * metadata early, both object and metadata are downloaded at once.
             * Fetch the populated header before decrypting.
             */
            if (meta['presignedUrl']) {
              meta['lastError'] = null;
              fileHeader = await utilClass.getFileHeader(meta, meta['srcFilePath']);
              validateFileHeader(meta, fileHeader, true);
            }

            Logger().debug(
              `Start decrypting the downloaded file for ${meta.dstFileName} at path: ${fullDstPath}.`,
            );
            timer.start();
            ...
            timer.stop();
            Logger().debug(
              `Successfully decrypted the downloaded file for ${meta.dstFileName} in ${timer.getDuration()} ms.`,
            );
          ...
          await new Promise((resolve) => {
            fs.stat(fullDstPath, (err, stat) => {
              ...
              Logger().debug(
                `File size verification: Downloaded file at "${meta.fullDstPath}" is ${stat.size} bytes.`,
              );
              resolve();
            });
          });
        ...
        if (downloadCompleted) {
          try {
            await fs.promises.rm(fullDstPath, { force: true });
          } catch (_cleanupErr) {
            // Preserve the original header/decryption error.
          }
        }
      }

      if (meta['resultStatus'] === resultStatus.RENEW_TOKEN) {
        ...
        Logger().debug(
          `Need to retry with lower concurrency for downloading file: ${meta.dstFileName}`,
        );
        lastErr = attemptErr || meta['lastError'] || lastErr;
