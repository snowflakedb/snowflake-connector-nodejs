...
function remote_storage_util()
{
  ...
  */
  this.downloadOneFile = async function (meta)
  {
    ...
    var fullDstPath = path.join(meta['localLocation'], path.basename(meta['dstFileName']));
    fullDstPath = fs.realpathSync(fullDstPath);
    ...
    var utilClass = getForStorageType(meta['stageInfo']['locationType']);
    var fileHeader = await utilClass.getFileHeader(meta, meta['srcFilePath']);
    ...
    for (var retry = 0; retry < maxRetry; retry++)
    {
      utilClass.nativeDownloadFile(meta, fullDstPath, maxConcurrency);
      ...
      if (meta['result_status'] == ResultStatus.DOWNLOADED)
      {
        if (meta['encryption_material'])
        {
          ...
          var tmpDstName = SnowflakeEncryptionUtil.decryptFile(
          ...
          fs.copyFileSync(tmpDstName, fullDstPath);
        }
        else
        {
          var fileInfo = fs.statSync(fullDstPath);
          meta['dstFileSize'] = fileInfo.size;
          return
        }
      }
      else if (meta['resultStatus'] == resultStatus.RENEW_TOKEN)
