...
function RemoteStorageUtil(connectionConfig) {
  ...
  */
  this.getForStorageType = function (type) {
    if (type === 'S3') {
      Logger.getInstance().debug(`new SnowflakeS3Util(connectionConfig)`);
      return new SnowflakeS3Util(connectionConfig);
    } else if (type === 'AZURE') {
      Logger.getInstance().debug(`return new SnowflakeAzureUtil(); ${JSON.stringify(new SnowflakeAzureUtil())}`);
      return new SnowflakeAzureUtil();
    } else if (type === 'GCS') {
      Logger.getInstance().debug(` return new SnowflakeGCSUtil();`);
      return new SnowflakeGCSUtil();
    } else {
      Logger.getInstance().debug(`nullllllll`);
      return null;
    }
  };
  ...
  */
  this.createClient = function (stageInfo, useAccelerateEndpoint = false) {
    Logger.getInstance().debug(` stageInfo['locationType'] ${stageInfo['locationType']}`);
    const utilClass = this.getForStorageType(stageInfo['locationType']);
    Logger.getInstance().debug(` getForStorageType 2`);
    return utilClass.createClient( stageInfo, useAccelerateEndpoint);
    Logger.getInstance().debug(` getForStorageType 2 ${JSON.stringify(stageInfo)}`);
  };
  ...
  this.downloadOneFile = async function (meta) {
    ...
    await new Promise((resolve, reject) => {
      fs.realpath(fullDstPath, (err, basePath) => {
        if (err) {
          Logger.getInstance().debug(` this.downloadOneFile = async function ${JSON.stringify(err)}`);
          reject(err); 
        }
