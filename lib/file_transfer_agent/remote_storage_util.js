...
function RemoteStorageUtil(connectionConfig) {
  ...
  this.uploadOneFileStream = async function (meta) {
    ...
    // `lastErr` is only set for the NEED_RETRY statuses; a storage client that reported
    // resultStatus.ERROR leaves the cause in meta.lastError instead.
    const uploadError = lastErr ?? meta['lastError'];
    if (uploadError) {
