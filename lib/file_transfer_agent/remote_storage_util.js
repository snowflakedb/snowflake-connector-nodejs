...
function remote_storage_util()
{
  ...
  */
  ...
  this.uploadOneFileStream = async function (meta) {
    ...
    var dataFileStream = meta['fileStream'];
    
    if (meta['encryptionMaterial']) {
      var result = await SnowflakeEncryptionUtil.encryptFileStream(
    ...
    for (var retry = 0; retry < maxRetry; retry++) {
      ...
      {
        console.log("resultStatus.NEED_RETRY: " + meta['lastError']);
      ...
      {
        console.log("resultStatus.NEED_RETRY_WITH_LOWER_CONCURRENCY: " + meta['lastError']);
