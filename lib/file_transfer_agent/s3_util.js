...
function S3Util(connectionConfig, s3, filestream) {
   ...
   *  - The upload wire shape (Buffer body with known length) sidesteps
   *    `STREAMING-UNSIGNED-PAYLOAD-TRAILER` mode that some HTTP middleboxes
   *    drop; checksums emit as a regular header instead of a trailer.
   ...
   *    transmits zero bytes for `fs.ReadStream`); Buffer bodies are not
   *    affected.
   *
