...
const RETRYABLE_HEADER_ERROR_CODES = new Set([
  'InternalError',
  'NetworkingError',
  'RequestTimeout',
  ...
  this.getFileHeader = async function (meta, filename) {
    ...
    try {
      ...
      try {
        ...
        const httpStatusCode = err && err['$metadata'] && err['$metadata']['httpStatusCode'];

        if (code === EXPIRED_TOKEN) {
          meta['resultStatus'] = resultStatus.RENEW_TOKEN;
          return null;
        } else if (code === NOT_FOUND || httpStatusCode === 404) {
          meta['resultStatus'] = resultStatus.NOT_FOUND_FILE;
          return FileHeader(null, null, null);
        } else if (code === '400' || httpStatusCode === 400) {
          meta['resultStatus'] = resultStatus.RENEW_TOKEN;
          return null;
        } else if (
          !code ||
          code === 'Error' ||
          RETRYABLE_HEADER_ERROR_CODES.has(code) ||
