...
function S3Util(connectionConfig, s3) {
  ...
  this.createClient = function (stageInfo, useAccelerateEndpoint) {
    ...
    if (proxy) {
      ...
      // The proxy agent is shared across all S3 clients created by this S3Util
      // instance (via httpsAgentCache). Calling client.destroy() propagates to
      // NodeHttpHandler.destroy() → agent.destroy(), which closes ALL keepAlive
      // sockets in the shared pool — killing concurrent requests that are still
      // in flight. Override destroy to a no-op so client.destroy() in each
      // method's finally block only cleans up the handler's own state without
      // destroying the shared connection pool.
  ...
  this.getFileHeader = async function (meta, filename) {
    ...
    try {
      ...
      try {
        await client.headObject(params).then(function (data) {
          akey = data;
        });
      } catch (err) {
        if (err['Code'] === EXPIRED_TOKEN) {
          ...
          // Throw instead of returning null. Returning null here causes a crash
          // downstream: if nativeDownloadFile later succeeds for the same file,
          // remote_storage_util.downloadOneFile enters the DOWNLOADED branch and
          // calls decryptFile(fileHeader.encryptionMetadata) — but fileHeader is
          // null, producing "TypeError: Cannot read properties of null (reading
          // 'encryptionMetadata')". Throwing lets the caller surface the real
          // error (e.g. a TLS handshake rejection under an HTTPS proxy) instead
          // of a confusing null-dereference crash.
          meta['lastError'] = err;
