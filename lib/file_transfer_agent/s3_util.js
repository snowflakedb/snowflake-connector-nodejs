...
function S3Util(connectionConfig, s3, filestream) {
   ...
   * multipart (large files), depending on `uploadPartSizeMb`. Buffer-bodied
   * uploads bound in-flight memory by `uploadPartSizeMb` regardless of file
   * size, give per-part retry granularity for multipart, and align with the
   * pattern already used in the Python and Java connectors.
   *
