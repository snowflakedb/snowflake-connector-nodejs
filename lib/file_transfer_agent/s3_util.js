...
function S3Util(connectionConfig, s3, filestream) {
   ...
   * multipart (large files), depending on `MULTIPART_THRESHOLD_BYTES`.
   * Buffer-bodied uploads bound in-flight memory by `MULTIPART_PART_SIZE_BYTES`
   * regardless of file size, give per-part retry granularity for multipart,
  ...
  this.uploadFile = async function (dataFile, meta, encryptionMetadata) {
    ...
    if (fileSize <= MULTIPART_THRESHOLD_BYTES) {
      // Single-PutObject path: file is below the multipart threshold.
    ...
    try {
      ...
      try {
        ...
        while (position < fileSize) {
          ...
          const thisPartSize = Math.min(MULTIPART_PART_SIZE_BYTES, remaining);
 ...
 * convention. Used by `uploadFileMultipart` from each of the create / part /
 * complete catch sites so retry semantics stay consistent across them.
 */
