...
function S3Util(connectionConfig, s3, filestream) {
  ...
  this.getFileHeader = async function (meta, filename) {
    ...
    if (!meta.isRetry) {
      this.addCustomHeaders(params, client, 'GET');
    }
    let akey;
  ...
  this.uploadFileStream = async function (fileStream, meta, encryptionMetadata) {
    ...
    if (meta.isRetry) {
      this.addCustomHeaders(params, client, 'PUT');
    }
  ...
  this.addCustomHeaders = function (params, client, method) {
    const customHeaders = RequestUtil.getCustomHeaders(connectionConfig, method, client.config.endPoint ?? SNOWFLAKE_S3_DESTINATION);
