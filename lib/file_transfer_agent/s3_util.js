...
function S3Util(connectionConfig, s3, filestream) {
  let AWS;
  try {
    AWS = typeof s3 !== 'undefined' ? s3 : require('@aws-sdk/client-s3');
  } catch (err) {
    Logger.default().info('AWS SDK is not installed, skipping AWS features.');
  }
  ...
  this.createClient = function (stageInfo, useAccelerateEndpoint) {
    ...
    if (proxy) {
      const proxyAgent = getProxyAgent(
        ...
        endPoint || SNOWFLAKE_S3_DESTINATION,
      );
