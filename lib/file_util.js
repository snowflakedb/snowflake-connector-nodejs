...
exports.getSecureHandle = async function (filePath, flags, fsPromises) {
  const fsp = fsPromises ? fsPromises : require('fs/promises');
  try {
    //const options = process.platform !== 'win32' ? 0o600 : undefined;
    const openTime = new Date().getTime();
    let failed;
    let fileHandle;
    try {
      fileHandle = await fsp.open(filePath, flags, 0o600);
    } catch (err) {
      failed = err;
    }
    const helpStats = await fsp.stat(filePath);
    Logger.getInstance().info(`Attempted to open at ${openTime}, file was created at ${helpStats.birthtimeMs}`);
