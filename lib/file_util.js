...
exports.validateNoExtraPermissionsForOthers = function (
  ...
  const fsp = fsPromises ? fsPromises : fs.promises;
  ...
  const validatePermissions = (stats) => {
    ...
    if ((permission & othersCanReadFilePermission) !== 0) {
      Logger().warn(
        `file ${filePath} is readable by someone other than the owner. Your Permission: ${permission.toString(8)}. Either remove read permissions from group and others or disable the permission check via ${SKIP_TOKEN_FILE_PERMISSIONS_VERIFICATION_ENV_VAR} env variable`,
      );
    }
...
exports.IsFileExisted = async function (filePath, filesystem = fsPromises) {
  try {
    await filesystem.access(filePath, fs.constants.F_OK);
    return true;
  } catch (err) {
    return false;
  }
