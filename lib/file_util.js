...
const { isWindows, exists } = require('./util');
 ...
 * @param filePath
 * @param expectedMode
 * @param fileStreamModule
 * @returns {Promise<boolean>} resolves always to true for Windows
 */
exports.validateOnlyUserReadWritePermissionAndOwner = async function (filePath, fileStreamModule) {
  const fsp = typeof fileStreamModule !== 'undefined' ? fileStreamModule : fsPromises;
  if (isWindows()) {
    return;
  }
  try {
...
async function removeStale (file) {
  ...
  try {
    ...
    if (new Date().getTime() - stat.birthtimeMs > timeoutMs) {
      await fsPromises.rmdir(file).catch((err) => {
        Logger.getInstance().warn(`Failed to remove stale file. Error: ${err.message}`);
      });
    }
  } catch (err) {
    return undefined;
...
exports.lockFile = async function (fileName) {
  const retryInterval = 100;
  const lckFile = fileName + '.lck';
  await removeStale(lckFile);
  let attempts = 0;
  let locked = false;
  const options = !isWindows() ? { mode: filePermission.OWNER_RW_ONLY } : {};

  while (attempts < 10) {
    Logger.getInstance().debug(`Attempting to get a lock on file ${fileName}, attempt: ${attempts}`);
    attempts++;
    try {
      await fsPromises.mkdir(lckFile, options);
      locked = true;
    } catch (err) {
      Logger.getInstance().error(`Failed to lock on file ${fileName}, attempt: ${attempts}, error: ${err.message}`);
    }
...
exports.unlockFile = async function (fileName) {
  const lckFile = fileName + '.lck';
  await fsPromises.rmdir(lckFile);
};
...
exports.IsFileOwnedByAdmin = async function (filePath) {
  if (isWindows()) {
    return true;
  }  
  ...
  try {
    ...
    Logger.getInstance().error(`Could not validate the ownership of the file: ${filePath}`);
    return false;
  }
};