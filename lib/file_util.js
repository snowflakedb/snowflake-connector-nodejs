...
exports.validateOnlyUserReadWritePermissionAndOwner = async function (filePath, fsPromises) {
  ...
  try {
    ...
    if (octalPermissions === '600') {
      ...
      throw new Error(
        `Invalid file permissions (${octalPermissions} for file ${filePath}). Make sure you have read and write permissions and other users do not have access to it. Please remove the file and re-run the driver.`,
      );
    }
