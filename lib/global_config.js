...
let credentialManager = createJsonFileCredentialManager();

exports.setCustomCredentialManager = function (customCredentialManager) {
  Errors.checkArgumentValid(Util.checkValidCustomCrednetialManager(customCredentialManager),
  ErrorCodes.ERR_GLOBAL_CONFIGURE_INVALID_CUSTOM_CREDENTIAL_MANAGER);
  
  credentialManager =  new function () {  
    this.read = async function (key) {  
      return await customCredentialManager.read(key);  
    };  
    this.write = async function (key, credential) {
      await customCredentialManager.write(key, credential);
    };  
    this.remove = async function (key) {
      await customCredentialManager.remove(key);
    };  
  };  
};
...
function createJsonFileCredentialManager() {
  return require('./authentication/SecureStorage/credentialManager');
} 
