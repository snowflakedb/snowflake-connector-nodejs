...
HttpClient.prototype.request = function (options)
{
  ...
  var sendRequest = async function sendRequest()
  {
    ...
    requestOptions.httpsAgent = requestOptions.agentClass();
    requestOptions.httpsAgent.keepAlive = requestOptions.agentOptions.keepAlive;

    delete requestOptions.body;
    delete requestOptions.agentClass;
    delete requestOptions.agentOptions;

    try
    {
      request = await require('axios').request(requestOptions)
        .then(async function (response)
        {
          // Retrieved from: https://stackoverflow.com/a/49428486
          ...
          var body = await streamToString(response.data)
          ...
          {
            await options.callback(null, normalizeResponse(response.data), body);
          }
        });
    }
    catch (err)
