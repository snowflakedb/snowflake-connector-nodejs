...
HttpClient.prototype.request = function (options)
{
  ...
  var sendRequest = async function sendRequest()
  {
    ...
    let cachedAgent = this._connectionConfig.getCachedAgent(url);
    if (!cachedAgent) {
      // add the agent and proxy options (only one per connection origin)
      const agentAndProxyOptions = this.getAgentAndProxyOptions(
        url,
        this._connectionConfig.getSocketKeepAlive(),
        this._connectionConfig.getProxy(),
        mock
      );
      cachedAgent = agentAndProxyOptions.agentClass(agentAndProxyOptions.agentOptions);
      this._connectionConfig.setCachedAgent(url, cachedAgent);
    }
    requestOptions.httpsAgent = cachedAgent;
 ...
 * @returns {*}
 */
HttpClient.prototype.getAgentAndProxyOptions = function (url, keepAlive, proxy, mock)
{
  return null;
};
