...
HttpClient.prototype.request = function (options) {
  Logger.getInstance().trace('Request%s - preparing for sending.', describeRequestFromOptions(options));

  let requestPromise;
  const requestOptions = prepareRequestOptions.call(this, options);
  let sendRequest = async function sendRequest() {
    Logger.getInstance().trace('Request%s - sending.', describeRequestFromOptions(requestOptions));
    ...
    requestPromise = axios.request(requestOptions).then(response => {
      ...
      Logger.getInstance().debug('Request%s - response received after %s milliseconds with status %s.', describeRequestFromOptions(requestOptions), httpResponseTime, response.status);
      // TODO: below line should be added here for parity with async method, but it will be a BCR, since previously JSON data was not parsed
      // parseResponseData(response);
      sanitizeAxiosResponse(response);
      if (Util.isFunction(options.callback)) {
        Logger.getInstance().trace('Request%s - calling callback function.', describeRequestFromOptions(requestOptions));
        return options.callback(null, normalizeResponse(response), response.data);
      } else {
        Logger.getInstance().trace('Request%s - callback function was not provided.', describeRequestFromOptions(requestOptions));
        return null;
      }
    }).catch(err => {
      const httpResponseTime = timer.getDuration();
      Logger.getInstance().debug('Request%s - failed after %s milliseconds.', describeRequestFromOptions(requestOptions), httpResponseTime);
      sanitizeAxiosError(err);
      if (Util.isFunction(options.callback)) {
        if (err.response) { // axios returns error for not 2xx responses - let's unwrap it
          Logger.getInstance().trace('Request%s - calling callback function for error from response. Received code: ', describeRequestFromOptions(requestOptions), err.response.status);
          options.callback(null, normalizeResponse(err.response), err.response.data);
        } else {
          Logger.getInstance().trace('Request%s - calling callback function for error without response.', describeRequestFromOptions(requestOptions));
          options.callback(err, normalizeResponse(null), null);
        }
        return null;
      } else {
        Logger.getInstance().warn('Request%s - callback function was not provided. Error will be re-raised.', describeRequestFromOptions(requestOptions));
        throw err;
      }
    });
  };
  ...
  Logger.getInstance().trace('Request%s - issued for the next tick.', describeRequestFromOptions(requestOptions));
  ...
  return {
    abort: function () {
      if (requestPromise) {
        Logger.getInstance().trace('Request%s - aborting.', describeRequestFromOptions(requestOptions));
...
HttpClient.prototype.requestAsync = async function (options) {
  Logger.getInstance().trace('Request%s - preparing for async sending.', describeRequestFromOptions(options));
  ...
  try {
    ...
    const httpResponseTime = timer.getDuration();
    Logger.getInstance().debug('Request%s - failed after %s milliseconds. Error will be re-raised.', describeRequestFromOptions(options), httpResponseTime);
    sanitizeAxiosError(err);
    throw err;
  }
};
...
function parseResponseData(response) {
  Logger.getInstance().trace('Request%s - parsing response data.', describeRequestFromResponse(response));
...
function sanitizeAxiosResponse(response) {
  Logger.getInstance().trace('Request%s - sanitizing response data.', describeRequestFromResponse(response));
...
function sanitizeAxiosError(error) {
  ...
  if (error.response) {
    Logger.getInstance().trace('Request%s - sanitizing response error data.', describeRequestFromResponse(error.response));
    sanitizeAxiosResponse(error.response);
  }
}

function prepareRequestOptions(options) {
  Logger.getInstance().trace('Request%s - constructing options.', describeRequestFromOptions(options));
  ...
  if (data) {
    ...
    zlib.gzip(bufferUncompressed, null, function (err, bufferCompressed) {
      ...
      if (!err) {
        ...
        Logger.getInstance().debug('Request%s - original buffer length: %d bytes. Compressed buffer length: %d bytes.', describeRequestFromOptions(options), bufferUncompressed.buffer.byteLength, bufferCompressed.buffer.byteLength);
      } else {
        ...
        Logger.getInstance().warn('Request%s - could not compress request data.', describeRequestFromOptions(options));
      }
    });
  }
  ...
  Logger.getInstance().trace('Request%s - options - timeout: %s, retryDelay: %s, responseType: %s', describeRequestFromOptions(options), requestOptions.timeout, requestOptions.retryDelay, requestOptions.responseType);
...
function normalizeResponse(response) {
  ...
  if (response && !response.getResponseHeader) {
    Logger.getInstance().trace('Request%s - normalizing.', describeRequestFromResponse(response));
 ...
 * @returns {string}
 */
function describeRequestFromOptions(requestOptions) {
  return describeRequestData({ method: requestOptions.method, url: requestOptions.url });
}
 ...
 * @returns {string}
 */
function describeRequestFromResponse(response) {
  ...
  return describeRequestData({ method: response.config?.method.toUpperCase(), url: response.config?.url });
}

function describeRequestData(requestData) {
