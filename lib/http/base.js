...
const ProxyAgent = require('urllib').ProxyAgent
...
HttpClient.prototype.request = function (options)
{
  ...
  var sendRequest = async function sendRequest()
  {
    ...
    var requestOptions =
      {
        ...
        timeout: timeout,
        requestOCSP: true,
        //TODO: SNOW-876346 urllib3 - it is still used -> maybe verify= flag should be used
        rejectUnauthorized: true,
        // to use burp as proxy
    ...
    requestOptions.dispatcher = agentAndProxyOptions.agentClass();
    requestOptions.retryDelay = this.constructExponentialBackoffStrategy();
    // TODO SNOW-876346 Previous httpCleint had keepAlive as a flag urllib.Agent contains property keepAliveTimeout
    requestOptions.keepAliveTimeout = 3600 * 1000; //1h

    if (Util.isFunction(options.callback)) {
      urllib.request(requestOptions.url, requestOptions)
        .then(async function ({ data, res }) {
          if (res.headers['x-goog-stored-content-encoding'] === 'GZIP') { // Google API sets own header and urllib cannot parse it automatically
            return new Promise((resolve, reject) =>
              zlib.gunzip(data, (err, decompressedData) => {
                if (err) {
                  reject(err);
                } else {
                  resolve({ data: decompressedData, res });
                }
              })
            );
          }
        ...
        .then(async function ({data, res}) {
          ...
          await options.callback(null, normalizeResponse(res), data.toString());
        })
