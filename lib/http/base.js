...
HttpClient.prototype.request = function (options) {
  ...
  let sendRequest = async function sendRequest() {
    // Logger.getInstance().trace(`requestOptions ${JSON.stringify(requestOptions)}`);
    request = axios.request(requestOptions).then(response => {
      // Logger.getInstance().trace(`response 1 ${JSON.stringify(response)}`);
      if (Util.isFunction(options.callback)) {
        Logger.getInstance().trace('RESPONSE 1 ');
        Logger.getInstance().trace(`response 1 ${JSON.stringify(response.data)}`);
      ...
      Logger.getInstance().trace(`err ${err}`);
      Logger.getInstance().trace(`err response ${JSON.stringify(err.response)}`);
      if (Util.isFunction(options.callback)) {
