...
function prepareRequestOptions(options, requestHandlers = {}) {
  ...
  const customizers = this._connectionConfig.getHTTPHeadersCustomizers();
  if (customizers.length !== 0 && options.isRetry) {
    Logger.getInstance().debug('Setting up http headers customizers');
    const customHeaders = [];
    for (const customizer of customizers) {
      if (customizer.applies(options.method, options.url)) {
        const newHeaders = customizer.newHeaders();
        requestUtil.isTryingToOverrideDriverHeader(headers, newHeaders);
        customHeaders.push(...newHeaders);
      }
    }
