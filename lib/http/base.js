...
function HttpClient(connectionConfig) {
  ...
  Logger().trace(
    'Initializing base HttpClient with Connection Config[%s]',
...
HttpClient.prototype.request = function (options) {
  Logger().trace(
    'Request%s - preparing for sending.',
    requestUtil.describeRequestFromOptions(options),
  );
  ...
  let sendRequest = async function sendRequest() {
    Logger().trace('Request%s - sending.', requestUtil.describeRequestFromOptions(requestOptions));
    ...
    requestObject.requestPromise = axios
      ...
      .then((response) => {
        ...
        Logger().debug(
          'Request%s - response received after %s milliseconds with status %s.',
        ...
        if (Util.isFunction(options.callback)) {
          Logger().trace(
            'Request%s - calling callback function.',
            requestUtil.describeRequestFromOptions(requestOptions),
          );
          return options.callback(null, normalizeResponse(response), response.data);
        } else {
          Logger().trace(
            'Request%s - callback function was not provided.',
            requestUtil.describeRequestFromOptions(requestOptions),
          );
          return null;
        }
      ...
      .catch((err) => {
        const httpResponseTime = timer.getDuration();
        Logger().debug(
          'Request%s - failed after %s milliseconds.',
          requestUtil.describeRequestFromOptions(requestOptions),
          httpResponseTime,
        );
        ...
        if (Util.isFunction(options.callback)) {
          ...
          Logger().warn(
            'Request%s - callback function was not provided. Error will be re-raised.',
            requestUtil.describeRequestFromOptions(requestOptions),
          );
          throw err;
        }
  ...
  Logger().trace(
    'Request%s - issued for the next tick.',
    requestUtil.describeRequestFromOptions(requestOptions),
  );
...
HttpClient.prototype.requestAsync = async function (options) {
  Logger().trace(
    'Request%s - preparing for async sending.',
    requestUtil.describeRequestFromOptions(options),
  );
  ...
  try {
    ...
    const httpResponseTime = timer.getDuration();
    Logger().debug(
      'Request%s - failed after %s milliseconds. Error will be re-raised.',
      requestUtil.describeRequestFromOptions(options),
      httpResponseTime,
    );
    sanitizeAxiosError(err);
...
function parseResponseData(response) {
  Logger().trace(
    'Request%s - parsing response data.',
    requestUtil.describeRequestFromResponse(response),
  );
...
function sanitizeAxiosResponse(response) {
  Logger().trace(
    'Request%s - sanitizing response data.',
    requestUtil.describeRequestFromResponse(response),
  );
  const requestInfo = {
...
function sanitizeAxiosError(error) {
  ...
  if (error.response) {
    Logger().trace(
      'Request%s - sanitizing response error data.',
      requestUtil.describeRequestFromResponse(error.response),
    );
...
function prepareRequestOptions(options, requestHandlers = {}) {
  Logger().trace(
    'Request%s - constructing options.',
    requestUtil.describeRequestFromOptions(options),
  );
  ...
  if (data) {
    ...
    zlib.gzip(bufferUncompressed, null, function (err, bufferCompressed) {
      ...
      if (!err) {
        ...
        Logger().debug(
          'Request%s - original buffer length: %d bytes. Compressed buffer length: %d bytes.',
          requestUtil.describeRequestFromOptions(options),
          bufferUncompressed.buffer.byteLength,
        ...
        Logger().warn(
          'Request%s - could not compress request data.',
          requestUtil.describeRequestFromOptions(options),
        );
      }
    });
  ...
  Logger().debug(
    'Request%s - options - timeout: %s, retryDelay: %s, responseType: %s',
    requestUtil.describeRequestFromOptions(options),
...
function normalizeHeaders(headers) {
  Logger().trace('Normalizing headers');
  if (Util.isObject(headers)) {
    ...
    Logger().trace('Headers were normalized');
    return normalizedHeaders;
  } else {
    Logger().trace('Headers were not an object. Original value will be returned.');
    return headers;
  }
}
...
function normalizeResponse(response) {
  ...
  if (response && !response.getResponseHeader) {
    Logger().trace('Request%s - normalizing.', requestUtil.describeRequestFromResponse(response));
    response.getResponseHeader = function (header) {
      return (
        response.headers && response.headers[Util.isString(header) ? header.toLowerCase() : header]
      );
    };
  }
