...
HttpClient.prototype.request = function (options)
{
  ...
  var sendRequest = async function sendRequest()
  {
    ...
    //TODO: Cacth error code: 'UND_ERR_CONNECT_TIMEOUT'
    //and retry if connection timeout.
    response = await request(requestOptions.url, requestOptions);
    
    if (Util.isFunction(options.callback))
    {
      response.body = response.data.toString();
      await options.callback(null, normalizeResponse(response), response.data.toString());
    }
  };
