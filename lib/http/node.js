...
function getProxyAgent(proxyOptions, parsedUrl, destination, mock) {
  ...
  const agentId = createAgentId(agentOptions.protocol, agentOptions.port, agentOptions.hostname, agentOptions.keepAlive);
...
function createAgentId(protocol, port, hostname, keepAlive) {
  return `${protocol}//${hostname}:${port}-${keepAlive ? 'keepAlive' : 'noKeepAlive'}`;
}
