...
const HttpsCrlAgent = require('../agent/https_crl_agent');
...
NodeHttpClient.prototype.constructExponentialBackoffStrategy = function () {
  ...
  Logger.getInstance().trace(
    'Calculated exponential backoff strategy sleep time: %d',
    nextSleepTimeInMilliseconds,
  );
...
function NodeHttpClient(connectionConfig) {
  Logger.getInstance().trace(
    'Initializing NodeHttpClient with Connection Config[%s]',
...
function getFromCacheOrCreate(AgentClass, options, agentId) {
  ...
  function createAgent(AgentClass, agentOptions, agentId) {
    ...
    Logger.getInstance().debug(
      'Agent[id: %s] - proxy settings used in requests: %s',
      agentId,
...
function isBypassProxy(proxy, destination, agentId) {
  ...
  if (matches) {
    Logger.getInstance().debug(
      'Agent[id: %s] - bypassing proxy allowed for destination: %s',
      agentId,
      destination,
    );
    return true;
...
NodeHttpClient.prototype.getAgent = function (parsedUrl, proxy, mock) {
  Logger.getInstance().trace(
    'Agent[url: %s] - getting an agent instance.',
    RequestUtil.describeURL(parsedUrl.href),
  );
  if (!proxy && GlobalConfig.isEnvProxyActive()) {
    ...
    if (proxy) {
      Logger.getInstance().debug(
        'Agent[url: %s] - proxy info loaded from the environment variable. Proxy host: %s',
        RequestUtil.describeURL(parsedUrl.href),
        proxy.host,
      );
    }
...
function getProxyAgent(proxyOptions, parsedUrl, destination, mock, connectionConfig) {
  Logger.getInstance().trace(
    'Agent[url: %s] - getting a proxy agent instance.',
    RequestUtil.describeURL(parsedUrl.href),
  );
  const agentOptions = {
  ...
  if (mock) {
    ...
    if (mockAgent.protocol === parsedUrl.protocol) {
      Logger.getInstance().debug(
        'Agent[url: %s] - the mock agent will be used.',
        RequestUtil.describeURL(parsedUrl.href),
      );
      return mockAgent;
    }
  ...
  if (isHttps) {
    agentOptions.crlConfig = connectionConfig.crlConfig;
    if (proxyOptions && !bypassProxy) {
      Logger.getInstance().trace(
        'Agent[id: %s] - using HTTPS agent enriched with proxy options.',
        agentId,
      );
      ...
      const AgentClass = isCrlValidationEnabled(connectionConfig.crlConfig) ? HttpsCrlAgent : HttpsOcspAgent;
    ...
    Logger.getInstance().trace(
      'Agent[id: %s] - using HTTP agent enriched with proxy options.',
      agentId,
    );
    enrichAgentOptionsWithProxyConfig(agentOptions, proxyOptions);
