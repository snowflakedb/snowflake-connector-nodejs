...
exports.getOriginalHeader = function () {
  return originalHeader;
};

exports.setOriginalHeader = function (header) {
  originalHeader = { ... header };
};

exports.cleanOverridingHeaders = function (originalHeader, newHeader) {
  const originalHeaderKeys = Object.keys(originalHeader);
  const newHeaderKeys = Object.keys(newHeader);

  if (originalHeaderKeys.length === 0 || newHeaderKeys.length === 0) {
    return;
  }

  for (const headerKey of newHeaderKeys) {
    if (originalHeaderKeys.find((key) => key.toLowerCase() === headerKey.toLowerCase())) {
      Logger.getInstance().debug(`Customizer attempted to override existing driver header ${originalHeaderKeys} which is not allowed. Skipping.`);
      delete newHeader[headerKey];
    }
  }
};

exports.isValidHTTPHeadersCustomizer = function (customizers) {
  const requireMethods = ['applies', 'newHeaders'];
...
exports.getCustomHeaders = function (connectionConfig, method, endPoint) {
  const customHeaders = {};

  if (connectionConfig.getHttpHeadersCustomizer().length > 0) {
    for (const customizer of connectionConfig.getHttpHeadersCustomizer()) {
      if (customizer.applies(method, endPoint)) {
        const headers = customizer.newHeaders();
