...
exports.init = async function (configFilePathFromConnectionString) {
  try {
    ...
    logger.info(
      'Initializing Easy Logging with logPath=%s and logLevel=%s from file: %s',
      logPath,
      config.loggingConfig.logLevel,
      config.configPath,
    );
    logger.configure({
      level: logLevel,
...
function allowedToInitialize(configFilePathFromConnectionString) {
  ...
  const triedToInitializeWithoutConfigFile =
    everTriedToInitialize && initTrialParameters.configFilePathFromConnectionString == null;
  const isGivenConfigFilePath = !!configFilePathFromConnectionString;
  const isAllowedToInitialize =
    !everTriedToInitialize || (triedToInitializeWithoutConfigFile && isGivenConfigFilePath);
  if (
    !isAllowedToInitialize &&
    initTrialParameters.configFilePathFromConnectionString !== configFilePathFromConnectionString
  ) {
    Logger.getInstance().warn(
      `Easy logging will not be configured for CLIENT_CONFIG_FILE=${configFilePathFromConnectionString} because it was previously configured for a different client config`,
    );
  }
...
async function getLogPath(config) {
  ...
  if (!logPath) {
    Logger.getInstance().warn(
      'LogPath in client config not found. Using home directory as a default value',
    );
    logPath = os.homedir();
    if (!exists(logPath)) {
  ...
  await fsPromises
    ...
    .then(async () => {
      if (!(await isFileModeCorrect(pathWithNodeJsSubdirectory, 0o700, fsPromises))) {
        Logger.getInstance().warn(
          'Log directory: %s could potentially be accessed by others',
          pathWithNodeJsSubdirectory,
        );
      }
    })
    .catch(async () => {
      try {
        await fsPromises.mkdir(pathWithNodeJsSubdirectory, {
          recursive: true,
          mode: 0o700,
        });
      } catch (err) {
        throw new EasyLoggingError(
          `Failed to create the directory for logs: ${pathWithNodeJsSubdirectory}`,
        );
      }
    });
