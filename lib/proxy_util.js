...
exports.convertProxyToString = function (proxy) {
  const connectionString = `${proxy.user}:${proxy.password}@${proxy.host}:${proxy.port}`;
  return (proxy.protocol).startsWith('https') ? `https://${connectionString}` : `http://${connectionString}`;
};