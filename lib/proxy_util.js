...
exports.getCompareAndLogEnvAndAgentProxies = function (agentOptions) {
  ...
  if (agentOptions.host) {
    ...
    const proxyString =
      `${Util.exists(agentOptions.user) ? `${agentOptions.user}:${agentOptions.password}@` : ''}${proxyHostAndPort}`.toLowerCase();
    logMessages.messages =
      logMessages.messages +
...
exports.getProxyFromEnv = function (isHttps = true) {
  ...
  Logger.getInstance().debug(
    `Util.getProxyEnv: Using ${protocol.toUpperCase()}_PROXY from the environment variable`,
  );
  if (proxyFromEnv.indexOf('://') === -1) {
    Logger.getInstance().info(
      'Util.getProxyEnv: the protocol was missing from the environment proxy. Use the HTTP protocol.',
    );
...
exports.getHostFromURL = function (destination) {
  ...
  try {
    ...
    Logger.getInstance().error(
      `Failed to parse the destination to URL with the error: ${err}. Return destination as the host: ${destination}`,
    );
    return destination;
  }
};
...
exports.getProxy = function (proxy, moduleName, isHttps) {
  if (!proxy && GlobalConfig.isEnvProxyActive()) {
    ...
    if (proxy) {
      Logger.getInstance().debug(
        `${moduleName} loads the proxy info from the environment variable host: ${proxy.host}`,
      );
    }
  }
  return proxy;
...
exports.hideEnvironmentProxy = function () {
  ...
  Logger.getInstance().debug(
    'As the useEnvProxy option is disabled, the proxy environment variables are temporarily hidden during the creation of an Azure client',
  );
  envProxyList = [];
...
function saveProxyInfoInList(envVar) {
  ...
  if (Util.exists(proxyEnv)) {
    Logger.getInstance().debug(
      `Temporarily exclude ${envVar} from the environment variable value: ${proxyEnv}`,
    );
  } else {
...
exports.restoreEnvironmentProxy = function () {
  ...
  for (const envVar of proxyEnvList) {
    if (Util.exists(nextValue)) {
      Logger.getInstance().debug(
        `The ${envVar} value exists with the value: ${nextValue} Restore back the proxy environment variable values`,
      );
    ...
    if (!Util.isWindows()) {
      if (Util.exists(nextValue)) {
        Logger.getInstance().debug(
          `The ${envVar.toUpperCase()} value exists with the value: ${nextValue} Restore back the proxy environment variable values (for Non-Windows machine)`,
        );
  ...
  Logger.getInstance().debug(
    'An Azure client has been created. Restore back the proxy environment variable values',
  );
};
...
exports.stringifyProxy = function (proxy) {
  ...
  return (
    `${proxy.protocol.startsWith('https') ? 'https' : 'http'}://` +
