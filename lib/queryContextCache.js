...
/**
   ...
   */
QueryContextCache.prototype.deserializeQueryContext = function(response) {
  let data
  try{
  ...
  if (!data||data.entries === null||typeof data === 'string') {
    // Clear the cache
    this.clearCache();
...
QueryContextCache.prototype.getQueryContextDTO = function (){
  ...
  return {
    entries: arr
    // entries: Array.from(this.getElements())
  };
};
