...
QueryContextCache.prototype.syncPriorityMap = function () {
  Logger.getInstance().debug(
    `syncPriorityMap called priorityMap size = ${this.priorityMap.size}, newPriorityMap size = ${this.newPriorityMap.size}`
  );
...
QueryContextCache.prototype.deserializeQueryContextElement = function (node) {
  ...
  if (typeof timestampNode === 'number') {
    ...
    Logger.getInstance().warn('deserializeQueryContextElement: `timestamp` field is not Number type');
    return null;
  }
  ...
  if (typeof priorityNode === 'number') {
    ...
    Logger.getInstance().warn('deserializeQueryContextElement: `priority` field is not Number type');
    return null;
  }
