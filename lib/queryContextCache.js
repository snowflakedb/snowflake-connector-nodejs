...
function QueryContextCache(capacity, sessionId) {
  Logger.getInstance().debug(
    `Creating new QueryContextCache with capacity ${capacity} for session ${sessionId}`,
  );
...
QueryContextCache.prototype.replaceQCE = function (oldQCE, newQCE) {
  ...
  Logger.getInstance().debug(
    `QCC session ${this.sessionId} - Replaced QCE: ${JSON.stringify(oldQCE)} with ${JSON.stringify(newQCE)}`,
  );
};
...
QueryContextCache.prototype.merge = function (newQCE) {
  Logger.getInstance().debug(
    `QCC session ${this.sessionId} - Merging QCE: ${JSON.stringify(newQCE)}`,
  );
  if (this.idMap.has(newQCE.id)) {
    Logger.getInstance().debug(
      `QCC session ${this.sessionId} - Element id ${newQCE.id} found in cache`,
    );
    ...
    if (newQCE.timestamp > qce.timestamp) {
      Logger.getInstance().trace(
        `QCC session ${this.sessionId} - New element is more recent. Current timestamp: ${qce.timestamp}, new timestamp: ${newQCE.timestamp}`,
      );
      if (qce.priority === newQCE.priority) {
        Logger.getInstance().trace(
          `QCC session ${this.sessionId} - Element priority (${qce.priority}) is the same`,
        );
        ...
        Logger.getInstance().trace(
          `QCC session ${this.sessionId} - Element priority changed. Current priority: ${qce.priority}, new priority: ${newQCE.priority}`,
        );
        // Change in priority
      ...
      Logger.getInstance().trace(
        `QCC session ${this.sessionId} - Element timestamp is the same, but priority changes. Current priority: ${qce.priority}, new priority: ${newQCE.priority}`,
      );
      // Same read timestamp but change in priority
      this.replaceQCE(qce, newQCE);
    } else {
      Logger.getInstance().trace(
        `QCC session ${this.sessionId} - Element is the same. Doing nothing.`,
      );
    }
  } else {
    ...
    if (this.priorityMap.has(newQCE.priority)) {
      ...
      Logger.getInstance().trace(
        `QCC session ${this.sessionId} - Element with the same priority found: ${JSON.stringify(qce)}. Replacing with new element: ${JSON.stringify(newQCE)}`,
      );
      // Replace with new data
      this.replaceQCE(qce, newQCE);
    } else {
      Logger.getInstance().debug(
        `QCC session ${this.sessionId} - Adding new element to the cache: ${JSON.stringify(newQCE)}`,
      );
      // new priority
      // Add new element in the cache
...
QueryContextCache.prototype.clearCache = function () {
  ...
  this.treeSet.clear();
  Logger.getInstance().trace(
    `QCC session ${this.sessionId} - clearCache() returns. Number of entries in cache now ${this.treeSet.size}`,
  );
};
...
QueryContextCache.prototype.deserializeQueryContext = function (data) {
  ...
  Logger.getInstance().debug(
    `QCC session ${this.sessionId} - deserializeQueryContext() called: data from server: ${stringifyData}`,
  );
  ...
  try {
    ...
    Logger.getInstance().debug(
      `QCC session ${this.sessionId} - deserializeQueryContextJson: Exception = ${e.getMessage}`,
    );
...
QueryContextCache.prototype.deserializeQueryContextElement = function (node) {
  ...
  if (typeof context === 'string') {
    ...
    entry.context = null;
    Logger.getInstance().debug(
      `QCC session ${this.sessionId} - deserializeQueryContextElement \`context\` field is empty`,
    );
  } else {
    Logger.getInstance().warn(
      `QCC session ${this.sessionId} - deserializeQueryContextElement: \`context\` field is not String type`,
    );
    return null;
  }
