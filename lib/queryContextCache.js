...
QueryContextCache.prototype.addQCE = function (qce) {
  ...
  this.sortTreeSet();
  Logger.getInstance().debug(`QCC session ${this.sessionId} - Added QCE: ${JSON.stringify(qce)}`);
};
...
QueryContextCache.prototype.removeQCE = function (qce) {
  ...
  Logger.getInstance().debug(`QCC session ${this.sessionId} - Removed QCE: ${JSON.stringify(qce)}`);
};
...
QueryContextCache.prototype.merge = function (newQCE) {
  ...
  if (this.idMap.has(newQCE.id)) {
    ...
    if (newQCE.timestamp > qce.timestamp) {
      Logger.getInstance().debug(`QCC session ${this.sessionId} - New element is more recent. Current timestamp: ${qce.timestamp}, new timestamp: ${newQCE.timestamp}`);
      if (qce.priority === newQCE.priority) {
        Logger.getInstance().debug(`QCC session ${this.sessionId} - Element priority (${qce.priority}) is the same`);
        // Same priority, overwrite new data at same place
        qce.timestamp = newQCE.timestamp;
        qce.context = newQCE.context;
      } else {
        Logger.getInstance().debug(`QCC session ${this.sessionId} - Element priority changed. Current priority: ${qce.priority}, new priority: ${newQCE.priority}`);
        // Change in priority
        this.replaceQCE(qce, newQCE);
      } 
    } else if (newQCE.timestamp === qce.timestamp && qce.priority !== newQCE.priority) {
      Logger.getInstance().debug(`QCC session ${this.sessionId} - Element timestamp is the same, but priority changes. Current priority: ${qce.priority}, new priority: ${newQCE.priority}`);
      // Same read timestamp but change in priority
      this.replaceQCE(qce, newQCE);
    } else {
      Logger.getInstance().debug(`QCC session ${this.sessionId} - Element is the same. Doing nothing.`);
    }
  } else {
    Logger.getInstance().debug(`QCC session ${this.sessionId} - New element`);
    // new id
    if (this.priorityMap.has(newQCE.priority)) {
      ...
      Logger.getInstance().debug(`QCC session ${this.sessionId} - Element with the same priority found: ${JSON.stringify(qce)}. Replacing with new element: ${JSON.stringify(newQCE)}`);
...
QueryContextCache.prototype.clearCache = function () {
  Logger.getInstance().debug(`QCC session ${this.sessionId} - clearCache() called`);
  this.idMap.clear();
  this.priorityMap.clear();
  this.treeSet.clear();
  Logger.getInstance().debug(`QCC session ${this.sessionId} - clearCache() returns. Number of entries in cache now ${this.treeSet.size}`);
};
...
QueryContextCache.prototype.deserializeQueryContext = function (data) {
  ...
  if (!data || stringifyData === '{}' || data.entries === null) {
    ...
    this.clearCache();
    Logger.getInstance().debug(`QCC session ${this.sessionId} - deserializeQueryContext() returns`);
