...
describe('RequestUtil', function () {
  describe('describeRequestFromOptions', function () {
    it('should describe request with default attributes when given basic request options', function () {
      ...
      assert.ok(
        result.includes(`${sfParams.paramsNames.SF_REQUEST_ID}=12345`),
        'Should include requestId',
      );
      assert.ok(
        result.includes(`${sfParams.paramsNames.SF_REQUEST_GUID}=abcde`),
        'Should include request_guid',
      );
      assert.ok(
        result.includes(`${sfParams.paramsNames.SF_WAREHOUSE_NAME}=test_warehouse`),
        'Should include warehouse',
      );
      assert.ok(
        result.includes(`${sfParams.paramsNames.SF_DB_NAME}=test_db`),
        'Should include databaseName',
      );
      assert.ok(
        result.includes(`${sfParams.paramsNames.SF_SCHEMA_NAME}=test_schema`),
        'Should include schemaName',
      );
    ...
    it('should use overridden attributes lists if provided', function () {
      ...
      assert.ok(
        result.includes('baseUrl=https://example.org'),
        'Should include only baseUrl with value',
      );
  ...
  describe('describeRequestFromResponse', function () {
    it('should describe the request from a response config', function () {
      ...
      assert.ok(
        result.includes(`${sfParams.paramsNames.SF_REQUEST_GUID}=xyz123`),
        'Should have request_guid',
      );
      assert.ok(
        result.includes(`${sfParams.paramsNames.SF_WAREHOUSE_NAME}=wh1`),
        'Should have warehouse',
      );
    });
