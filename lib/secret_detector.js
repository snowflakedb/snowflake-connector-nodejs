...
function SecretDetector(customPatterns, mock) {
  ...
  if (customPatterns) {
    ...
    if (customPatterns.regex.length !== customPatterns.mask.length) {
      throw new Error(
        "The customPatterns object must have equal length for both 'regex' and 'mask'",
      );
    }
  ...
  const AWS_KEY_PATTERN = new RegExp(
    String.raw`(aws_key_id|aws_secret_key|access_key_id|secret_access_key)\s*=\s*'([^']+)'`,
    'gi',
  );
  const AWS_TOKEN_PATTERN = new RegExp(
    String.raw`(accessToken|tempToken|keySecret)\s*:\s*"([a-z0-9/+]{32,}={0,2})"`,
    'gi',
  );
  ...
  const PRIVATE_KEY_DATA_PATTERN = new RegExp(
    String.raw`"privateKeyData": "([a-z0-9/+=\\n]{10,})"`,
    'gim',
  );
  ...
  const CONNECTION_TOKEN_PATTERN = new RegExp(
    String.raw`(token|assertion content)([\'\"\s:=]+)([a-z0-9=/:_\%\-\+]{8,})`,
    'gi',
  );
  ...
  const PASSCODE_PATTERN = new RegExp(
    String.raw`(passcode|otp|pin|otac)\s*([:=])\s*([0-9]{4,6})`,
    'gi',
  );

  const OAUTH_CLIENT_SECRET_PATTERN = new RegExp(
    String.raw`(oauthClientId|oauthClientSecret|clientSecret)([\'\"\s:=]+)([a-z0-9!\"#\$%&\\\'\(\)\*\+\,-\./:;<=>\?\@\[\]\^_` +
      '`' +
      String.raw`\{\|\}~]{8,})`,
    'gi',
  );
