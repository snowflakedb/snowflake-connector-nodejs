...
function LargeResultSetService(connectionConfig, httpClient) {
  ...
  this.getObject = function getObject(options) {
    ...
    const callback = function callback(err, response, body) {
      ...
      const debugUrl = options.url.slice(
        options.url.indexOf('results/') + 'results/'.length,
        options.url.indexOf('?'),
      );
      Logger.getInstance().info(`${logPrefix} - url: ${debugUrl}`);
      const debugHeaders = {
        'content-type': response.headers['content-type'],
        'content-length': response.headers['content-length'],
      };
      Logger.getInstance().info(`${logPrefix} - headers: ${JSON.stringify(debugHeaders)}`);
      Logger.getInstance().info(
        `${logPrefix} - parsedBodyLength: ${body ? body.length : 'no body'}`,
      );
      ...
      if (Util.isFunction(options.callback)) {
        try {
          ...
          Logger.getInstance().error(
            `Callback failed for ${options.__debug.statementId} - ${debugUrl} with ${e}`,
          );
        }
      }
    };
