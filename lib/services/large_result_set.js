...
function LargeResultSetService(connectionConfig, httpClient) {
  ...
  this.getObject = function getObject(options) {
    ...
    const callback = function callback(err, response, body) {
      const logPrefix = `LargeResultSetService.getObject[${options.__debug.statementId}]`;
      Logger.getInstance().info(`${logPrefix} - rowCount: ${options.__debug.rowCount}`);
      Logger.getInstance().info(
        `${logPrefix} - compressedSize: ${options.__debug.compressedSize}, uncompressedSize: ${options.__debug.uncompressedSize}`,
      );
      Logger.getInstance().info(`${logPrefix} - url: ${options.url}`);
      Logger.getInstance().info(`${logPrefix} - headers: ${JSON.stringify(options.headers)}`);
      Logger.getInstance().info(`${logPrefix} - err: ${JSON.stringify(err).slice(0, 30)}`);
      Logger.getInstance().info(`${logPrefix} - body: ${JSON.stringify(body).slice(0, 30)}`);
      ...
      if (err || isUnsuccessfulResponse(response)) {
        ...
        if (numRetries < maxNumRetries && isRetryableError(response, err)) {
          ...
          Logger.getInstance().trace(
            'Request will be retried after %d milliseconds',
            Math.floor(nextSendRequestWaitTimeMs),
          );
          ...
          if (isUnsuccessfulResponse(response)) {
            err = Errors.createLargeResultSetError(
              ErrorCodes.ERR_LARGE_RESULT_SET_RESPONSE_FAILURE,
              response,
            );
          } else {
            err = Errors.createNetworkError(
              ErrorCodes.ERR_LARGE_RESULT_SET_NETWORK_COULD_NOT_CONNECT,
              err,
            );
          }
        }
      ...
      if (Util.isFunction(options.callback)) {
        try {
          ...
          Logger.getInstance().error(
            `Callback failed for ${options.__debug.statementId} - ${options.url} with ${e}`,
          );
        }
      }
    };
