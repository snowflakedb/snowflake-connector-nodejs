...
const { getAuthenticator } = require('../authentication/authentication');
const authWeb = require('../authentication/auth_web');
        ...
        callback: async function (err, response, body) {
          ...
          if (err) {
            ...
            if (body && !body.success) {
              ...
              if (auth instanceof authWeb && body.code === GSErrors.code.ID_TOKEN_INVALID) {
                err = Errors.createRequestFailedError(
                  ErrorCodes.ERR_SF_RESPONSE_FAILURE, response);
