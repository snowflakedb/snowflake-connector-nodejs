...
StateConnecting.prototype.continue = function () {
  ...
  let numRetries = connectionConfig.getNumofRetries();
  let sleep = connectionConfig.getRetrySfStartingSleepTime();
  let totalElaspedTime = sleep + connectionConfig.getElapsedTime();
    ...
    if (!err) {
      ...
      if (Errors.isNetworkError(err) || Errors.isRequestFailedError(err)) {
        if (numRetries < maxLoginRetries && (
          ...
          (maxRetryTimeout === 0 || totalElaspedTime + connectionConfig.getElapsedTime() < maxRetryTimeout)) {
          connectionConfig.setNumofRetries(++numRetries);
          ...
          connectionConfig.setElapsedTime(totalElaspedTime);
