...
function StateAbstract(options)
{
  ...
  function sendHttpRequest(requestOptions, httpClient)
  {
    ...
    const realRequestOptions =
      {
        ...
        callback: async function (err, response, body)
        {
          ...
          this.retryReason = response.statusCode;
        }
      };
    ...
    if (requestOptions.retry > 2)
    {
      ...
      if(httpClient._connectionConfig.getIncludeRetryReason()){
        realRequestOptions.url += ("&" + 'retryReason=' + realRequestOptions.retryReason )
      }
    }
