...
const { getCurrentAuth, getAuthenticator } = require('../authentication/authentication');
    ...
    if (!err) {
      ...
      if (Errors.isNetworkError(err) || Errors.isRequestFailedError(err)) {
        if (numRetries < maxLoginRetries && (
          ...
          (maxRetryTimeout === 0 || totalElaspedTime < maxRetryTimeout)) {
          numRetries++;
          const jitter = Util.getJitteredSleepTime(numRetries, sleep, totalElaspedTime, maxRetryTimeout);
          sleep = jitter.sleep;
          totalElaspedTime = jitter.totalElaspedTime;
          ...
          if (auth instanceof AuthOkta) {
            ...
            const retryOption = {
              remainingTimeout: maxRetryTimeout - totalElaspedTime, 
              numRetries: numRetries,
            }
            auth.reauthenticate(requestOptions.json, retryOption).then(() =>{
              numRetries = retryOption.numRetries;
              remainingTimeout = maxRetryTimeout - remainingTimeout;
              return;
            });
          } else {
            return;
