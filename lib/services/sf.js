        ...
        if (err) {
          ...
          if (response.statusCode === 401 && response.body) {
            ...
            try {
              ...
              Logger().debug('HTTP Error: %s', response.statusCode);
            }
            if (innerCode === '390104') {
          ...
          if (response.request.isRedirect && !body?.success) {
            Logger().debug('Request was redirected. Retry again');
            err = Errors.createRequestFailedError(
              ErrorCodes.ERR_SF_RESPONSE_REDIRECT_ERROR,
              response,
            );
          }
          // if success flag is false, the operation we tried to perform failed
            ...
            if (
              ...
              Logger().debug('ID Token being used has expired. Reauthenticating');
...
StateConnecting.prototype.continue = function () {
  ...
  const requestCallback = async function (err, body) {
    ...
    if (!err) {
      ...
      Logger().debug(`New session with id ${parent.snowflakeService.getSessionId()} initialized`);
      ...
      if (Errors.isNetworkError(err) || Errors.isRequestFailedError(err)) {
        if (
          ...
          if (sleep <= 0) {
            Logger().debug('Reached out to the max Login Timeout');
          ...
          if (auth instanceof AuthOkta) {
            Logger().debug('OKTA authentication requires token refresh.');
            const retryOption = {
              totalElapsedTime,
              numRetries,
            };
            ...
            Logger().debug('AuthKeyPair authentication requires token refresh.');
            await auth.reauthenticate(context.options.json);
          }
          setTimeout(sendRequest, sleep * 1000);
          return;
        } else {
          Logger().debug('Failed to all retries to SF.');
  ...
  const sendRequest = function () {
    ...
    Logger().debug('Contacting SF: %s, (%s/%s)', targetUrl, numRetries, maxLoginRetries);
