...
const CredentialManager = require('../authentication/SecureStorage/credentialManager');
          ...
          else
          {
            ...
            if (body && !body.success)
            {
              ...
              if(body.code === '390195' && data.authnMethod === 'TOKEN') {
                ...
                CredentialManager.deleteCredential(connectionConfig.account, connectionConfig.username, authenticationTypes.ID_TOKEN_AUTHENTICATOR);
    ...
    if (!err)
    {
      ...
      if (connectionConfig.getClientStoreTemporaryCredential()) {
        if(body.data.idToken){
          CredentialManager.writeCredential(connectionConfig.account, connectionConfig.username, 
            authenticationTypes.ID_TOKEN_AUTHENTICATOR, body.data.idToken);

          Logger.getInstance().debug("Id_token is saved");
        }
      }
