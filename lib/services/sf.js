...
function SnowflakeService(connectionConfig, httpClient, config)
{
  ...
  var transitionTo = function (state, transitionContext)
  {
    ...
    if (currentState !== state)
    {
      ...
      // const option = state.connectionConfig;

      // if (option.getClientStoreTemporaryCredential()) {
      //   SecureStorage.writeCredential(option.account, option.username, 'ID_TOKEN', state.tokenInfo.getIdToken()).then(() =>
      //   {
      //     Logger.getInstance().info("Id_token is saved");
      //   });

      //   if (option.getClientRequestMFAToken()) {
      //     SecureStorage.writeCredential(option.account, option.username, 'MFA_TOKEN', state.tokenInfo.getMFAToken()).then(() =>
      //     {
    ...
    const realRequestOptions =
      {
        ...
        callback: async function (err, response, body)
        {
          ...
          if (connectionConfig.getClientStoreTemporaryCredential()) {
            if(body.data.idToken){
              await SecureStorage.writeCredential(connectionConfig.account, connectionConfig.username, "ID_TOKEN", body.data.idToken);
            }

            if (connectionConfig.getClientRequestMFAToken() && body.data.mfaToken) {
              await SecureStorage.writeCredential(connectionConfig.account, connectionConfig.username, "MFA_TOKEN", body.data.mfaToken);
            }
          }
...
StateConnecting.prototype.continue = function ()
{
  ...
  if(Util.exists(this.connectionConfig))
...
function TokenInfo(config)
{
  ...
  let idTokenExpirationTime;
  let mfaTokenExpirationTime
  ...
  this.clearTokens = function ()
  {
    ...
    idToken = undefined;
    idTokenExpirationTime = undefined;
    mfaToken = undefined;
    mfaTokenExpirationTime = undefined;
  };
  ...
  this.update = function (data)
  {
    ...
    idTokenExpirationTime = currentTime + 1000 * (data.idTokenValidityInSeconds);
    mfaTokenExpirationTime = currentTime + 1000 * (data.mfaTokenValidityInSeconds);
  };
