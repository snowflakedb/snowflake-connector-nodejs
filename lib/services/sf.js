...
function SnowflakeService(connectionConfig, httpClient, config) {
  ...
  if (Util.exists(config)) {
    ...
    if (!connectionConfig.masterToken) {
      Logger.getInstance().warn(
        'masterToken not provided with sessionToken; using sessionToken as masterToken. ' +
          'Token renewal will fail if the session token expires.',
      );
    }
    tokenInfoConfig = {
...
StateConnected.prototype.connect = function (options) {
  // If session tokens were pre-supplied (no login was needed), treat connect()
  // as a no-op success. This allows pools and other callers that always invoke
  // connect() to work transparently with pre-authenticated sessions.
  if (this.connectionConfig.getAuthenticator() === AuthenticationTypes.SESSION_TOKEN) {
    const callback = options.callback;
    process.nextTick(function () {
      callback(null);
    });
