          ...
          else
          {
            ...
            if (body && !body.success)
            {
              ...
              if(body.code === '390195' && data.authnMethod === 'TOKEN') {
                ...
                const {CredentialManager} = require('../global_config');
                CredentialManager.remove(connectionConfig.account, connectionConfig.username, authenticationTypes.ID_TOKEN_AUTHENTICATOR);
    ...
    if (!err)
    {
      ...
      if (connectionConfig.getClientStoreTemporaryCredential()) {
        if(body.data.idToken){
          CredentialManager.write(connectionConfig.account, connectionConfig.username, 
