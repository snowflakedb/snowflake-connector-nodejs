...
} = require('../telemetry/inband_telemetry');
// TODO(SNOW-3548350): remove with the telemetry emission (target: 2026-11-30).
const { shapeToTelemetryPayload } = require('../connection/connection_identifier_shape');
  ...
  Request.prototype.sendAsync = async function () {
    ...
    return new Promise((resolve, reject) => {
      sendHttpRequest(
        ...
        httpClient,
        snowflakeService.getAuthenticator(),
      );
    });
  };
...
StateConnecting.prototype.continue = async function () {
  ...
  const requestCallback = async function (err, body) {
    ...
    if (!err) {
      ...
      // TODO(SNOW-3548350): remove this block when the
      // connection-identifier-shape data collection wraps up
      // (target: 2026-11-30).
      //
      // Emit one in-band telemetry record per successful login describing
      // which connection-identifier fields the user supplied (no hostname
      // or account value is included). Mirrors PR snowflakedb/gosnowflake#1797
      // (Go) and PR snowflakedb/snowflake-connector-python#2877 (Python).
      // Like the `minicore_error` emit above this is unconditional —
      // it does not gate on `CLIENT_TELEMETRY_ENABLED`, matching the
      // existing in-band-telemetry precedent in this driver and
      // intentionally diverging from Go/Python/JDBC. The local kill
      // switch is `SF_TELEMETRY_DISABLE_CONNECTION_SHAPE=true`.
      if (!isConnectionShapeTelemetryDisabledByEnv()) {
        const inputShape =
          typeof connectionConfig._getInputShape === 'function'
            ? connectionConfig._getInputShape()
            : undefined;
        if (inputShape) {
          const shapeRequest = buildInbandTelemetryRequest(
            connectionConfig,
            CLIENT_CONNECTION_IDENTIFIER_SHAPE,
            shapeToTelemetryPayload(inputShape),
          );
          parent
