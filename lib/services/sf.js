        ...
        callback: async function (err, response, body)
        {
          ...
          else
          {
            ...
            if (body && !body.success)
            {
              ...
              if(body.code === '390100' && data.authnMethod === 'USERNAME_PASSWORD') {
                Logger.getInstance().trace("MFA Token being used has expired. Reauthenticating");
                await SecureStorage.deleteCredential(connectionConfig.account, connectionConfig.username, authenticationTypes.MFA_TOKEN_AUTHENTICATOR);
                await connectionConfig.auth.reauthenticate(requestOptions.json);
                httpClient.request(realRequestOptions);
                return;
              }
