 ...
 */
exports.nextSleepTime = function (base, cap, previousSleep) {
  return Math.min(
    cap,
    Math.abs(previousSleep * 3 - base) * Math.random() + Math.min(base, previousSleep * 3),
  );
};
 ...
 */
exports.getJitteredSleepTime = function (
  numofRetries,
  currentSleepTime,
  totalElapsedTime,
  maxRetryTimeout,
  ...
  const sleep =
    maxRetryTimeout !== 0 ? Math.min(maxRetryTimeout - totalElapsedTime, nextsleep) : nextsleep;
...
function getNextSleepTime(numofRetries, currentSleepTime) {
  ...
  return chooseRandom(
    currentSleepTime + getJitter(currentSleepTime),
    nextSleep + getJitter(currentSleepTime),
  );
}
...
exports.checkValidCustomCredentialManager = function (customCredentialManager) {
  ...
  for (const method of requireMethods) {
    if (
      !Object.hasOwnProperty.call(customCredentialManager, method) ||
...
exports.shouldRetryOktaAuth = function ({
  maxRetryTimeout,
  maxRetryCount,
  numRetries,
  startTime,
  ...
  return (
    (maxRetryTimeout === 0 || Date.now() < startTime + remainingTimeout) &&
...
exports.validatePath = function (dir) {
  try {
    ...
    Logger.getInstance().error(
      'The location is invalid. Please check this location is accessible or existing',
    );
    return false;
  }
};
    ...
    s.once('error', (err) => {
      ...
      if (err['code'] === 'EADDRINUSE') {
        ...
        Logger.getInstance().trace(
          `There is unexpected error during verification of port availability. Port: ${port}. Error: ${JSON.stringify(err)}`,
        );
      }
    });
    s.once('listening', () => {
      s.close();
      Logger.getInstance().trace(
        `Closing server run for verification whether the port is available. Port: ${port}`,
      );
      resolve('Listening');
    });
    s.listen(port);
