...
exports.getProxyFromEnv = function (isHttps) {
  let proxyFromEnv;
  if (!isHttps) {
    proxyFromEnv = this.getEnvVar('http_proxy');
    Logger.getInstance().debug('Util.getProxyEnv: Using HTTP_PROXY from the environment variable');
  } else {
    proxyFromEnv = this.getEnvVar('https_proxy');
    Logger.getInstance().debug('Util.getProxyEnv: Using HTTPS_PROXY from the environment variable');
  }
