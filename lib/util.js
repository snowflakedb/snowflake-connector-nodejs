 ...
 * @param filePath
 * @param expectedMode
 * @param fsPromisesModule
 * @returns {Promise<boolean>} resolves always to true for Windows
 */
exports.isFileModeCorrect = async function (filePath, expectedMode, fsPromisesModule) {
  if (os.platform() === 'win32') {
    return true;
  }
  return await fsPromisesModule.stat(filePath).then((stats) => {
 ...
 * @returns {Promise<boolean>} resolves always to true for Windows
 */
exports.isFileWritableOnlyByUser = async function (configFilePath, fsPromisesModule) {
  if (os.platform() === 'win32') {
    return true;
  }
  const stats = await fsPromisesModule.stat(configFilePath);
