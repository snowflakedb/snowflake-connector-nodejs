...
describe.skip('system$get_objects()', function () {
  ...
  const createStage = 'create or replace stage test_stage ' + "url = 's3://some_url';";
  const createFileFormat = 'create or replace file format ' + "test_file_format type = 'csv';";
  const createSequence = 'create or replace sequence test_sequence;';
  const createSqlUdfAdd1Number =
    'create or replace function add1 (n number) ' + "returns number as 'n + 1';";
  const createSqlUdfAdd1String =
    'create or replace function add1 (s string) ' + "returns string as 's || ''1''';";
  ...
  it('desc file format', function (done) {
    testGetObjectsOnStmt({
      ...
      output: {
        FILE_FORMAT: ['S3TESTACCOUNT.NODE_TESTDB.NODE_TESTSCHEMA.TEST_FILE_FORMAT'],
      },
      callback: done,
    });
  });
  ...
  async.series(
    [
      ...
      function (callback) {
        ...
        const sqlText = util.format('%s as "%s";', buildSqlSystem$GetObjects(queryId), columnName);
