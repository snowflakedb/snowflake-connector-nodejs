...
describe.skip('exclude support warehouses', function () {
  const createSysWh = "create or replace warehouse syswh warehouse_size = 'xsmall'";
  const dropSysWh = 'drop warehouse syswh';

  const testWhName = 'SF_TEST_WH';

  const createTestWh = util.format(
    'create or replace warehouse %s ' + "warehouse_size = 'xsmall'",
    testWhName,
  );
    ...
    async.series(
      [
        ...
        function (cb) {
          ...
          getWarehouseId(connSnowflake, 'EXTERNALACCOUNT', testWhName, function (warehouseId) {
  ...
  function getWarehouseId(conn, accountName, warehouseName, callback) {
    const sqlText = util.format("show warehouses like '%s' in %s", warehouseName, accountName);
   ...
   * @param callback the callback to invoke once the operation is complete.
   */
  function setWhSnowflakeSupportFlag(conn, warehouseId, snowflakeSupportFlag, callback) {
    const sqlText = util.format(
      'select system$set_wh_snowflake_support_flag(%s, %s);',
      warehouseId,
      snowflakeSupportFlag,
    );
