...
describe('MFA authentication', function () {
  ...
  async function getTotp(authTest, seed = '') {
    ...
    if (!authTest.runAuthTestsManually) {
      try {
        const result = await authTest.execWithTimeout('node', [totpGeneratorPath, seed], 40000);
  ...
  it('MFA successful authentication with TOTP codes', async function () {
    ...
    if (!connectionSuccess) {
      ...
      throw new Error(
        `Failed to connect with any of the ${totpCodes.length} TOTP codes. Last error: ${errorMessage}`,
      );
    }
    ...
    if (authTest.error) {
      throw new Error(
        `Failed to connect with cached MFA token. Error: ${authTest.error.toString()}`,
      );
    }
