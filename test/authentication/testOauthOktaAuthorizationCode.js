...
describe('Oauth Okta Authorization code tests', function () {
  ...
  describe('Oauth Okta Authorization code tests', async () => {
    it('Successful connection', async () => {
      const connectionOption = { ...connParameters.oauthOktaAuthorizationCode, clientStoreTemporaryCredential: false };
      authTest.createConnection(connectionOption);
      const provideCredentialsPromise = authTest.execWithTimeout('node', [provideBrowserCredentialsPath, 'externalOauthOktaSuccess', login, password], 15000);
    ...
    it('Mismatched Username', async () => {
      const connectionOption = { ...connParameters.oauthOktaAuthorizationCode, username: 'differentUsername', clientStoreTemporaryCredential: false };
      authTest.createConnection(connectionOption);
      const provideCredentialsPromise = authTest.execWithTimeout('node', [provideBrowserCredentialsPath, 'externalOauthOktaSuccess', login, password], 15000);
      await authTest.connectAndProvideCredentials(provideCredentialsPromise);
    ...
    it('External browser timeout', async () => {
      const connectionOption = { ...connParameters.oauthOktaAuthorizationCode, browserActionTimeout: 100, clientStoreTemporaryCredential: false };
      authTest.createConnection(connectionOption);
  ...
  describe('Oauth Okta Authorization - token cache', async () => {
    const connectionOption = { ...connParameters.oauthOktaAuthorizationCode, clientStoreTemporaryCredential: true };
    ...
    it('refreshes the token for credential cache key', async function () {
      const newToken = await authUtil.readCache(accessTokenKey);
