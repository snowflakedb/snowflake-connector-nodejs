...
describe.only('Generic approach poc', () => {

  const repeatTimesPerfRun = 5;
  const perfTestResults = [];

  const runDroppingMinAndMax = async (name, fn) => {
    const results = [];
    ...
    avg /= (repeatTimesPerfRun - 2);
    // console.log(`Average time is ${avg} ms`);
    perfTestResults.push(`${name}|${parseInt(avg)} ms`);
  };

  after(() => {
    console.log('Perf test results');
    console.log('DRIVER|ROWS|FORMAT|TYPE|RUNS(drop worst and best)');
    console.log('=================================================');
    perfTestResults.forEach(line => console.log(line));
  });
      ...
      ['JSON', 'ARROW'].forEach(resultFormat => {
        ...
        [10, 10000, 1000000].forEach(sourceRowCount => {
          it(`GENERIC|${sourceRowCount}|${resultFormat}|ROWS|${repeatTimesPerfRun}`, async () => {
            await runDroppingMinAndMax(`GENERIC|${sourceRowCount}|${resultFormat}|ROWS|${repeatTimesPerfRun}`, async () => {
          ...
          it(`GENERIC|${sourceRowCount}|${resultFormat}|DELAYED|${repeatTimesPerfRun}`, async () => {
            await runDroppingMinAndMax(`GENERIC|${sourceRowCount}|${resultFormat}|DELAYED|${repeatTimesPerfRun}`, async () => {
          ...
          it(`GENERIC|${sourceRowCount}|${resultFormat}|STREAM|${repeatTimesPerfRun}`, async () => {
            await runDroppingMinAndMax(`GENERIC|${sourceRowCount}|${resultFormat}|STREAM|${repeatTimesPerfRun}`, async () => {
              let fetchedRows = 0;
              let invalidRows = 0;
              const options = {
                handleRow: row => {
  ...
  describe('Perf selects standard nodejs', () => {
    ...
    [10, 10000, 1000000].forEach(sourceRowCount => {
      ...
      it(`NODEJS|${sourceRowCount}|${resultFormat}|ROWS|${repeatTimesPerfRun}`, async () => {
        await runDroppingMinAndMax(`NODEJS|${sourceRowCount}|${resultFormat}|ROWS|${repeatTimesPerfRun}`, async () => {
      ...
      it(`NODEJS|${sourceRowCount}|${resultFormat}|STREAM|${repeatTimesPerfRun}`, async () => {
        await runDroppingMinAndMax(`NODEJS|${sourceRowCount}|${resultFormat}|STREAM|${repeatTimesPerfRun}`, async () => {
