...
describe('Execute test', function () {
  ...
  describe('testDescribeOnly', async function () {
    const selectWithDescribeOnly =
      "SELECT 1.0::NUMBER(30,2) as C1, 2::NUMBER(38,0) AS C2, 't3' AS C3, 4.2::DOUBLE AS C4, 'abcd'::BINARY(8388608) AS C5, true AS C6";
    const expectedRows = [
      { C1: 1, C2: 2, C3: 't3', C4: 4.2, C5: { type: 'Buffer', data: [171, 205] }, C6: true },
    ];
    const testCases = [
      {
    ...
    const executeQueryAndVerifyResultDependOnDescribeOnly = async (
      describeOnly,
        ...
        connection.execute({
          ...
          complete: (err, stmt, rows) => {
            ...
            if (rows.length > 0) {
              ...
              columnsNames.every((element, index) =>
                assert.strictEqual(element, columnsNamesInMetadata[index]),
              );
            }
            return resolve(rows);
          },
...
describe('Execute test - variant', function () {
  ...
  const createItCallback = function (testCase, rowAsserts) {
    return function (done) {
      ...
      const tempFilePath = testUtil.createTempFile(
        os.tmpdir(),
        sampleTempFile,
        testCase.sampleData,
      );
      ...
      testUtil
        .executeCmdAsync(connection, putVariant)
    ...
    const testCases = [
      ...
      },
      {
        ...
        assertionCheck: (row) => {
          ...
          assert.strictEqual(
            row[TEST_COL]['node'][TEST_HEADER][TEST_ATTRIBUTE_CUSTOM_PREFIX + TEST_ATTRIBUTE_NAME],
      ...
      },
      {
        ...
        assertionCheck: (row) => {
          assert.strictEqual(row[TEST_COL]['node'][TEST_HEADER][ELEMENT_VALUE_FIELD], TEST_XML_VAL);
          assert.equal(
            row[TEST_COL]['node'][TEST_HEADER]['attributes'][
              TEST_ATTRIBUTE_CUSTOM_PREFIX + TEST_ATTRIBUTE_NAME
            ],
            TEST_ATTRIBUTE_VALUE,
          );
        },
      },
      {
        ...
        assertionCheck: (row) => {
          assert.strictEqual(row[TEST_COL]['node'][TEST_HEADER][ELEMENT_VALUE_FIELD], TEST_XML_VAL);
          assert.strictEqual(
            row[TEST_COL]['node'][TEST_HEADER]['@_'][TEST_ATTRIBUTE_NAME],
            TEST_ATTRIBUTE_VALUE,
          );
        },
      },
  ...
  describe('connection.execute() Resubmitting requests using requestId and different connections', function () {
    ...
    it('Do not INSERT twice when the same request id and connection', async () => {
      let result;
      result = await testUtil.executeCmdAsync(
        firstConnection,
        "INSERT INTO test_request_id VALUES ('testValue');",
      );
      const requestId = result.statement.getRequestId();
      ...
      assert.strictEqual(result.statement.getRequestId(), requestId);

      result = await testUtil.executeCmdAsync(firstConnection, 'SELECT * from test_request_id ;');
    ...
    it('Execute SELECT for the same request id and different data', async () => {
      ...
      let result = await testUtil.executeCmdAsync(
        firstConnection,
        'SELECT * from test_request_id;',
      );
      assert.strictEqual(result.rows.length, 1);
      const requestId = result.statement.getRequestId();
