...
describe('ConnectionConfig: basic', function () {
  ...
  describe('test - get PROXY NO PROXY from the env variable ', function () {
    ...
    testCases.forEach(({ name, httpsProxy, httpProxy, noProxy, result }) => {
      it(name, function (){
        ...
        const connectionProxy =  new ConnectionConfig(mandatoryOption).getProxy();
        const keys = Object.keys(result);
        assert.strictEqual(keys.length, Object.keys(connectionProxy).length);

        for (const key in keys) {
          assert.strictEqual(connectionProxy[key], result[key]);
        }
      });
    });
  });
