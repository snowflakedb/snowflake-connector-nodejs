  ...
  it('small result', function (done) {
    ...
    ResultTestCommon.testResult(
      ...
      function (result) {
        ...
        assert.strictEqual(sessionState.getCurrentDatabase(), responseData.finalDatabaseName);

        assert.strictEqual(sessionState.getCurrentSchema(), responseData.finalSchemaName);

        assert.strictEqual(sessionState.getCurrentWarehouse(), responseData.finalWarehouseName);

        assert.strictEqual(sessionState.getCurrentRole(), responseData.finalRoleName);
        ...
        for (rowIndex = 0, rowsLength = rows.length; rowIndex < rowsLength; rowIndex++) {
          row = rows[rowIndex];
          ...
          for (
            columnIndex = 0, columnsLength = columns.length;
