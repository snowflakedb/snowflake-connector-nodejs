...
function TestingQCC() {
    ...
    for (let i = 0; i < MAX_CAPACITY; i++) {
      ...
      this.qcc.merge(
        new QueryContextElement(
          this.expectedIDs[i],
          this.expectedReadTimestamp[i],
          this.expectedPriority[i],
          Context,
        ),
      );
    }
  ...
  this.initCacheWithDataInRandomOrder = function () {
    ...
    this.qcc.merge(
      new QueryContextElement(
        this.expectedIDs[3],
        this.expectedReadTimestamp[3],
        this.expectedPriority[3],
        CONTEXT,
      ),
    );
    this.qcc.merge(
      new QueryContextElement(
        this.expectedIDs[2],
        this.expectedReadTimestamp[2],
        this.expectedPriority[2],
        CONTEXT,
      ),
    );
    this.qcc.merge(
      new QueryContextElement(
        this.expectedIDs[4],
        this.expectedReadTimestamp[4],
        this.expectedPriority[4],
        CONTEXT,
      ),
    );
    this.qcc.merge(
      new QueryContextElement(
        this.expectedIDs[0],
        this.expectedReadTimestamp[0],
        this.expectedPriority[0],
        CONTEXT,
      ),
    );
    this.qcc.merge(
      new QueryContextElement(
        this.expectedIDs[1],
        this.expectedReadTimestamp[1],
        this.expectedPriority[1],
        CONTEXT,
      ),
    );
  };
...
describe('Query Context Cache Test', function () {
  ...
  it('test - the number of contexts is over the size of capacity', function () {
    ...
    const extraQCE = new QueryContextElement(
      BASE_ID + i,
      BASE_READ_TIMESTAMP + i,
      BASE_PRIORITY + i,
      CONTEXT,
    );
  ...
  it('test updating timestamp', function () {
    ...
    const updatedQCE = new QueryContextElement(
      BASE_ID + updatedID,
  ...
  it('test updating priority', function () {
    ...
    const updatedQCE = new QueryContextElement(
      ...
      testingQcc.expectedPriority[updatedID],
      CONTEXT,
    );
    testingQcc.qcc.merge(updatedQCE);
    testingQcc.qcc.checkCacheCapacity();
  ...
  it('test - the same priority is added', function () {
    ...
    testingQcc.qcc.merge(
      new QueryContextElement(BASE_ID + i, BASE_READ_TIMESTAMP + i, updatedPriority, CONTEXT),
    );
    testingQcc.qcc.checkCacheCapacity();
  ...
  it('test - the new context has the same id but different timestamp ', function () {
    ...
    // Add one more element with same priority
    const i = 2;
    const samePriorityQCE = new QueryContextElement(
      BASE_ID + i,
      BASE_READ_TIMESTAMP + i - 10,
      BASE_PRIORITY + i,
      CONTEXT,
    );
