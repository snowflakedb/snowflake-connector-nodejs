...
describe('SocketUtil Test - simulating ProxyAgent creation', () => {
  const HttpProxyUtilFromEnvProxy = { host: 'localhost', port: 8080, protocol: 'http:', noProxy: undefined, useForOCSP: true };
  const HttpsProxyUtilFromEnvProxy = { host: 'localhost', port: 8080, protocol: 'https:', noProxy: undefined, useForOCSP: true };
  ...
  [
    ...
    it(`${name}`, () => {
      assert.deepEqual(agentWithoutClosingSlash, shouldMatch, `proxy string from within ProxyAgent was ${agentWithoutClosingSlash}, expected ${shouldMatch}`);
    });
  });
});